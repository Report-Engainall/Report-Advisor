-- A work item can only produce an outcome from an actionable state.
-- BLOCKED/CANCELLED/COMPLETED are terminal or non-actionable and must fail closed.
...
BEGIN
  ...
  IF v_status NOT IN ('OPEN','IN_PROGRESS') THEN
    RAISE EXCEPTION 'WORK_ITEM_NOT_ACTIONABLE';
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.complete_decision_work_item(uuid,numeric,jsonb) FROM PUBLIC, anon;
