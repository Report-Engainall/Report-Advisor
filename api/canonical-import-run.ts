...
function serviceRoleClient(): SupabaseClient {
  ...
  if (!url || !key) throw new Error('missing_supabase_worker_configuration');
  return createClient(url, key, {
    auth: { persistSession: false, autoRefreshToken: false, detectSessionInUrl: false },
  });
}
...
async function enqueueOrLoad(workerClient: SupabaseClient, companyId: string, jobKey: string, fileName: string, sourceHash: string) {
  const { data, error } = await workerClient.rpc('enqueue_report_execution_job', {
...
async function retryIfNeeded(workerClient: SupabaseClient, jobId: string, companyId: string, status: string): Promise<string> {
  if (status !== 'failed') return status;
  const { data, error } = await workerClient.rpc('retry_report_execution_job', { p_job_id: jobId, p_company_id: companyId });
  if (error) throw error;
...
export default async function handler(req: any, res: any) {
  ...
  if (!requireConfig(res, ['SUPABASE_URL', 'VITE_SUPABASE_ANON_KEY', 'SUPABASE_SERVICE_ROLE_KEY'])) return;
  ...
  try {
    ...
    const workerClient = serviceRoleClient();
    ...
    let job = await enqueueOrLoad(workerClient, companyId, jobKey, fileName, sourceHash);
    if (job.company_id !== companyId) throw new Error('REPORT_EXECUTION_JOB_TENANT_MISMATCH');
    job.status = await retryIfNeeded(workerClient, job.id, companyId, job.status);
    ...
    }, new (await import('../src/lib/report-execution/durable-worker-adapter.js')).SupabaseReportExecutionStore(workerClient));
