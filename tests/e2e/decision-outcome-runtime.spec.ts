...
test('real Evidence -> Recommendation -> Decision -> Approval -> Work -> Outcome', async ({ browser }) => {
  ...
  const recommendationId = await rpc(a, 'create_runtime_recommendation', {
    p_category: 'runtime-certification',
    p_priority: 'medium',
    ...
    p_expected_impact: 1,
    p_evidence_snapshot_id: evidence.id,
  ...
  const decisionId = await rpc(a, 'create_runtime_decision', {
    ...
    p_decision_type: 'runtime-certification',
    p_confidence: 1,
    p_expected_impact: 1,
  ...
  // Self-approval must fail. The real approval actor is authenticated separately below.
  const selfApproval = await a.evaluate(async ({ apiBase, anonKey, decisionId }) => {
    const authEntry = Object.entries(localStorage).find(([key]) => key.includes('-auth-token'))?.[1];
    const session = JSON.parse(String(authEntry));
    const response = await fetch(`${apiBase}/rest/v1/rpc/request_decision_approval`, { method: 'POST', headers: { apikey: anonKey, Authorization: `Bearer ${session.access_token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ p_decision_id: decisionId, p_reason: 'runtime certification' }) });
    return { status: response.status, body: await response.text() };
  }, { apiBase: BASE().apiBase, anonKey: BASE().anonKey, decisionId });
  expect(selfApproval.status).toBe(200);
  const approvalId = JSON.parse(selfApproval.body);
  ...
  const approval = await rpc(b, 'decide_approval', { p_approval_id: approvalId, p_approve: true, p_reason: 'Independent authenticated approval' });
  ...
  const workItemId = await rpc(b, 'create_decision_work_item', {
    ...
    p_assignee_id: actorA.userId,
    p_assignee_label: 'Authenticated certification assignee',
    p_title: `Runtime certification work ${suffix}`,
    p_description: 'Real persisted lifecycle work item created from the approved real-evidence decision.',
    p_priority: 'MEDIUM',
    p_due_at: null,
    p_expected_impact: 1,
    p_evidence_refs: [{ id: evidence.id, type: 'kpi_evidence_snapshot', kpi_key: evidence.kpi_key }],
  });
  ...
  const inProgress = await rest(a, `/rest/v1/decision_work_items?select=id,status,started_at&company_id=eq.${actorA.tenantId}&id=eq.${encodeURIComponent(workItemId)}`);
  expect(inProgress).toHaveLength(1);
  expect(inProgress[0].status).toBe('IN_PROGRESS');
  expect(inProgress[0].started_at).toBeTruthy();

  await expect.poll(async () => {
    const rows = await rest(a, `/rest/v1/decision_work_items?select=id,status&company_id=eq.${actorA.tenantId}&id=eq.${encodeURIComponent(workItemId)}`);
    return rows[0]?.status;
  }).toBe('IN_PROGRESS');

  expect(await rpc(a, 'complete_decision_work_item', {
    p_work_item_id: workItemId,
    p_actual_impact: 1,
    p_evidence: { source_evidence_snapshot_id: evidence.id, lifecycle: 'COMPLETED' },
  })).toBe(true);

  const completed = await rest(a, `/rest/v1/decision_work_items?select=id,decision_id,status,completed_at,actual_impact&company_id=eq.${actorA.tenantId}&id=eq.${encodeURIComponent(workItemId)}`);
  expect(completed).toHaveLength(1);
  expect(completed[0].status).toBe('COMPLETED');
  expect(completed[0].completed_at).toBeTruthy();

  const finalEvidence = await rpc(a, 'capture_kpi_evidence_snapshot', {
  ...
  const outcomeId = await rpc(a, 'record_decision_outcome', {
    p_decision_fingerprint: decisionKey,
    p_evidence_snapshot_id: String(finalEvidence.id),
    p_action_id: String(workItemId),
    p_observed_at: new Date().toISOString(),
    p_label: 'correct',
    p_actual_value: Number(finalEvidence.value),
    p_expected_value: Number(evidence.value),
    p_impact_value: 1,
    p_notes: 'Real final evidence persisted after completed work item.',
  });
  ...
  const terminalDecision = await rest(a, `/rest/v1/business_intelligence_decisions?select=id,status&company_id=eq.${actorA.tenantId}&id=eq.${encodeURIComponent(decisionId)}`);
  expect(terminalDecision).toHaveLength(1);
  expect(['APPROVED', 'EXECUTED']).toContain(terminalDecision[0].status);
