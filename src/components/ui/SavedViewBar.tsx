...
export function SavedViewBar<T extends Record<string, string | number | boolean | null>>({ value, onChange, names, activeName, storageReady, save, load, remove, reset, resetValue, labels = {}, className = '' }: SavedViewBarProps<T>) {
  ...
  return <div dir="rtl" className={`flex flex-col gap-2 rounded-2xl border border-ink-200 bg-white p-3 shadow-sm sm:flex-row sm:items-center ${className}`}>
    ...
    <div className="flex flex-1 items-center gap-2"><input value={name} onChange={event => setName(event.target.value)} className="input min-w-0 flex-1 py-2 text-xs" placeholder={labels.placeholder ?? 'اسم العرض'} aria-label="اسم العرض المحفوظ" />
      <button type="button" onClick={() => { if (save(name, value)) setName(''); }} className="btn-secondary shrink-0 px-3 text-xs" disabled={!name.trim()}><Save size={14}/> {labels.save ?? 'حفظ'}</button></div>
