...
function nativeFileName(filePath: string): string { return filePath.split(/[\\/]/).pop() || 'report'; }
...
export function FolderBatchImportPanel() {
  ...
  const nativeRootRef=useRef<string|null>(null); const storeRef=useRef(new IndexedDbFolderSnapshotStore());
  const isDesktop = Boolean(window.desktopFolderWatch?.isAvailable);

  const processNativeFile=async(payload:{path:string;reason:string;size:number;modifiedAt:string})=>{
    const api=window.desktopFolderWatch;if(!api)return;
    try{
      const buffer=await api.readFile(payload.path);
      const file=new File([buffer],nativeFileName(payload.path),{type:'application/octet-stream',lastModified:Date.parse(payload.modifiedAt)||Date.now()});
      await processFolderFiles([{file,format:'unknown',relativePath:payload.path}],entityType,setProgress);
    }catch(e:unknown){setError(e instanceof Error?e.message:'فشل معالجة الملف من مجلد سطح المكتب');}
  };

  useEffect(()=>{
    const api=window.desktopFolderWatch;if(!api?.isAvailable)return;
  ...
  const processScan=async()=>{
    ...
    if(changed.length===0){setProgress(p=>p?{...p,processed:p.total,current:p.current,results:p.results}:null);return;}
  ...
  const chooseAndRun=async()=>{
    ...
    if(isDesktop){
      try{
        setRunning(true);const api=window.desktopFolderWatch;if(!api)throw new Error('DESKTOP_WATCH_UNAVAILABLE');
        const selected=await api.selectDirectory();if(!selected)return;
        nativeRootRef.current=selected.path;setFolderName(selected.name||selected.path);setPathHint(selected.path);
        await api.start(selected.path);setWatching(true);
      }catch(e:unknown){setError(e instanceof Error?e.message:'فشل اختيار مجلد سطح المكتب');}
      finally{setRunning(false);}return;
    }
    try{setRunning(true);const handle=await selectWatchedFolder();handleRef.current=handle;await saveFolderHandle('default',handle);setFolderName(handle.name||pathHint||'المجلد المحدد');const files=await scanDirectory(handle);if(!files.length)throw new Error('لم يتم العثور على ملفات تقارير مدعومة داخل المجلد.');const result=await processFolderFiles(files,entityType,setProgress);setProgress(result);}catch(e:unknown){if(e instanceof DOMException&&e.name==='AbortError')return;setError(e instanceof Error?e.message:'فشل الوصول إلى المجلد');}finally{setRunning(false);}
  };
  ...
  return <Card><CardHeader title="سحب ومزامنة التقارير من مجلد" subtitle={isDesktop?'نسخة سطح المكتب: اختر مجلدًا حقيقيًا من Windows. يراقبه التطبيق محليًا، يلتقط الملفات الجديدة والمتغيرة، ويرسلها إلى نفس مسار الاستيراد المركزي.':'اختر مجلدًا مرة واحدة. يراقبه التطبيق، يكتشف الإضافات والتعديلات ببصمة SHA-256، ويتجاوز غير المتغير، ويواصل بقية التحليل حتى عند فشل استخراج ملف.'}/><CardBody>
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mb-4"><select value={entityType} onChange={e=>setEntityType(e.target.value as BatchEntityType)} className="input"><option value="sales_invoices">فواتير المبيعات</option><option value="products">المنتجات</option><option value="customers">العملاء</option></select><input value={pathHint} onChange={e=>setPathHint(e.target.value)} placeholder={isDesktop?'مسار المجلد يظهر هنا بعد الاختيار':'وصف المجلد'} className="input lg:col-span-2" readOnly={isDesktop}/></div>
    <div className="flex flex-wrap items-center gap-3"><button onClick={()=>void chooseAndRun()} disabled={running||watching} className="btn-primary inline-flex items-center gap-2">{running?<Loader2 size={16} className="animate-spin"/>:<FolderOpen size={16}/>} {running?'جارٍ السحب والمعالجة...':'اختيار المجلد ومعالجته'}</button>{!watching?<button onClick={()=>void startWatch()} disabled={running} className="btn-secondary inline-flex items-center gap-2">{isDesktop?<Monitor size={16}/>:<Radio size={16}/>} تشغيل المزامنة التلقائية</button>:<button onClick={stopWatch} className="btn-secondary inline-flex items-center gap-2"><Square size={15}/> إيقاف المزامنة</button>}{folderName&&<Badge variant="neutral">المجلد: {folderName}</Badge>}<Badge variant={watching?'success':'neutral'}>{watching?<><Radio size={12}/> مراقبة مستمرة</>:<><ShieldCheck size={12}/> {isDesktop?'مراقبة Windows الأصلية جاهزة':'قراءة محلية آمنة'}</>}</Badge></div>
    <p className="text-xs text-ink-400 mt-3">{isDesktop?'تعمل نسخة Windows عبر Native Host مع عزل الصلاحيات، حماية المسار داخل جذر المجلد، مراقبة أحداث الملفات مع فحص دوري احتياطي، ثم تمرير الملف إلى محرك الاستيراد المركزي. إغلاق النافذة يخفي التطبيق ويبقي المراقبة في الخلفية حتى اختيار خروج من شريط النظام.':'تعمل المراقبة داخل المتصفح بعد منح صلاحية المجلد. تُحفظ البصمات محليًا، وتُحفظ أدلة المجلد/الملف في قاعدة البيانات عند ربط جلسة المؤسسة. مسارات الشبكة UNC تحتاج Local Sync Agent لاحقًا لأنها لا تُمنح للويب بمجرد كتابة المسار.'}</p>
