...
function nativeFileName(relativePath: string): string {
  return relativePath.split(/[\\/]/).pop() || 'report';
}
...
export function FolderBatchImportPanel() {
  ...
  const processNativeFile = useCallback(async (payload: NativeFilePayload) => {
    ...
    try {
      ...
      const file = new File([buffer], nativeFileName(payload.relativePath), {
        type: 'application/octet-stream',
        lastModified: Date.parse(payload.modifiedAt) || Date.now(),
      });
      await processFolderFiles(
        [{ file, format: 'unknown', relativePath: payload.relativePath }],
        entityType,
        setProgress,
      );
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : 'فشل معالجة الملف من مجلد سطح المكتب');
    } finally {
  ...
  const chooseAndRun = async () => {
    ...
    if (isDesktop) {
      try {
        ...
        setError(e instanceof Error ? e.message : 'فشل اختيار مجلد سطح المكتب');
      } finally {
        setRunning(false);
      }
      return;
  ...
  const startWatch = async () => {
    ...
    if (isDesktop) {
      try {
        ...
        setError(e instanceof Error ? e.message : 'تعذر تشغيل المزامنة الأصلية لسطح المكتب.');
      }
      return;
    }

    try {
      let handle: BrowserDirectoryHandle | undefined = handleRef.current ?? await loadFolderHandle('default');
      ...
      stopRef.current = startWatchedFolder(
        handle,
        async () => {
      ...
      setError(e instanceof Error ? e.message : 'تعذر تشغيل المزامنة التلقائية.');
    }
  };
  ...
  return (
    <Card>
      <CardHeader
        title="سحب ومزامنة التقارير من مجلد"
        subtitle={isDesktop
          ? 'نسخة سطح المكتب: اختر مجلدًا حقيقيًا من Windows. يراقبه التطبيق محليًا، يلتقط الملفات الجديدة والمتغيرة، ويرسلها إلى نفس مسار الاستيراد المركزي.'
          : 'اختر مجلدًا مرة واحدة. يراقبه التطبيق، يكتشف الإضافات والتعديلات ببصمة SHA-256، ويتجاوز غير المتغير، ويواصل بقية التحليل حتى عند فشل استخراج ملف.'}
      />
      <CardBody>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mb-4">
          ...
          <input
            ...
            placeholder={isDesktop ? 'مسار المجلد يظهر هنا بعد الاختيار' : 'وصف المجلد'}
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button onClick={() => void chooseAndRun()} disabled={running || watching} className="btn-primary inline-flex items-center gap-2">
            {running ? <Loader2 size={16} className="animate-spin" /> : <FolderOpen size={16} />}
            {running ? 'جارٍ السحب والمعالجة...' : 'اختيار المجلد ومعالجته'}
          </button>
          {!watching ? (
            <button onClick={() => void startWatch()} disabled={running} className="btn-secondary inline-flex items-center gap-2">
              {isDesktop ? <Monitor size={16} /> : <Radio size={16} />} تشغيل المزامنة التلقائية
            </button>
          ) : (
            <button onClick={stopWatch} className="btn-secondary inline-flex items-center gap-2">
              <Square size={15} /> إيقاف المزامنة
            </button>
          )}
          {folderName && <Badge variant="neutral">المجلد: {folderName}</Badge>}
          <Badge variant={watching ? 'success' : 'neutral'}>
            {watching ? <><Radio size={12} /> مراقبة مستمرة</> : <><ShieldCheck size={12} /> {isDesktop ? 'مراقبة Windows الأصلية جاهزة' : 'قراءة محلية آمنة'}</>}
          </Badge>
        </div>

        <p className="text-xs text-ink-400 mt-3">
          {isDesktop
            ? 'تعمل نسخة Windows عبر Native Host مع عزل الصلاحيات، حماية المسار داخل جذر المجلد، مراقبة أحداث الملفات مع فحص دوري احتياطي، ثم تمرير الملف إلى محرك الاستيراد المركزي. اختيار المجلد يُحفظ محليًا على الجهاز ويمكن استئناف المراقبة بعد إعادة تشغيل التطبيق. إغلاق النافذة يخفي التطبيق ويبقي المراقبة في الخلفية حتى اختيار خروج من شريط النظام.'
            : 'تعمل المراقبة داخل المتصفح بعد منح صلاحية المجلد. تُحفظ البصمات محليًا، وتُحفظ أدلة المجلد/الملف في قاعدة البيانات عند ربط جلسة المؤسسة. مسارات الشبكة UNC تحتاج Local Sync Agent لاحقًا لأنها لا تُمنح للويب بمجرد كتابة المسار.'}
        </p>

        {error && (
          <div className="mt-4 p-3 rounded-lg bg-danger-50 text-danger-700 text-sm flex gap-2">
        ...
        {progress && (
          <div className="mt-5 space-y-3">
            <div className="flex justify-between text-sm">
              <b>{progress.current ? `يعالج: ${progress.current}` : 'اكتملت الدفعة'}</b>
              <span>{pct}%</span>
            </div>
            <div className="h-2 bg-ink-100 rounded-lg overflow-hidden">
              <div className="h-full bg-primary-500 transition-all" style={{ width: `${pct}%` }} />
            </div>
            <div className="flex flex-wrap gap-2">
              <Badge variant="success"><CheckCircle2 size={12} /> ناجح: {completed}</Badge>
              <Badge variant="danger"><XCircle size={12} /> فشل: {failed}</Badge>
              <Badge variant="warning">متجاوز/مكرر: {skipped}</Badge>
              <Badge variant="neutral">الإجمالي: {progress.total}</Badge>
            </div>
            {progress.results.length > 0 && (
              <div className="border rounded-lg divide-y max-h-72 overflow-auto">
                {progress.results.map((r, i) => (
                  <div key={`${r.name}-${i}`} className="p-3 flex items-center justify-between gap-3 text-sm">
                    <div className="min-w-0">
                      <div className="font-medium truncate">{r.name}</div>
                      <div className="text-xs text-ink-400">{r.path} · {r.format} · {r.rows} صف</div>
                      {(r.error || r.warning) && <div className="text-xs text-danger-600 mt-1">{r.error || r.warning}</div>}
                    </div>
                    <Badge variant={r.status === 'completed' ? 'success' : r.status === 'failed' ? 'danger' : 'warning'}>
                      {r.status === 'completed' ? `تم (${r.committed})` : r.status === 'failed' ? 'فشل' : 'متجاوز'}
                    </Badge>
                  </div>
                ))}
              </div>
