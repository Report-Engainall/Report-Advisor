...
import { Activity, BarChart3, Brain, ChevronDown, ClipboardCheck, Database, FileBarChart, Files, Gauge, Layers3, LayoutDashboard, ListChecks, LogOut, Package, Presentation, Scale, ScanSearch, Settings, Target, Upload, UserCircle, Users, Warehouse, AlertCircle, PlugZap } from 'lucide-react';
...
function CreditMark(){return <span className="text-[13px] font-black">â—«</span>}
function ScenarioMark(){return <span className="text-[12px] font-black">â—ژ</span>}
...
const sectionMeta: Record<string, { hint: string; tag: string }> = {
  'decision-center': { hint: 'ط§ظ„طµظˆط±ط© ط§ظ„طھظ†ظپظٹط°ظٹط© ظˆط§ظ„ظ‚ط±ط§ط±', tag: 'DECIDE' },
  'data-operations': { hint: 'ظ…طµط§ط¯ط±طŒ ظ…ط³طھظ†ط¯ط§طھ ظˆط¬ظˆط¯ط©', tag: 'DATA' },
  analytics: { hint: 'ط§ظ„ظ…ط¤ط´ط±ط§طھ ظˆط§ظ„طھظ‚ط§ط±ظٹط± ط§ظ„طھط¬ط§ط±ظٹط©', tag: 'BI' },
  intelligence: { hint: 'ط¥ط´ط§ط±ط§طھ ظˆط§ط³طھظƒط´ط§ظپ ط°ظƒظٹ', tag: 'INTEL' },
  reference: { hint: 'ظƒظٹط§ظ†ط§طھ ظˆظ…ط¹ط§ط±ظپ ط£ط³ط§ط³ظٹط©', tag: 'MASTER' },
  admin: { hint: 'ط§ظ„طھظ‡ظٹط¦ط© ظˆط¶ط¨ط· ظ…ط³ط§ط­ط© ط§ظ„ط¹ظ…ظ„', tag: 'ADMIN' },
};
...
export function Sidebar({alertCount=0,onNavigate,user}:{alertCount?:number;onNavigate?:()=>void;user?:User|null}){
 ...
 return <aside dir={language==='ar'?'rtl':'ltr'} className={'ag-sidebar flex h-screen w-[284px] shrink-0 flex-col border-ink-200 bg-white text-ink-900 '+(language==='ar'?'border-l':'border-r')}>
    ...
    <div className="ag-sidebar-workspace rounded-[14px] border border-primary-100 bg-gradient-to-br from-primary-50/80 via-white to-[#fff8e8] p-3 shadow-sm">
      <div className="flex items-center gap-2">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-[10px] font-black text-ink-800">ظ…ط±ظƒط² ط§ظ„ظ‚ط±ط§ط±</div>
          <div className="mt-0.5 text-[9px] text-ink-400">{workspaceMode==='essential'?'ط£ط³ط§ط³ظٹط©':workspaceMode==='advanced'?'ظ…طھظ‚ط¯ظ…ط©':'ط®ط¨ظٹط±ط©'} آ· ظˆطµظˆظ„ طھط¯ط±ظٹط¬ظٹ</div>
        </div>
      </div>
      <div className="mt-2 grid grid-cols-2 gap-1.5">
        <Link to="/command-center" onClick={onNavigate} className="flex items-center justify-center gap-1 rounded-[8px] bg-ink-950 px-2 py-2 text-[10px] font-black text-white hover:bg-ink-800"><Target size={12}/> ظ…ط±ظƒط² ط§ظ„ظ‚ط±ط§ط±</Link>
        <Link to="/import" onClick={onNavigate} className="flex items-center justify-center gap-1 rounded-[8px] border border-ink-200 bg-white px-2 py-2 text-[10px] font-black text-ink-700 hover:border-primary-200 hover:bg-primary-50 hover:text-primary-800"><Upload size={12}/> ط¥ط¯ط®ط§ظ„ ط§ظ„ط¨ظٹط§ظ†ط§طھ</Link>
      </div>
    </div>
  </div>
  <div className="ag-sidebar-shortcuts px-3 pb-1">
    <div className="flex items-center justify-between px-2 pb-2"><div className="section-kicker">ط£ط¯ظ„ط© ط³ط±ظٹط¹ط©</div><span className="text-[9px] font-black text-ink-500">EVIDENCE</span></div>
  ...
  <nav className="flex-1 overflow-y-auto px-3 py-3" aria-label={language==='ar'?'ط§ظ„طھظ†ظ‚ظ„ ط§ظ„ط±ط¦ظٹط³ظٹ ظ„ظ„ظ…ظ†طµط©':'Primary analytics navigation'}><div className="space-y-1">{visibleSections.map(section=>{const active=activeSection===section.id,open=expandedSection===section.id;const meta=sectionMeta[section.id]??{hint:'',tag:''};return <div key={section.id} className="ag-nav-section"><button type="button" onClick={()=>setExpandedSection(open?'':section.id)} className={'ag-section-toggle flex w-full items-center gap-2.5 rounded-[11px] px-2.5 py-2.5 text-right transition '+(active?'ag-section-toggle-active':'text-ink-600 hover:bg-ink-50 hover:text-ink-950')} aria-expanded={open}><span className={'flex h-8 w-8 shrink-0 items-center justify-center rounded-[8px] '+(active?'bg-white text-primary-700':'bg-ink-50 text-ink-400')}>{sectionIcons[section.id] ?? <Database size={17} />}</span><span className="min-w-0 flex-1"><span className="flex items-center gap-2"><span className="text-[13px] font-black">{language==='ar'?section.title:section.enTitle}</span><span className="ag-section-tag">{meta.tag}</span></span><span className="mt-0.5 block truncate text-[9px] font-medium text-ink-400">{meta.hint}</span></span>{section.id==='decision-center'&&alertCount>0&&<span className="min-w-4 rounded-full bg-danger-600 px-1 text-center text-[9px] font-black text-white">{alertCount}</span>}<span className="flex h-6 min-w-6 items-center justify-center rounded-full border border-ink-200 bg-white/70 text-[9px] font-black text-ink-400">{section.items.length}</span><ChevronDown size={14} className={'shrink-0 text-ink-300 transition-transform '+(open?'':'-rotate-90')}/></button>{open&&<div className="ag-nav-sublist mr-3 mt-0.5 space-y-0.5 border-r border-ink-200 pr-2">{section.items.map(item=>{const activeItem=resolveNavigationItem(location.pathname)?.path===item.path;return <Link key={item.path} to={item.path} onClick={onNavigate} className={'ag-nav-item nav-item '+(activeItem?'ag-nav-item-active':'ag-nav-item-inactive')}><span className="shrink-0">{item.iconNode}</span><span className="min-w-0 flex-1 truncate">{language==='ar'?item.label:item.enLabel}</span>{activeItem&&<span className="ag-nav-current"/>}</Link>})}</div>}</div>})}</div></nav>
  <div className="border-t border-ink-200 p-3"><div className="flex items-center gap-2.5"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[8px] bg-ink-100 text-[11px] font-black text-ink-700">{getDisplayName(user??null).slice(0,1)||'ظ…'}</div><div className="min-w-0 flex-1"><div className="truncate text-[12px] font-bold text-ink-800">{getDisplayName(user??null)}</div><div className="truncate text-[10px] text-ink-400" dir="ltr">{getDisplayEmail(user??null)}</div></div><button type="button" onClick={()=>void signOut()} className="rounded-[8px] p-2 text-ink-400 hover:bg-ink-100 hover:text-ink-800" aria-label={language==='ar'?'طھط³ط¬ظٹظ„ ط§ظ„ط®ط±ظˆط¬':'Sign out'}><LogOut size={15}/></button></div></div>
 </aside>
}
