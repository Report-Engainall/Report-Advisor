...
export function Sidebar({alertCount=0,onNavigate,user}:{alertCount?:number;onNavigate?:()=>void;user?:User|null}){
 ...
 const[signingOut,setSigningOut]=useState(false); const signOut=async()=>{if(signingOut)return;setSigningOut(true);try{const{error}=await supabase.auth.signOut({scope:'local'});if(error)throw error;onNavigate?.();window.location.replace('/');}catch(error){console.error('[Sidebar] Sign out failed',error);setSigningOut(false)}};
 return <aside dir={language==='ar'?'rtl':'ltr'} className={'ag-sidebar flex h-screen w-[284px] shrink-0 flex-col border-ink-200 bg-white text-ink-900 '+(language==='ar'?'border-l':'border-r')}>
  ...
  <div className="border-t border-ink-200 p-3"><div className="flex items-center gap-2.5"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[8px] bg-ink-100 text-[11px] font-black text-ink-700">{getDisplayName(user??null).slice(0,1)||'م'}</div><div className="min-w-0 flex-1"><div className="truncate text-[12px] font-bold text-ink-800">{getDisplayName(user??null)}</div><div className="truncate text-[10px] text-ink-400" dir="ltr">{getDisplayEmail(user??null)}</div></div><button type="button" onClick={()=>void signOut()} disabled={signingOut} aria-busy={signingOut} className="flex min-h-11 min-w-11 disabled:cursor-wait disabled:opacity-50 items-center justify-center rounded-[8px] p-2 text-ink-400 hover:bg-ink-100 hover:text-ink-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-400" aria-label={language==='ar'?'تسجيل الخروج':'Sign out'}><LogOut size={15}/></button></div></div>
 </aside>
}
