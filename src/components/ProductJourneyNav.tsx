...
const steps: JourneyStep[] = [
  ...
  { path: '/decision-experience', label: 'الدليل والقرار', description: 'Evidence → Recommendation → Decision', icon: FileSearch, stage: 'evidence' },
  { path: '/decision-experience', label: 'الموافقة', description: 'Accountability → Approval', icon: ShieldCheck, stage: 'approval' },
  { path: '/decision-experience', label: 'التنفيذ', description: 'Work → Next Action', icon: Target, stage: 'work' },
  { path: '/decision-experience', label: 'النتيجة والتعلّم', description: 'Expected → Actual → Learning', icon: BookOpen, stage: 'outcome' },
  { path: '/reports/executive', label: 'القصة التنفيذية', description: 'Decision Story → Executive Report', icon: Route },
];

function buildJourneyHref(path: string, search: string, stage?: string) {
  const params = new URLSearchParams(search);
  if (path === '/decision-experience') {
    if (stage) params.set('stage', stage);
    return params.toString() ? `${path}?${params.toString()}` : path;
  }
  return params.get('recommendationId') ? `${path}?recommendationId=${encodeURIComponent(params.get('recommendationId')!)}` : path;
}
    ...
    <nav aria-label="دورة قيمة Report-Advisor" className="mb-5 overflow-x-auto rounded-2xl border border-ink-200 bg-white p-2 shadow-sm">
      <div className="flex min-w-max items-stretch gap-2">
        {steps.map(({ path, label, description, icon: Icon, stage }, index) => {
          const active = activePath === path && (!stage || currentParams.get('stage') === stage) || (path === '/decision-experience' && activePath.startsWith('/decision-experience') && stage === currentParams.get('stage'));
          const href = buildJourneyHref(path, location.search, stage);
          return (
            <Link
              key={`${path}-${label}`}
              to={href}
