...
const COMMANDS: CommandItem[] = [
  ...
  { label: 'التقارير', description: 'مركز التقارير والتصدير', path: '/reports', keywords: ['reports', 'report', 'تقارير', 'snapshot', 'diff'] },
  { label: 'التحليلات', description: 'RFM وABC والأعمار والتحليلات', path: '/analytics', keywords: ['analytics', 'rfm', 'abc', 'aging', 'تحليلات'] },
  { label: 'الذكاء', description: 'التوصيات والتنبؤات والسيناريوهات', path: '/intelligence', keywords: ['ai', 'intelligence', 'forecast', 'سيناريو', 'تنبؤ', 'recommendation'] },
    ...
    >
      ...
      <div className="relative w-full max-w-2xl overflow-hidden rounded-2xl border border-ink-200 bg-white shadow-2xl" dir="rtl">
        <div className="flex items-center gap-3 border-b border-ink-100 px-4 py-3">
          <Search size={19} className="text-ink-400" aria-hidden="true" />
          <input
            ref={inputRef}
            value={query}
        ...
        <div id={LISTBOX_ID} className="max-h-[55vh] overflow-y-auto p-2" role="listbox" aria-label="نتائج لوحة الأوامر">
          {filtered.length === 0 ? (
            ...
            >
              <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${index === active ? 'bg-primary-100 text-primary-700' : 'bg-ink-100 text-ink-500'}`}>
              ...
              {index === active && <ArrowRight size={16} className="shrink-0 text-primary-500" aria-hidden="true" />}
            </button>
          ))}
        </div>
        <div className="flex flex-wrap items-center gap-3 border-t border-ink-100 bg-ink-50/70 px-4 py-2 text-[11px] text-ink-400">
          <span>↑↓ للتنقل</span><span>Home/End للبداية والنهاية</span><span>Enter للفتح</span><span>Esc للإغلاق</span>
        </div>
      </div>
    </div>
  );
