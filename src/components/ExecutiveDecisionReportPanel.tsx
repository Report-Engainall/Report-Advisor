...
function evidenceRefs(row: ExecutiveDecisionReport['rows'][number]): string[] {
  ...
  add(row.decision.evidence);
  add(row.recommendation?.evidence_snapshot_id);
  add(row.recommendation?.metric_versions);
  row.work_items.forEach((item) => add(item.evidence_refs));
  add(row.decision_outcome?.evidence_snapshot_id);
  add(row.recommendation_outcome?.evidence);
...
export function ExecutiveDecisionReportPanel() {
  ...
  return <section className="rounded-2xl border border-ink-200 bg-white p-5 print:border-0 print:p-0">
    ...
    <div className="mt-3 flex flex-wrap gap-2 text-xs" aria-label="حالة التقرير"><span className="rounded-lg bg-ink-50 px-2 py-1">{freshness.status === 'LIVE_READ_MODEL' ? 'قراءة من read model' : 'لا توجد قرارات محفوظة'}</span><span className="rounded-lg bg-ink-50 px-2 py-1">أحدث نشاط: {new Date(freshness.asOf).toLocaleString('ar')}</span></div>
    <div className="mt-4 space-y-3">
      ...
      {report.rows.map((row) => { const refs = evidenceRefs(row); return <article key={row.decision.id} className="rounded-xl border border-ink-100 p-4 print:break-inside-avoid">
        ...
        <div className="mt-3 rounded-lg border border-ink-100 p-3 text-xs"><p className="font-semibold">Evidence / lineage references</p>{refs.length ? <div className="mt-2 flex flex-wrap gap-2">{refs.map((ref) => <code key={ref} className="rounded bg-ink-50 px-2 py-1 break-all">{ref}</code>)}</div> : <p className="mt-1 text-ink-500">لا توجد مراجع evidence محفوظة في هذا السجل.</p>}</div>
      </article>; })}
    </div>
  </section>;
}
