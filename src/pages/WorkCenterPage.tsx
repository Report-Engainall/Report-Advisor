    ...
    <div dir="rtl" className="space-y-6 pb-8">
      <section className="rounded-3xl border border-ink-200 bg-white p-5 shadow-sm sm:p-7">
        <div className="max-w-3xl">
          <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-primary-50 px-3 py-1 text-xs font-bold text-primary-800"><FileUp size={14} /> مركز العمل</div>
          <h1 className="text-2xl font-black tracking-tight text-ink-950 sm:text-3xl">ابدأ من ملف أعمالك</h1>
          <p className="mt-3 text-sm leading-7 text-ink-600 sm:text-base">Report-Advisor لا يبدأ برقم KPI. يبدأ بالمصدر، ثم يحول الملف إلى بيانات موثقة، وبعدها إلى معرفة وقرار قابل للتتبع.</p>
        </div>
      ...
      <section className="rounded-3xl border border-ink-200 bg-white p-4 shadow-sm sm:p-6">
        <div className="mb-4 flex items-center justify-between gap-3"><div><h2 className="text-lg font-black text-ink-900">رحلة الملف الحقيقية</h2><p className="mt-1 text-xs text-ink-500">هذه رحلة المعالجة الفعلية وليست حالات عرض تجميلية.</p></div><span className="hidden rounded-full bg-ink-100 px-3 py-1 text-[11px] font-bold text-ink-600 sm:inline-flex">Fail-closed</span></div>
        <div className="grid gap-2 md:grid-cols-3 xl:grid-cols-5">{journey.map(([number, title, description], index) => <div key={title} className="relative rounded-2xl border border-ink-100 bg-ink-50 p-3"><div className="flex items-center gap-2"><span className="text-[10px] font-black text-primary-600">{number}</span><b className="text-xs text-ink-900">{title}</b>{index < journey.length - 1 && <ArrowLeft className="mr-auto hidden text-ink-300 xl:block" size={13}/>}</div><p className="mt-2 text-[11px] leading-5 text-ink-500">{description}</p></div>)}</div>
      </section>
      <section className="rounded-3xl border border-ink-200 bg-white p-4 shadow-sm sm:p-6">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">{outcomes.map(([title, description]) => <div key={title} className="rounded-2xl border border-ink-100 p-3"><b className="block text-sm text-ink-900">{title}</b><span className="mt-1 block text-[11px] leading-5 text-ink-500">{description}</span></div>)}</div>
      </section>
