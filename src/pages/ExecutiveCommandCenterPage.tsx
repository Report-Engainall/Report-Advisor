...
export function ExecutiveCommandCenterPage() {
  ...
  const cards = useMemo<HealthCard[]>(() => { if (!complete) return []; const rs = receivableStatus(complete); const ms = marginStatus(complete.grossMargin); const cs = collectionStatus(complete.collectionRate); return [
    ...
    { label: 'العملاء', value: number(complete.activeCustomers), status: complete.activeCustomers > 0 ? 'good' : 'watch', icon: Activity, context: `إجمالي العملاء: ${number(complete.totalCustomers)}` },
    { label: 'التغطية التشغيلية', value: number(complete.invoiceCount), status: complete.invoiceCount > 0 ? 'good' : 'watch', icon: Database, context: `متوسط الفاتورة: ${number(complete.avgInvoiceValue)}` },
