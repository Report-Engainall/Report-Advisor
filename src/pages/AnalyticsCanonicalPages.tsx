...
export function AgingAnalysisPage() {
  ...
  useEffect(() => { void load(); }, []);
  if (loading) return <LoadingState />; if (error) return <ErrorState message={error} onRetry={load} />;
  const isCalculated = snapshot?.status === 'CALCULATED';
  const buckets = isCalculated && snapshot ? Object.entries(snapshot.buckets).map(([name, amount]) => ({ name, amount })) : [];
  return <div className="space-y-6 animate-fade-in" dir="rtl"><PageHeader title="تحليل أعمار الذمم" subtitle="الإجماليات مستقلة عن pagination وتستخدم نفس مصدر حقيقة الذمم" />{!isCalculated && <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">الحقيقة المالية غير مكتملة. غير المؤرخ: {formatNumber(snapshot?.undatedRows ?? 0)}، الناقص: {formatNumber(snapshot?.incompleteRows ?? 0)}.</div>}<Card><CardHeader title="توزيع الأعمار"/><CardBody>{isCalculated ? <SimpleBarChart data={buckets} dataKey="amount" nameKey="name"/> : <div className="py-12 text-center text-sm text-ink-500">لا يمكن عرض توزيع مالي موثوق حتى تكتمل البيانات.</div>}</CardBody></Card><Card><CardHeader title="الإجماليات"/><DataTable columns={[{key:'name',label:'الفئة (يوم)'},{key:'amount',label:'المبلغ',align:'right',render:(r:any)=>isCalculated ? formatCurrency(r.amount) : '—'}]} data={buckets}/></Card></div>;
}
