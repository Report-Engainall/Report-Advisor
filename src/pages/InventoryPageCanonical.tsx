...
export function InventoryPageCanonical() {
  ...
  return <div className="space-y-6 animate-fade-in">
    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <Card><CardBody><div className="text-xs text-ink-500 mb-1">مخزون منخفض</div><div className="text-xl font-bold text-warning-600">{formatNumber(snapshot.lowStock)}</div></CardBody></Card>
      <Card><CardBody><div className="text-xs text-ink-500 mb-1">نفد المخزون</div><div className="text-xl font-bold text-danger-600">{formatNumber(snapshot.outOfStock)}</div></CardBody></Card>
    </div>
    {snapshot.unknownRows > 0 && <div role="alert" className="rounded-xl border border-warning-200 bg-warning-50 px-4 py-3 text-sm text-warning-800">لا يمكن اعتبار قيمة المخزون مكتملة: {formatNumber(snapshot.unknownRows)} صفوف تفتقد بيانات كافية. لم يتم تحويلها إلى صفر.</div>}
    <Card>
      ...
      <DataTable columns={[
        ...
        { key: 'value', label: 'القيمة', align: 'right', render: (r: InventoryReportRow) => formatCurrency(r.value) },
