...
function errorMessage(error: unknown): string {
  return error instanceof Error ? error.message : 'تعذر تحميل التقرير';
}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {reportCards.map((r) => (
          <Link key={r.path} to={r.path}>
            <Card hover className="h-full"><CardBody><div className="flex items-start gap-3">
              <div className={`w-11 h-11 rounded-xl bg-${r.color}-50 text-${r.color}-600 flex items-center justify-center flex-shrink-0`}><r.icon size={20} /></div>
              <div><h3 className="font-semibold text-ink-800 text-sm">{r.title}</h3><p className="text-xs text-ink-500 mt-1">{r.desc}</p></div>
            </div></CardBody></Card>
          </Link>
        ))}
      </div>
...
export function SalesReportPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const [k, t, tc, tp, cat, inv] = await Promise.all([
        fetchDashboardKPIs(), fetchMonthlyTrend(6), fetchTopCustomers(10), fetchTopProducts(10), fetchCategoryBreakdown(), fetchSalesInvoices(0, 20),
      ]);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title="تقرير المبيعات" subtitle="تحليل شامل لأداء المبيعات" actions={<button onClick={exportSales} className="btn-secondary text-xs">تصدير XLSX</button>} />
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card><CardBody><div className="text-xs text-ink-500 mb-1">إجمالي المبيعات</div><div className="text-xl font-bold text-ink-900">{formatCurrency(kpis?.totalSales)}</div></CardBody></Card>
        <Card><CardBody><div className="text-xs text-ink-500 mb-1">عدد الفواتير</div><div className="text-xl font-bold text-ink-900">{formatNumber(kpis?.invoiceCount)}</div></CardBody></Card>
        <Card><CardBody><div className="text-xs text-ink-500 mb-1">متوسط قيمة الفاتورة</div><div className="text-xl font-bold text-ink-900">{formatCurrency(kpis?.avgInvoiceValue)}</div></CardBody></Card>
        <Card><CardBody><div className="text-xs text-ink-500 mb-1">معدل التحصيل</div><div className="text-xl font-bold text-ink-900">{kpis?.collectionRate == null ? '—' : `${kpis.collectionRate.toFixed(1)}%`}</div></CardBody></Card>
      </div>
      ...
      <Card><CardHeader title="آخر الفواتير" subtitle="20 فاتورة الأخيرة" /><DataTable columns={[
        ...
        { key: 'status', label: 'الحالة', align: 'center', render: (r: SalesInvoice) => {
          const map: Record<string, { variant: 'success' | 'primary' | 'neutral'; label: string }> = { paid: { variant: 'success', label: 'مدفوعة' }, confirmed: { variant: 'primary', label: 'مؤكدة' }, draft: { variant: 'neutral', label: 'مسودة' } };
          const status = map[r.status] ?? { variant: 'neutral', label: r.status };
...
export function PurchasesReportPage() {
  ...
  const [loading, setLoading] = useState(true);
  useEffect(() => { fetchPurchaseInvoices(0, 20).then(res => { setPurchases(res.data); setLoading(false); }).catch(() => setLoading(false)); }, []);
  if (loading) return <LoadingState />;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card><CardBody><div className="text-xs text-ink-500 mb-1">الموردين النشطين</div><div className="text-xl font-bold text-ink-900">{formatNumber(new Set(purchases.map(p => p.supplier_id)).size)}</div></CardBody></Card>
        <Card><CardBody><div className="text-xs text-ink-500 mb-1">متوسط الفاتورة</div><div className="text-xl font-bold text-ink-900">{formatCurrency(averagePurchase)}</div></CardBody></Card>
      </div>
      <Card><CardHeader title="آخر فواتير المشتريات" /><DataTable columns={[
...
export function InventoryReportPage() {
  ...
  const [loading, setLoading] = useState(true);
  useEffect(() => { fetchInventoryBalances().then(data => { setBalances(data as InventoryBalanceRow[]); setLoading(false); }).catch(() => setLoading(false)); }, []);
  if (loading) return <LoadingState />;
  const totalValue = balances.reduce((s, b) => s + (b.quantity == null || b.unit_cost == null ? 0 : b.quantity * b.unit_cost), 0);
  const lowStock = balances.filter(b => b.quantity != null && b.product?.reorder_point != null && b.quantity <= b.product.reorder_point);
  const outOfStock = balances.filter(b => b.quantity != null && b.quantity <= 0);
  const exportInventory = () => downloadReportArtifact('inventory-report', 'تقرير المخزون', ['المنتج', 'المستودع', 'الكمية', 'التكلفة', 'القيمة'], balances.map((balance) => ({
    'المنتج': balance.product?.name ?? null, 'المستودع': balance.warehouse?.name ?? null, 'الكمية': balance.quantity, 'التكلفة': balance.unit_cost, 'القيمة': balance.quantity == null || balance.unit_cost == null ? null : balance.quantity * balance.unit_cost,
  })));
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title="تقرير المخزون" subtitle="حالة المخزون والتقييم" actions={<button onClick={exportInventory} className="btn-secondary text-xs">تصدير XLSX</button>} />
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <Card><CardHeader title="تفاصيل المخزون" /><DataTable columns={[
        { key: 'product', label: 'المنتج', render: (r: InventoryBalanceRow) => r.product?.name || '—' },
        { key: 'warehouse', label: 'المستودع', render: (r: InventoryBalanceRow) => r.warehouse?.name || '—' },
        { key: 'quantity', label: 'الكمية', align: 'right', render: (r: InventoryBalanceRow) => formatNumber(r.quantity) },
        { key: 'unit_cost', label: 'التكلفة', align: 'right', render: (r: InventoryBalanceRow) => formatCurrency(r.unit_cost) },
        { key: 'value', label: 'القيمة', align: 'right', render: (r: InventoryBalanceRow) => formatCurrency(r.quantity == null || r.unit_cost == null ? null : r.quantity * r.unit_cost) },
        { key: 'status', label: 'الحالة', align: 'center', render: (r: InventoryBalanceRow) => {
          if (r.quantity == null) return <Badge variant="neutral">بيانات ناقصة</Badge>;
          if (r.quantity <= 0) return <Badge variant="danger">نفد</Badge>;
          if (r.product?.reorder_point != null && r.quantity <= r.product.reorder_point) return <Badge variant="warning">منخفض</Badge>;
...
export function ProfitabilityReportPage() {
  ...
  const [categories, setCategories] = useState<CategoryBreakdown[]>([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => { Promise.all([fetchDashboardKPIs(), fetchCategoryBreakdown()]).then(([k, c]) => { setKpis(k); setCategories(c); setLoading(false); }).catch(() => setLoading(false)); }, []);
  if (loading) return <LoadingState />;
  const exportProfitability = () => downloadReportArtifact('profitability-report', 'تقرير الأرباح والربحية', ['الفئة', 'المبيعات', 'الربح', 'الهامش', 'الكمية'], categories.map((category) => ({
    'الفئة': category.name, 'المبيعات': category.sales, 'الربح': category.profit, 'الهامش': category.sales > 0 ? `${((category.profit / category.sales) * 100).toFixed(1)}%` : null, 'الكمية': category.quantity,
  })));
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title="تقرير الأرباح والربحية" subtitle="تحليل الربحية حسب الفئة والمنتج" actions={<button onClick={exportProfitability} className="btn-secondary text-xs">تصدير XLSX</button>} />
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card><CardBody><div className="text-xs text-ink-500 mb-1">إجمالي المبيعات</div><div className="text-xl font-bold text-ink-900">{formatCurrency(kpis?.totalSales)}</div></CardBody></Card>
        <Card><CardBody><div className="text-xs text-ink-500 mb-1">إجمالي التكلفة</div><div className="text-xl font-bold text-ink-900">{formatCurrency(kpis?.totalCost)}</div></CardBody></Card>
        <Card><CardBody><div className="text-xs text-ink-500 mb-1">إجمالي الربح</div><div className="text-xl font-bold text-success-600">{formatCurrency(kpis?.grossProfit)}</div></CardBody></Card>
        <Card><CardBody><div className="text-xs text-ink-500 mb-1">هامش الربح</div><div className="text-xl font-bold text-success-600">{kpis?.grossMargin == null ? '—' : `${kpis.grossMargin.toFixed(1)}%`}</div></CardBody></Card>
      </div>
      <Card><CardHeader title="الربحية حسب الفئة" /><DataTable columns={[
