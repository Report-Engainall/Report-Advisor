...
import { Users, Package, Calendar, WalletCards } from 'lucide-react';
...
export function AnalyticsCenterPage() { return <div className="ag-analytics-surface space-y-6 animate-fade-in"><PageHeader title="مركز التحليلات" subtitle="تحليلات متقدمة لاكتشاف الأنماط والاتجاهات" /><div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">{analyticsCards.map((r) => <Link key={r.path} to={r.path}><Card hover className="h-full"><CardBody><div className="flex items-start gap-3"><div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${analyticsIconClasses[r.color] ?? analyticsIconClasses.primary}`}><r.icon size={20} /></div><div><h3 className="font-semibold text-ink-800 text-sm">{r.title}</h3><p className="text-xs text-ink-500 mt-1">{r.desc}</p></div></div></CardBody></Card></Link>)}</div></div>; }
...
export function AgingAnalysisPage(){
  ...
  useEffect(()=>{void load();},[load]); if(loading)return <LoadingState/>; if(error)return <ErrorState message={error} onRetry={load}/>;
  return <div className="ag-analytics-surface space-y-6 animate-fade-in"><PageHeader title="تحليل أعمار الذمم" subtitle="توزيع الذمم من المصدر المعتمد، مع حفظ حالة البيانات الناقصة"/>{status==='INSUFFICIENT_DATA'&&<div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">بعض السجلات لا تكفي لحساب العمر بثقة: {unknownRows==null?'غير متاح':formatNumber(unknownRows)}. تظهر كـ«دون تاريخ» أو «حالة غير مكتملة» بدل تحويلها إلى صفر.</div>}<Card><CardHeader title="توزيع الأعمار"/><CardBody>{buckets.length?<SimpleBarChart data={buckets} dataKey="amount" nameKey="name"/>:<div className="text-sm text-ink-500 py-16 text-center">لا توجد بيانات</div>}</CardBody></Card><Card><CardHeader title="التفاصيل"/><DataTable columns={[{key:'name',label:'الفئة (يوم)'},{key:'amount',label:'المبلغ',align:'right',render:(r:AgingSnapshotRow)=>formatCurrency(r.amount)},{key:'count',label:'عدد الفواتير',align:'center',render:(r:AgingSnapshotRow)=>formatNumber(r.count)}]} data={buckets}/></Card></div>;
}
