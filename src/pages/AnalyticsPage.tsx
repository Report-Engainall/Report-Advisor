...
export function ABCAnalysisPage(){
  ...
  const load=useCallback(async()=>{try{setLoading(true);setError(null);const snapshot=await fetchABCSnapshot(500);setData(snapshot.rows);setStatus(snapshot.status);setUnknownRows(snapshot.unknownRows);setAsOf(snapshot.asOf);}catch(e:unknown){setError(e instanceof Error?e.message:'فشل تحميل تحليل ABC');}finally{setLoading(false);}},[]);
  useEffect(()=>{void load();},[load]); if(loading)return <LoadingState/>; if(error)return <ErrorState message={error} onRetry={load}/>;
  const counts=new Map<string,number>(); data.forEach(r=>{if(r.class)counts.set(r.class,(counts.get(r.class)||0)+1);});
  return <div className="space-y-6 animate-fade-in"><PageHeader title="تحليل ABC" subtitle="تصنيف المنتجات حسب مساهمة الإيرادات من المصدر المعتمد"/><TruthContextStrip periodLabel="لقطة تحليلية من المصدر" status={status} asOf={asOf}/>{status==='INSUFFICIENT_DATA'&&<div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">لا يمكن إصدار تصنيف ABC موثوق من البيانات الحالية. السجلات الناقصة: {unknownRows==null?'غير متاح':formatNumber(unknownRows)}.</div>}<div className="grid grid-cols-3 gap-4">{['A','B','C'].map(c=><Card key={c}><CardBody><div className="text-xs text-ink-500 mb-1">الفئة {c}</div><div className="text-xl font-bold text-ink-900">{counts.get(c)??'—'} منتج</div><div className="text-xs text-ink-400 mt-1">حدود الفئات يحسبها المصدر التحليلي نفسه</div></CardBody></Card>)}</div><Card><CardHeader title="تصنيف المنتجات"/><DataTable columns={[{key:'product_name',label:'المنتج'},{key:'revenue',label:'الإيرادات',align:'right',render:(r:ABCSnapshotRow)=>formatCurrency(r.revenue)},{key:'cumulative_pct',label:'النسبة التراكمية',align:'right',render:(r:ABCSnapshotRow)=>r.cumulative_pct==null?'غير متاح':`${r.cumulative_pct.toFixed(1)}%`},{key:'class',label:'الفئة',align:'center',render:(r:ABCSnapshotRow)=><Badge variant={r.class==='A'?'success':r.class==='B'?'primary':'neutral'}>{r.class??'غير معروف'}</Badge>}] } data={data.slice(0,30)}/></Card></div>;
}
