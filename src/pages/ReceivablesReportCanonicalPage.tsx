...
export function ReceivablesReportCanonicalPage() {
  ...
  const exportRows = async () => {
    try {
      ...
      setError(cause instanceof Error ? cause.message : 'تعذر تجهيز ملف التصدير');
    } finally {
      setExporting(false);
    }
  };
  return <div dir="rtl" className="space-y-6 animate-fade-in">
    ...
    <PageHeader title="تقرير الذمم والتحصيل" subtitle="الإجماليات والصفحات مشتقة من نفس الحقيقة المعتمدة على الخادم." actions={<button type="button" onClick={() => void exportRows()} disabled={exporting} className="btn-secondary text-xs disabled:opacity-50">{exporting ? 'جارٍ تجهيز التصدير...' : 'تصدير XLSX'}</button>} />
    ...
    <div className="flex items-center justify-between"><span className="text-xs text-ink-500">عرض {snapshot.rows.length} من {formatNumber(snapshot.total_rows)}</span><div className="flex gap-2"><button type="button" disabled={page===0} onClick={() => setPage(p => Math.max(0,p-1))} className="px-3 py-1.5 rounded-lg border border-ink-200 text-xs disabled:opacity-40">السابق</button><button type="button" disabled={page+1>=totalPages} onClick={() => setPage(p => p+1)} className="px-3 py-1.5 rounded-lg border border-ink-200 text-xs disabled:opacity-40">التالي</button></div></div>
  </div>;
}
