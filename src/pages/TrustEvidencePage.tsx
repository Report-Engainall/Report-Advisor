...
export function TrustEvidencePage() {
  ...
  return <div dir="rtl" className="space-y-6 animate-fade-in">
    ...
    <section className="grid gap-4 lg:grid-cols-[1.1fr_.9fr]">
      <Card><CardHeader title="حالة جودة البيانات الحالية" subtitle={snapshot?.status ?? 'غير متاح'} /><CardBody className="space-y-3">{snapshot?.entities?.slice(0,6).map(entity => <div key={entity.name} className="flex items-center justify-between rounded-xl border border-ink-100 bg-ink-50/40 px-3 py-3"><span className="text-xs font-bold text-ink-800">{entity.name}</span><span className="text-xs font-black text-ink-500">{entity.issues} مشكلة</span></div>)}</CardBody></Card>
