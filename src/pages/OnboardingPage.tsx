    ...
    <div dir="rtl" className="space-y-6">
      ...
      <section className="grid gap-4 lg:grid-cols-[1.35fr_.65fr]">
        <Card>
          ...
          <CardBody>
            {snapshot ? <>
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-ink-100 bg-ink-50/60 p-4">
                <div><div className="text-xs text-ink-400">تغطية المؤشرات الأساسية</div><div className="mt-1 text-3xl font-black text-ink-950">{snapshot.coverage}%</div></div>
                <div className="text-left"><div className="text-[10px] font-bold text-ink-400">حالة المصدر</div><div className="mt-1 text-xs font-black text-ink-800">{snapshot.kpis.status === 'CONFIRMED' ? 'مصدر مؤكد' : snapshot.kpis.status === 'CALCULATED' ? 'محسوب من المصدر' : 'بيانات غير كافية'}</div><div className="mt-1 text-[10px] text-ink-400">حتى {snapshot.asOf ?? 'غير متاح'}</div></div>
              </div>
