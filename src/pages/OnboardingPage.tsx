...
type OnboardingState = { userReady: boolean; companyName: string | null; role: string | null; importsCount: number | null; dataQualityIssues: number | null; };
...
function stateFor(stepId: string, data: OnboardingState): StepState {
  ...
  if (stepId === 'import') return data.importsCount == null ? 'UNKNOWN' : data.importsCount > 0 ? 'READY' : 'ACTION_REQUIRED';
  if (stepId === 'quality') return data.dataQualityIssues == null ? 'UNKNOWN' : data.dataQualityIssues === 0 ? 'READY' : 'ACTION_REQUIRED';
  return 'UNKNOWN';
}

export function OnboardingPage() {
  const [data, setData] = useState<OnboardingState>({ userReady: false, companyName: null, role: null, importsCount: null, dataQualityIssues: null });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [{ data: company }, { data: membership }, { count: importsCount, error: importsError }] = await Promise.all([
        ...
        supabase.from('import_jobs').select('id', { count: 'exact', head: true }).eq('company_id', companyId),
      ]);
      ...
      setData({ userReady: true, companyName: company?.name ?? null, role: membership?.role ?? null, importsCount: importsCount ?? 0, dataQualityIssues });
    } catch (cause) {
  ...
  return (
    <div dir="rtl" className="space-y-6">
      <PageHeader title="بدء الاستخدام التجاري" subtitle="مسار onboarding مبني على حالة الحساب والـtenant والبيانات الحقيقية الحالية." />
