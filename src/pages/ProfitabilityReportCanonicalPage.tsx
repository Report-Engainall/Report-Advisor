...
export function ProfitabilityReportCanonicalPage() {
  ...
  return <div className="space-y-6 animate-fade-in">
    ...
    <Card><CardBody><div className="flex flex-wrap items-center gap-3"><span className={`rounded-full px-3 py-1 text-xs font-medium ${calculated ? 'bg-success-50 text-success-700' : 'bg-warning-50 text-warning-700'}`}>{calculated ? 'محسوب من المصدر' : 'بيانات غير كافية'}</span><span className="text-xs text-ink-500">العملة: {snapshot.currency || 'غير محددة'}</span><span className="text-xs text-ink-500">حالة العملة: {snapshot.currency_status === 'CONSISTENT' ? 'متسقة' : 'غير كافية'}</span><span className="text-xs text-ink-500">حتى {snapshot.as_of}</span></div></CardBody></Card>
