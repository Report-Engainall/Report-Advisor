...
export function CustomersPage() {
  ...
  const load = useCallback(async () => {
    try { setLoading(true); setError(null); const data = await fetchCustomers(); setCustomers(data); }
    catch (e: unknown) { setError(e instanceof Error ? e.message : 'تعذر تحميل العملاء'); }
...
export function InventoryPage() {
  ...
  const [filter, setFilter] = useState<'all' | 'low' | 'out'>('all'); const [page, setPage] = useState(0); const pageSize = 25;
  ...
  const filteredRows = snapshot.filteredRows; const totalPages = Math.max(1, Math.ceil(filteredRows / pageSize));
  return <div className="space-y-6 animate-fade-in"><PageHeader title="المخزون" subtitle="حالة المخزون من مصدر المخزون المعتمد، مع فصل إجماليات الأعمال عن صفحات العرض." />
    ...
    <div className="flex flex-wrap gap-2">{[{ v: 'all' as const, l: 'الكل' },{ v: 'low' as const, l: 'منخفض' },{ v: 'out' as const, l: 'نفد' }].map(f => <button key={f.v} onClick={() => setFilter(f.v)} className={`px-3 py-1.5 rounded-lg text-xs font-medium ${filter === f.v ? 'bg-primary-600 text-white' : 'bg-white text-ink-600 border border-ink-200'}`}>{f.l}</button>)}</div>
    {error&&<div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</div>}
    <Card><DataTable columns={[{ key: 'product', label: 'المنتج', render: (r: InventoryReportRow) => r.product?.name || '—' },{ key: 'sku', label: 'SKU', render: (r: InventoryReportRow) => <span className="font-mono text-xs text-ink-500">{r.product?.sku || '—'}</span> },{ key: 'warehouse', label: 'المستودع', render: (r: InventoryReportRow) => r.warehouse?.name || '—' },{ key: 'quantity', label: 'الكمية', align: 'right', render: (r: InventoryReportRow) => r.quantity == null ? 'غير متاح' : formatNumber(r.quantity) },{ key: 'unit_cost', label: 'التكلفة', align: 'right', render: (r: InventoryReportRow) => r.unit_cost == null ? 'غير متاح' : formatCurrency(r.unit_cost) },{ key: 'value', label: 'القيمة', align: 'right', render: (r: InventoryReportRow) => r.value == null ? 'غير متاح' : formatCurrency(r.value) },{ key: 'status', label: 'الحالة', align: 'center', render: (r: InventoryReportRow) => { if (r.quantity == null) return <Badge variant="neutral">غير معروف</Badge>; if (r.quantity <= 0) return <Badge variant="danger">نفد</Badge>; if (r.product?.reorder_point != null && r.quantity <= r.product.reorder_point) return <Badge variant="warning">منخفض</Badge>; return <Badge variant="success">متاح</Badge>; }}]} data={snapshot.rows} emptyMessage="لا توجد بيانات مخزون" /></Card>
    <div className="flex items-center justify-between gap-3"><span className="text-xs text-ink-500">عرض {snapshot.rows.length} من {formatNumber(filteredRows)} نتيجة مطابقة للفلتر</span><div className="flex items-center gap-2"><button disabled={page===0} onClick={()=>setPage(p=>Math.max(0,p-1))} className="px-3 py-1.5 rounded-lg border border-ink-200 text-xs disabled:opacity-40">السابق</button><span className="text-xs text-ink-600">صفحة {page+1} / {totalPages}</span><button disabled={page+1>=totalPages} onClick={()=>setPage(p=>p+1)} className="px-3 py-1.5 rounded-lg border border-ink-200 text-xs disabled:opacity-40">التالي</button></div></div>
  </div>;
}
