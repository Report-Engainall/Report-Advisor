      ...
      <div data-testid="workspace-editor">
        ...
        <CardBody>
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
            {PRESET_OPTIONS.map(option => <button key={option.id} type="button" onClick={() => setPreset(option.id)} className={'rounded-2xl border p-4 text-right transition ' + (preferences.preset === option.id ? 'border-primary-500 bg-primary-50' : 'border-ink-100 bg-white hover:border-primary-200 hover:bg-primary-50/40')}><div className="flex items-center justify-between gap-2"><span className="font-bold text-ink-900">{option.label}</span>{preferences.preset === option.id && <Check size={17} className="text-primary-600" />}</div><div className="mt-1.5 text-[11px] leading-5 text-ink-500">{option.hint}</div></button>)}
          </div>
          <div className="mt-5 flex items-center gap-2"><SlidersHorizontal size={17} className="text-primary-600" /><div className="text-sm font-bold text-ink-900">كثافة مساحة العمل</div></div>
          <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-3">
            {([['essential', 'أساسية', 'المسارات اليومية والصفحات الأساسية.'], ['advanced', 'متقدمة', 'تضيف التحليلات والربحية والطلب والقرار.'], ['expert', 'خبيرة', 'تُظهر كامل المساحة المتخصصة.']] as Array<[WorkspaceMode, string, string]>).map(([mode, label, hint]) => <button key={mode} type="button" onClick={() => setMode(mode)} className={'rounded-2xl border p-4 text-right ' + (preferences.mode === mode ? 'border-primary-500 bg-primary-50' : 'border-ink-100 bg-white hover:bg-ink-50')}><div className="flex items-center justify-between gap-2"><span className="font-bold text-ink-900">{label}</span>{preferences.mode === mode && <Check size={17} className="text-primary-600" />}</div><div className="mt-1.5 text-[11px] leading-5 text-ink-500">{hint}</div></button>)}
          </div>
          <div className="mt-5 grid gap-4 xl:grid-cols-2">
            <div className="rounded-2xl border border-ink-100 bg-white p-4"><div className="text-sm font-bold text-ink-900">الصفحة الافتراضية بعد تسجيل الدخول</div><select value={preferences.defaultLandingPath} onChange={event => commit({ defaultLandingPath: event.target.value })} className="mt-3 w-full rounded-xl border border-ink-200 bg-white px-3 py-2.5 text-sm text-ink-800">{WORKSPACE_ROUTE_OPTIONS.map(option => <option key={option.path} value={option.path}>{option.label}</option>)}</select></div>
