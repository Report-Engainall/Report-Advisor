      ...
      <Card>
        ...
        <CardBody>
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
            <button type="button" aria-pressed={workspaceMode === 'essential'} onClick={() => setMode('essential')} className={workspaceMode === 'essential' ? 'rounded-2xl border border-primary-500 bg-primary-50 p-4 text-right' : 'rounded-2xl border border-ink-100 bg-white p-4 text-right hover:bg-ink-50'}><div className="flex items-center justify-between gap-2"><span className="font-bold text-ink-900">أساسية</span>{workspaceMode === 'essential' && <Check size={18} className="text-primary-600" />}</div><div className="mt-2 text-xs leading-5 text-ink-500">المهام اليومية: القيادة، الاستيراد، التقارير، التحصيل، المخزون، العملاء والقرارات.</div></button>
            <button type="button" aria-pressed={workspaceMode === 'advanced'} onClick={() => setMode('advanced')} className={workspaceMode === 'advanced' ? 'rounded-2xl border border-primary-500 bg-primary-50 p-4 text-right' : 'rounded-2xl border border-ink-100 bg-white p-4 text-right hover:bg-ink-50'}><div className="flex items-center justify-between gap-2"><span className="font-bold text-ink-900">متقدمة</span>{workspaceMode === 'advanced' && <Check size={18} className="text-primary-600" />}</div><div className="mt-2 text-xs leading-5 text-ink-500">تضيف التحليلات المتخصصة، الطلب، الربحية، المقاييس والسيناريوهات.</div></button>
            <button type="button" aria-pressed={workspaceMode === 'expert'} onClick={() => setMode('expert')} className={workspaceMode === 'expert' ? 'rounded-2xl border border-primary-500 bg-primary-50 p-4 text-right' : 'rounded-2xl border border-ink-100 bg-white p-4 text-right hover:bg-ink-50'}><div className="flex items-center justify-between gap-2"><span className="font-bold text-ink-900">خبيرة</span>{workspaceMode === 'expert' && <Check size={18} className="text-primary-600" />}</div><div className="mt-2 text-xs leading-5 text-ink-500">تُظهر كامل المساحة بما فيها العرض التجاري والمساحات المتخصصة.</div></button>
          </div>
        </CardBody>
      </Card>
      <Card>
