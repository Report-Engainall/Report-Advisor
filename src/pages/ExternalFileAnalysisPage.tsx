...
function downloadCsv(dataset: Dataset) {
  ...
  const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `${dataset.name.replace(/[^\p{L}\p{N}_-]+/gu, '_')}.csv`; a.click(); URL.revokeObjectURL(url);
}
...
export function ExternalFileAnalysisPage() {
  ...
  async function analyze(selected: File) {
    ...
    try {
      ...
      const buffer = await selected.arrayBuffer(); const scan = securityScan(selected, buffer); if (!scan.passed) throw new Error(scan.issues.join(' — '));
      const detection = detectFormat(selected, buffer); if (detection.format === 'unknown') throw new Error('تعذر تحديد صيغة الملف');
      const hash = await computeSHA256(buffer); const parsed = await parseFile(buffer, selected.name, detection.format); if (!parsed.length) throw new Error('لم يتم العثور على بيانات قابلة للتحليل داخل الملف');
      setFile({ name:selected.name, size:selected.size, format:detection.format, hash }); setDatasets(parsed);
    } catch (e) { setError(e instanceof Error ? e.message : 'فشل تحليل الملف'); } finally { setLoading(false); }
  }
  ...
  const summary = useMemo(() => dataset ? {
    ...
    numeric: dataset.columns.filter(c => ['integer','decimal','currency','percentage'].includes(c.dataType)).length,
    issues: dataset.columns.reduce((n,c) => n + c.qualityIssues.length, 0),
  } : null, [dataset]);
  return <div className="space-y-6" dir="rtl">
    <PageHeader title="مختبر الملفات والبيانات" subtitle="حوّل أي ملف خارجي إلى مصدر قابل للفهم والتحليل دون إسقاط الحقول أو إجبار الملف على نموذج أعمال مسبق" />
    <Card><CardBody>
      <div className="grid gap-5 lg:grid-cols-[1fr_auto] items-center">
        <div><div className="flex items-center gap-2"><Sparkles size={18}/><h2 className="font-semibold">Universal File Intelligence</h2></div><p className="mt-2 text-sm leading-6 text-ink-500">فحص أمني → كشف الصيغة → استخراج → اكتشاف الأوراق والجداول → profiling → mapping → جودة → جاهزية للتحليل. المسار تحليلي ولا يكتب قاعدة البيانات.</p><div className="mt-3 flex flex-wrap gap-2"><Badge variant="neutral">كل الأعمدة</Badge><Badge variant="neutral">أنواع البيانات</Badge><Badge variant="neutral">Mapping Evidence</Badge><Badge variant="neutral">Quality Signals</Badge><Badge variant="neutral">OCR عربي + English</Badge><Badge variant="neutral">SHA-256</Badge></div></div>
    ...
    {dataset && <>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[['الصفوف',dataset.rowCount],['الأعمدة',dataset.columnCount],['المعيّنة',summary?.mapped??0],['غير المعيّنة',summary?.unmapped??0],['تحتاج مراجعة',summary?.review??0],['مشاكل الجودة',summary?.issues??0]].map(([label,value])=><Card key={String(label)}><CardBody><div className="text-xs text-ink-400">{label}</div><div className="mt-1 text-xl font-bold">{Number(value).toLocaleString()}</div></CardBody></Card>)}
      </div>
      <Card><CardHeader title="ذكاء المخطط" subtitle="كل حقل يحتفظ بهويته الأصلية ويُعامل كمرشح مستقل للمطابقة والتحليل" action={<button type="button" onClick={()=>downloadCsv(dataset)} className="btn-secondary text-xs inline-flex items-center gap-1"><Download size={14}/> تصدير البيانات المحللة</button>}/><CardBody><div className="overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="border-b border-ink-100"><th className="p-2 text-right">الحقل الأصلي</th><th className="p-2 text-right">الحقل القياسي</th><th className="p-2 text-right">النوع</th><th className="p-2 text-right">الثقة</th><th className="p-2 text-right">الفرادة</th><th className="p-2 text-right">القيم الفارغة</th></tr></thead><tbody>{dataset.columns.map(c=><tr key={c.name} className="border-b border-ink-50"><td className="p-2 font-medium">{c.name}</td><td className="p-2">{c.mappedField||<span className="text-ink-400">غير معين — محفوظ</span>}</td><td className="p-2"><Badge variant="neutral">{c.dataType}</Badge></td><td className="p-2">{c.mappingConfidence}%</td><td className="p-2">{Math.round(c.uniqueRatio*100)}%</td><td className="p-2">{c.nullCount.toLocaleString()}</td></tr>)}</tbody></table></div></CardBody></Card>
      <Card><CardHeader title="المعاينة الكاملة" subtitle={`عرض ${Math.min(dataset.preview.length, 50)} صفًا مع ${dataset.columnCount} عمودًا`}/><CardBody><div className="overflow-x-auto"><DataTable columns={dataset.columns.map(c=>({key:c.name,label:c.name,render:(r:any)=>String(r[c.name]??'')}))} data={dataset.preview.slice(0,50)} emptyMessage="لا توجد صفوف للعرض"/></div></CardBody></Card>
