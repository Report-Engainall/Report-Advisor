...
export function ExternalFileAnalysisPage() {
  ...
  return <div className="space-y-6" dir="rtl">
    ...
    <Card><CardBody>
      ...
      <button type="button" onClick={() => inputRef.current?.click()} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') inputRef.current?.click(); }} className="mt-5 w-full cursor-pointer rounded-2xl border-2 border-dashed border-ink-200 bg-white p-8 text-center transition-colors hover:border-primary-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"><Upload className="mx-auto mb-2 text-primary-500" size={30}/><b>اسحب الملف هنا أو اضغط للاختيار</b><p className="mt-1 text-xs text-ink-400">الحد الآمن {Math.round(MAX_FILE_SIZE / 1024 / 1024)} MB · لا توجد كتابة تلقائية لبيانات الأعمال</p></button>
    ...
    {file && <>
      ...
      <section className="grid gap-4 lg:grid-cols-[1.3fr_.7fr]">
        <Card><CardBody>
          <div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 text-success-600" size={20}/><div><div className="text-sm font-black text-ink-900">تم اجتياز مرحلة المعاينة الآمنة</div><p className="mt-1 text-xs leading-6 text-ink-500">تم فحص الملف واكتشاف نوعه وحساب بصمته وتحليل بنيته. هذه النتيجة ما زالت <b>معاينة/تحليل ملف</b> وليست Canonical Data committed إلى قاعدة الأعمال.</p></div></div>
          <div className="mt-4 grid gap-2 sm:grid-cols-3"><div className="rounded-xl border border-ink-100 bg-ink-50 p-3"><div className="text-[10px] font-bold text-ink-400">أمان الملف</div><div className="mt-1 text-xs font-black text-success-700">PASS</div></div><div className="rounded-xl border border-ink-100 bg-ink-50 p-3"><div className="text-[10px] font-bold text-ink-400">البصمة</div><div className="mt-1 font-mono text-[10px] text-ink-700">{file.hash.slice(0,16)}…</div></div><div className="rounded-xl border border-ink-100 bg-ink-50 p-3"><div className="text-[10px] font-bold text-ink-400">حالة البيانات</div><div className="mt-1 text-xs font-black text-primary-700">معاينة فقط</div></div></div>
        </CardBody></Card>
        <Card><CardBody className="flex h-full flex-col justify-between gap-4"><div><div className="text-xs font-black text-ink-800">الخطوة التجارية التالية</div><p className="mt-1 text-xs leading-5 text-ink-500">لتحويل المعاينة إلى بيانات أعمال موثوقة، استخدم مسار الاستيراد والتحليل الكانوني؛ هذه الصفحة لا تدّعي إتمام الالتزام.</p></div><Link to="/import/analyze" className="btn-primary inline-flex items-center justify-center gap-2 text-xs">افتح مسار الاستيراد الكانوني <ArrowUpLeft size={14}/></Link></CardBody></Card>
      </section>
    </>}
