...
export function CanonicalImportPage() {
  ...
  return <div className="space-y-5 animate-fade-in">
    ...
    {step === 'done' && result && <Card><CardBody><div className="flex flex-col items-center py-10 gap-4"><CheckCircle2 className="text-success-500" size={52}/><h3 className="text-xl font-semibold">تم الاعتماد من المسار السلطوي</h3><div className="rounded-full bg-success-50 px-3 py-1 text-[10px] font-black text-success-700">AUTHORITATIVE SOURCE VERIFIED</div><div className="grid grid-cols-2 gap-3 w-full max-w-2xl text-center sm:grid-cols-4"><div className="p-3 rounded-lg bg-ink-50"><div className="text-xs text-ink-400">الصفوف السلطوية</div><b>{formatNumber(result.total)}</b></div><div className="p-3 rounded-lg bg-primary-50"><div className="text-xs text-primary-700">جودة المصدر</div><b>{result.authoritativeQualityScore ?? 'غير متاح'}%</b></div><div className="p-3 rounded-lg bg-ink-50"><div className="text-xs text-ink-400">ثقة الفهم</div><b>{result.understandingConfidence ?? 0}%</b></div><div className="p-3 rounded-lg bg-ink-50"><div className="text-xs text-ink-400">لقطة الدليل</div><b>{result.snapshotId ? 'مثبتة' : 'غير متاحة'}</b></div></div><div className="w-full max-w-2xl rounded-xl border border-ink-100 bg-ink-50/60 p-3"><div className="text-[9px] font-black tracking-[.1em] text-ink-400">SERVER SOURCE HASH</div><div className="mt-1 break-all font-mono text-[10px] text-ink-700">{result.sourceHash ?? 'غير متاح'}</div><div className="mt-2 text-[10px] leading-5 text-ink-400">هذه البصمة أُعيد حسابها من المصدر المخزّن على الخادم؛ لا تعتمد النتيجة على صفوف أرسلها المتصفح كحقيقة.</div></div><p className="max-w-xl text-center text-[11px] leading-5 text-ink-500">تم تسجيل الاعتماد بعد إعادة قراءة المصدر السلطوي وفحص أمانه وصيغته وجودته ومصالحته وتشغيل المسار الكانوني. الفئة التجارية لا تُخترع من الواجهة.</p><button type="button" onClick={reset} className="btn-primary"><Upload size={14}/> تحليل ملف آخر</button></div></CardBody></Card>}

    <Card><CardHeader title="سجل الاستيرادات" subtitle="أحدث 500 عملية مرتبطة بحسابك؛ العمليات الأقدم تبقى محفوظة ولا تعطل عرض السجل" action={<button type="button" onClick={() => void loadHistory()} className="btn-secondary text-xs"><RefreshCw size={13}/> تحديث</button>}/>{loadingHistory?<LoadingState message="جارٍ تحميل السجل..."/>:historyError?<ErrorState message={historyError} onRetry={() => void loadHistory()} />:history.length===0?<EmptyState icon={<Database size={32}/>} title="لا توجد عمليات سابقة" message="لم يُثبت مصدر سابق لهذا الحساب بعد؛ ابدأ الآن من مدخل الاستيراد الموحد." action={<button type="button" onClick={reset} className="btn-primary text-[11px]"><Upload size={13}/> اختيار مصدر</button>}/>:<DataTable columns={[{key:'file_name',label:'المصدر'},{key:'total_rows',label:'الصفوف',align:'center'},{key:'valid_rows',label:'صالح',align:'center'},{key:'invalid_rows',label:'مراجعة',align:'center'},{key:'status',label:'الحالة',align:'center',render:(r:any)=><StatusBadge status={r.status}/>},{key:'semantic_understanding_confidence',label:'ثقة الفهم',align:'center',render:(r:any)=>r.semantic_understanding_confidence == null ? <Badge variant="neutral">غير مسجل</Badge> : <Badge variant={r.semantic_understanding_confidence>=75?'success':r.semantic_understanding_confidence>=50?'warning':'danger'}>{r.semantic_understanding_confidence}% · فهم</Badge>},{key:'snapshot_id',label:'لقطة الدليل',align:'center',render:(r:any)=>r.snapshot_id ? <Badge variant="success">مثبتة</Badge> : <Badge variant="neutral">غير مثبتة</Badge>},{key:'created_at',label:'التاريخ',render:(r:any)=>formatDateTime(r.created_at)}]} data={history} emptyMessage="لا توجد عمليات سابقة"/>}</Card>
  </div>;
}
