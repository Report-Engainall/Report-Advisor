...
export function CanonicalImportPage() {
  ...
  return <div className="space-y-6 animate-fade-in">
    ...
    {step === 'upload' && <Card><CardBody>
      <div className="mb-5"><label className="block text-sm font-medium text-ink-700 mb-2">نوع البيانات</label><div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {ENTITIES.map(e => <button key={e.value} onClick={() => setEntityType(e.value)} className={`p-4 rounded-lg border-2 text-right ${entityType === e.value ? 'border-primary-500 bg-primary-50/50' : 'border-ink-100'}`}><Database size={18}/><div className="text-sm font-medium mt-2">{e.label}</div><div className="text-[11px] text-ink-400">{e.required.length} حقول مطلوبة</div></button>)}
      </div></div>
      <div onClick={() => inputRef.current?.click()} className="border-2 border-dashed rounded-xl p-10 text-center cursor-pointer hover:border-primary-400 transition-colors">
        ...
        <Upload className="mx-auto text-primary-500 mb-3" size={28}/><h3 className="font-semibold">اختر ملفًا للاستيراد</h3><p className="text-sm text-ink-500 mt-1">Excel، CSV، JSON، PDF، Word والصور</p><p className="text-xs text-ink-300 mt-3">الحد الأقصى: {MAX_FILE_SIZE / 1024 / 1024} MB</p>
      </div>
