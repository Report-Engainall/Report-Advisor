...
function statusClass(status: CertificationStatus): string { if (status === 'CERTIFIED') return 'bg-success-50 text-success-700'; if (status === 'REVIEWED') return 'bg-primary-50 text-primary-700'; if (status === 'DEPRECATED') return 'bg-ink-100 text-ink-600'; return 'bg-warning-50 text-warning-700'; }
function freshnessClass(state: 'FRESH' | 'STALE' | 'UNKNOWN'): string { if (state === 'FRESH') return 'text-success-600'; if (state === 'STALE') return 'text-warning-600'; return 'text-ink-400'; }
...
export function MetricInspectorPage() {
  ...
  const load = useCallback(async () => { try { setLoading(true); setError(null); const result = await listSemanticMetricContracts(); setItems(result); setSelected(current => current ? result.find(item => item.definition.metricId === current.definition.metricId) ?? result[0] ?? null : result[0] ?? null); } catch (e) { setError(e instanceof Error ? e.message : 'تعذر تحميل حوكمة المؤشرات'); } finally { setLoading(false); } }, []);
