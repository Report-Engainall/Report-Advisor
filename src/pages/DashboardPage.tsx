...
import { fetchMonthlyTrend, fetchTopCustomers, fetchTopProducts, fetchCategoryBreakdown, fetchRecommendations, fetchAlerts, fetchAgingBuckets } from '@/lib/queries';
  ...
  const load = useCallback(async (silent = false) => {
    try {
      ...
      const [k, t, tc, tp, cat, recs, alts, ag] = await Promise.all([
        fetchCanonicalDashboardKPIs(), fetchMonthlyTrend(trendMonths), fetchTopCustomers(5), fetchTopProducts(5),
        fetchCategoryBreakdown(), fetchRecommendations(), fetchAlerts(), fetchAgingBuckets(),
      ]);
      setKpis(k); setTrend(t); setTopCustomers(tc); setTopProducts(tp); setCategories(cat); setRecommendations(recs); setAlerts(alts); setAging(ag);
