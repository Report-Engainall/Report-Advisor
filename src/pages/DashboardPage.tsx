...
import { readWorkspacePreferences, type DashboardWidgetId } from '@/lib/workspace-preferences';
...
export function DashboardPage() {
  ...
  const [visibleWidgets, setVisibleWidgets] = useState<DashboardWidgetId[]>(() => readWorkspacePreferences(null).dashboardWidgets);
  ...
  useEffect(() => {
    let active = true;
    const sync = () => { void resolveCurrentCompanyId().then(companyId => { if (active) setVisibleWidgets(readWorkspacePreferences(companyId).dashboardWidgets); }).catch(() => undefined); };
    sync();
    window.addEventListener('report-advisor:workspace-preferences', sync);
    return () => { active = false; window.removeEventListener('report-advisor:workspace-preferences', sync); };
  }, []);
  ...
  const widgetVisible = (id: DashboardWidgetId) => visibleWidgets.includes(id);
  ...
  return (
    <div dir="rtl" className="animate-fade-in space-y-6 pb-10">
      ...
      {widgetVisible('signals') && <section className="rounded-[18px] border border-[#15372f] bg-[#0d2a24] p-4 text-white shadow-[0_14px_40px_rgba(13,42,36,.16)] sm:p-5">
      ...
      {widgetVisible('kpis') && <section className="grid gap-3 lg:grid-cols-4">
      ...
      {widgetVisible('kpis') && <section className="grid grid-cols-2 gap-px overflow-hidden rounded-[10px] border border-ink-200 bg-ink-200 sm:grid-cols-4">
