...
import { fetchDashboardKPIs, fetchRecommendations, fetchAlerts, fetchAgingBuckets } from '@/lib/queries';
import { fetchDashboardSecondaryTruth } from '@/lib/dashboard-secondary-truth';
...
export function DashboardPage() {
  ...
  const load = useCallback(async (silent = false) => {
    try {
      ...
      setError(null);
      const [k, secondary, recs, alts, ag] = await Promise.all([
        fetchDashboardKPIs(), fetchDashboardSecondaryTruth(trendMonths), fetchRecommendations(), fetchAlerts(), fetchAgingBuckets(),
      ]);
      const completeTrend = secondary.status === 'CALCULATED'
        ? secondary.trend.filter(x => x.sales !== null && x.cost !== null && x.profit !== null).map(x => ({ month: x.month, label: x.month, sales: x.sales as number, cost: x.cost as number, profit: x.profit as number, invoices: x.invoices }))
        : [];
      const completeCustomers = secondary.status === 'CALCULATED' ? secondary.topCustomers.filter(x => x.value !== null).map(x => ({ id: x.id, name: x.name, value: x.value as number })) : [];
      const completeProducts = secondary.status === 'CALCULATED' ? secondary.topProducts.filter(x => x.value !== null).map(x => ({ id: x.id, name: x.name, value: x.value as number, secondary: x.secondary === null ? undefined : x.secondary })) : [];
      const completeCategories = secondary.status === 'CALCULATED' ? secondary.categories.filter(x => x.sales !== null && x.profit !== null && x.quantity !== null).map(x => ({ name: x.name, sales: x.sales as number, profit: x.profit as number, quantity: x.quantity as number })) : [];
      setKpis(k); setTrend(completeTrend); setTopCustomers(completeCustomers); setTopProducts(completeProducts); setCategories(completeCategories);
