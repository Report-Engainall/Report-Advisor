...
export function DashboardPage() {
  ...
  const grossMarginHint = kpis.grossMargin === null ? 'هامش: غير متاح — بيانات التكلفة غير مكتملة' : `هامش: ${kpis.grossMargin.toFixed(1)}%`;

  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div><div className="flex items-center gap-2"><h1 className="text-2xl font-bold text-ink-900">لوحة القيادة</h1><span className="inline-flex items-center gap-1 rounded-full bg-success-50 px-2 py-1 text-[11px] font-medium text-success-700"><CheckCircle2 size={13} /> بيانات مؤكدة</span></div><p className="mt-1 text-sm text-ink-500">نظرة تنفيذية على الأداء والذمم والمخزون والأولويات</p></div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <KPICard label="إجمالي الربح" value={kpis.grossProfit} format="currency" icon={<TrendingUp size={16} />} status={kpis.status} hint={grossMarginHint} />
