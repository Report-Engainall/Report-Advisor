...
export interface FinancialIntelligenceInput {
  ...
  receivablesItems?: Parameters<typeof prioritizeReceivables>[0];
  supplierItems?: Parameters<typeof prioritizeSupplierPayments>[0];
  periodDays?: number;
}
...
export interface FinancialIntelligence {
  ...
  reserve: ReserveProtection;
  collections: ReceivablePriority[];
  supplierPayments: PaymentPriority[];
  profitability: { revenue: number; costOfSales: number | null; grossProfit: number | null; marginPct: number | null; status: 'READY' | 'PROFIT_UNAVAILABLE' };
  warnings: string[];
}
...
export function buildFinancialIntelligence(input: FinancialIntelligenceInput): FinancialIntelligence {
  ...
  const cost = input.costOfSales != null && Number.isFinite(input.costOfSales) && input.costOfSales >= 0 ? input.costOfSales : null;
  const grossProfit = cost == null ? null : revenue - cost;
  const marginPct = cost == null || revenue <= 0 ? null : grossProfit! / revenue * 100;
  const ccc = cashConversionCycle({
    receivables: Math.max(0, input.receivables),
    revenue,
    inventory: Math.max(0, input.inventory),
    costOfSales: cost ?? 0,
    payables: Math.max(0, input.payables),
    purchases: Math.max(0, input.purchases),
    periodDays: input.periodDays ?? 365,
  });
  const liquidity = projectLiquidity({
    openingLiquidity: Math.max(0, input.openingCash),
    horizons: [0, 7, 15, 30, 60, 90],
    dailyInflow: Math.max(0, input.dailyInflow),
    dailyOutflow: Math.max(0, input.dailyOutflow),
  ...
  const reserve = protectCashReserve({
    ...
    committedOutflow: Math.max(0, input.committedOutflow ?? 0),
    collectibleInflow: Math.max(0, input.dailyInflow) * 30,
  });
  ...
  if (cost == null) warnings.push('PROFIT_UNAVAILABLE: تكلفة المبيعات الفعلية غير متاحة؛ لم يتم استخدام المشتريات كبديل.');
  ...
  if (reserve.status === 'PROTECT') warnings.push('حماية الاحتياطي النقدي مفعلة؛ يجب عدم استنزاف النقد التشغيلي.');
  return { ccc, liquidity, reserve, collections, supplierPayments, profitability: { revenue, costOfSales: cost, grossProfit, marginPct, status: cost == null ? 'PROFIT_UNAVAILABLE' : 'READY' }, warnings };
}
