...
import { calculateGrossProfitTruth, type GrossProfitDateRange, type GrossProfitStatus, type GrossProfitTruth } from './grossProfitTruthCore';
export { calculateGrossProfitTruth } from './grossProfitTruthCore';
...
export async function fetchGrossProfitTruth(range:GrossProfitDateRange={}):Promise<GrossProfitTruth>{
 ...
 const approvedInvoices=invoices.filter(i=>['confirmed','posted','paid'].includes(String(i.status).toLowerCase())&&(!range.startDate||i.invoice_date>=range.startDate)&&(!range.endDate||i.invoice_date<=range.endDate));
