...
export async function fetchSalesInvoices(page=0,pageSize=20):Promise<{data:SalesInvoice[];count:number|null}>{if(!Number.isInteger(page)||page<0)throw new Error('REPORT_QUERY_INVALID_PAGE');if(!Number.isInteger(pageSize)||pageSize<1||pageSize>500)throw new Error('REPORT_QUERY_INVALID_PAGE_SIZE');const companyId=await resolveCurrentCompanyId();if(!companyId)throw new Error('TENANT_REQUIRED');const from=page*pageSize,to=from+pageSize-1,{data,count,error}=await supabase.from('sales_invoices').select('*, customer:customers(id,name)',{count:'exact'}).eq('company_id',companyId).order('invoice_date',{ascending:false}).order('created_at',{ascending:false}).order('id',{ascending:true}).range(from,to);if(error)throw error;if(!Array.isArray(data))throw new Error('REPORT_DATA_UNAVAILABLE: sales invoices rows missing');for(const row of data){if(!row||typeof row!=='object'||Array.isArray(row))throw new Error('REPORT_DATA_UNAVAILABLE: sales invoice row shape invalid');const item=row as Record<string,unknown>;if(typeof item.id!=='string'||!item.id.trim()||typeof item.company_id!=='string'||item.company_id!==companyId||typeof item.customer_id!=='string'||!item.customer_id.trim()||typeof item.invoice_number!=='string'||!item.invoice_number.trim()||typeof item.invoice_date!=='string'||!Number.isFinite(new Date(item.invoice_date).getTime())||typeof item.status!=='string'||!item.status.trim()||!Number.isFinite(Number(item.total))||!Number.isFinite(Number(item.paid_amount)))throw new Error('REPORT_DATA_UNAVAILABLE: sales invoice row shape invalid');}return{data:data as SalesInvoice[],count};}
export async function fetchPurchaseInvoices(page=0,pageSize=20):Promise<{data:PurchaseInvoice[];count:number|null}>{if(!Number.isInteger(page)||page<0)throw new Error('REPORT_QUERY_INVALID_PAGE');if(!Number.isInteger(pageSize)||pageSize<1||pageSize>500)throw new Error('REPORT_QUERY_INVALID_PAGE_SIZE');const companyId=await resolveCurrentCompanyId();if(!companyId)throw new Error('TENANT_REQUIRED');const from=page*pageSize,to=from+pageSize-1,{data,count,error}=await supabase.from('purchase_invoices').select('*, supplier:suppliers(id,name)',{count:'exact'}).eq('company_id',companyId).order('invoice_date',{ascending:false}).order('id',{ascending:true}).range(from,to);if(error)throw error;if(!Array.isArray(data))throw new Error('REPORT_DATA_UNAVAILABLE: purchase invoices rows missing');for(const row of data){if(!row||typeof row!=='object'||Array.isArray(row))throw new Error('REPORT_DATA_UNAVAILABLE: purchase invoice row shape invalid');const item=row as Record<string,unknown>;if(typeof item.id!=='string'||!item.id.trim()||typeof item.company_id!=='string'||item.company_id!==companyId||typeof item.supplier_id!=='string'||!item.supplier_id.trim()||typeof item.invoice_number!=='string'||!item.invoice_number.trim()||typeof item.invoice_date!=='string'||!Number.isFinite(new Date(item.invoice_date).getTime())||typeof item.status!=='string'||!item.status.trim()||!Number.isFinite(Number(item.total))||!Number.isFinite(Number(item.paid_amount)))throw new Error('REPORT_DATA_UNAVAILABLE: purchase invoice row shape invalid');}return{data:data as PurchaseInvoice[],count};}
...
export async function fetchImportRecords(limit = MAX_IMPORT_RECORD_ROWS, focusJobId?: string): Promise<ImportRecord[]> {
  if (!Number.isInteger(limit) || limit < 1 || limit > MAX_IMPORT_RECORD_ROWS) throw new Error('REPORT_QUERY_INVALID_IMPORT_LIMIT');
  const companyId = await resolveCurrentCompanyId();
  if (!companyId) throw new Error('TENANT_REQUIRED');
  const { data, error } = await supabase.from('import_jobs').select('id, company_id, job_type, status, total_rows, valid_rows, invalid_rows, quarantined_rows, progress, error_message, created_at, completed_at, result_summary').eq('company_id', companyId).order('created_at', { ascending: false }).order('id', { ascending: true }).range(0, limit - 1);
  if (error) throw error;
  let rows = data ?? [];
  if (focusJobId && !rows.some(row => row.id === focusJobId)) {
    const { data: focusedRow, error: focusedError } = await supabase.from('import_jobs').select('id, company_id, job_type, status, total_rows, valid_rows, invalid_rows, quarantined_rows, progress, error_message, created_at, completed_at, result_summary').eq('company_id', companyId).eq('id', focusJobId).maybeSingle();
    if (focusedError) throw focusedError;
    if (focusedRow) rows = [focusedRow, ...rows];
  }
  return rows.map(row => {
    const summary = (row.result_summary as Record<string, unknown> | null) ?? {};
    const sourceType = typeof summary.source_type === 'string' && summary.source_type.trim() ? summary.source_type.trim() : 'import';
    const fileName = typeof summary.file_name === 'string' && summary.file_name.trim() ? summary.file_name.trim() : String(row.job_type ?? 'import');
    const fileSize = typeof summary.file_size === 'number' && Number.isFinite(summary.file_size) && summary.file_size >= 0 ? summary.file_size : 0;
    return {
      id: row.id, company_id: row.company_id, job_type: row.job_type, status: row.status,
      file_name: fileName, file_size: fileSize, source_type: sourceType,
      total_rows: row.total_rows == null ? null : Number(row.total_rows),
      valid_rows: row.valid_rows == null ? null : Number(row.valid_rows),
      invalid_rows: row.invalid_rows == null ? null : Number(row.invalid_rows),
      quarantined_rows: row.quarantined_rows == null ? null : Number(row.quarantined_rows),
      entity_type: row.job_type ?? null, progress: row.progress == null ? null : Number(row.progress),
      error_message: row.error_message ?? null, created_at: row.created_at, completed_at: row.completed_at ?? null,
    };
  });
}
