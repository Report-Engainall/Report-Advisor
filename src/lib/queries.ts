...
type ImportJobCounters = { processed_rows: number; valid_rows: number; invalid_rows: number; duplicate_rows: number; status: string; progress: number };

async function loadImportJobCounters(id: string, companyId: string): Promise<ImportJobCounters> {
  const { data, error } = await supabase.from('import_jobs')
    .select('processed_rows,valid_rows,invalid_rows,duplicate_rows,status,progress')
...
export async function updateImportRecord(id: string, patch: ImportRecordPatch): Promise<void> {
  const companyId = await resolveCurrentCompanyId();
  if (!companyId) throw new Error('TENANT_REQUIRED');
  const current = await loadImportJobCounters(id, companyId);
  ...
  if (patch.progress !== undefined || patch.error_message !== undefined || patch.status !== undefined) {
    const processedRows = patch.progress === undefined
      ? current.processed_rows
      : Math.max(0, Math.round(patch.progress));
