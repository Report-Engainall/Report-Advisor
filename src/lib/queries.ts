...
export async function fetchImportRecords(): Promise<ImportRecord[]> { const companyId = await resolveCurrentCompanyId(); if (!companyId) throw new Error('TENANT_REQUIRED'); const { data, error } = await supabase.from('import_jobs').select('id, company_id, job_type, status, total_rows, valid_rows, invalid_rows, quarantined_rows, progress, error_message, created_at, completed_at, result_summary').eq('company_id', companyId).order('created_at', { ascending: false }).order('id', { ascending: true }).range(0, MAX_IMPORT_RECORD_ROWS - 1); if (error) throw error; return (data ?? []).map(row => { const summary = (row.result_summary as Record<string, unknown> | null) ?? {}; const understandingRaw = summary.semantic_understanding_confidence; const understandingConfidence = typeof understandingRaw === 'number' && Number.isFinite(understandingRaw) ? Math.max(0, Math.min(100, Math.round(understandingRaw))) : null; const snapshotId = typeof summary.snapshot_id === 'string' && summary.snapshot_id.trim() ? summary.snapshot_id : null; return { id: row.id, company_id: row.company_id, file_name: String(summary.file_name ?? row.job_type ?? 'import'), file_size: 0, source_type: 'import', status: row.status, total_rows: row.total_rows == null ? null : Number(row.total_rows), valid_rows: row.valid_rows == null ? null : Number(row.valid_rows), invalid_rows: row.invalid_rows == null ? null : Number(row.invalid_rows), quarantined_rows: row.quarantined_rows == null ? null : Number(row.quarantined_rows), entity_type: row.job_type ?? null, progress: row.progress == null ? null : Number(row.progress), error_message: row.error_message ?? null, semantic_understanding_confidence: understandingConfidence, snapshot_id: snapshotId, created_at: row.created_at, completed_at: row.completed_at ?? null }; }); }
