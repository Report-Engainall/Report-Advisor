...
export type ReceivablesReportRow = ReceivablesReportRowCanonical;
export type { ReceivablesReportPage, CanonicalExportRow };
...
export interface InventoryValuation {
  ...
  data_status: 'NO_DATA' | 'INSUFFICIENT_DATA' | 'CALCULATED';
}
...
async function canonicalExportRows(functionName: string): Promise<CanonicalExportRow[]> {
  const companyId = await resolveCurrentCompanyId();
  if (!companyId) throw new Error('TENANT_REQUIRED');
  const { data, error } = await supabase.rpc(functionName, { p_company_id: companyId, p_max_rows: 10000 });
  if (error) throw error;
  const payload = (data ?? {}) as Record<string, unknown>;
  if (!Array.isArray(payload.rows)) throw new Error('REPORT_DATA_UNAVAILABLE: canonical export rows missing');
...
export interface ImportRecordPage { data: ImportRecord[]; count: number; page: number; page_size: number; }
...
export async function createImportRecord(input: ImportRecordInput): Promise<ImportRecord> {
  const companyId = await resolveCurrentCompanyId();
  if (!companyId) throw new Error('TENANT_REQUIRED');
  if (input.total_rows == null) throw new Error('IMPORT_TOTAL_ROWS_REQUIRED');
  const hasSourceUpload = Boolean(input.source_object_path);
  if (hasSourceUpload && (!input.file_name || !input.file_mime || !input.file_size || input.file_size <= 0)) {
    throw new Error('IMPORT_SOURCE_METADATA_INCOMPLETE');
  }
  const { data, error } = await supabase.rpc('import_create_job', {
    ...
    p_source_object_path: input.source_object_path ?? null,
    p_file_name: hasSourceUpload ? input.file_name : null,
    p_file_size: hasSourceUpload ? input.file_size : null,
    p_file_mime: hasSourceUpload ? input.file_mime : null,
  });
  if (error) throw error;
...
export async function fetchImportRecordPage(page = 0, pageSize = DEFAULT_IMPORT_PAGE_SIZE): Promise<ImportRecordPage> {
  if (!Number.isInteger(page) || page < 0) throw new Error('REPORT_QUERY_INVALID_PAGE');
  if (!Number.isInteger(pageSize) || pageSize < 1 || pageSize > MAX_IMPORT_RECORD_ROWS) throw new Error('REPORT_QUERY_INVALID_PAGE_SIZE');
  const companyId = await resolveCurrentCompanyId();
  if (!companyId) throw new Error('TENANT_REQUIRED');
  const from = page * pageSize;
  const to = from + pageSize - 1;
  const { data, count, error } = await supabase.from('import_jobs').select('id, company_id, job_type, status, total_rows, valid_rows, invalid_rows, quarantined_rows, progress, error_message, created_at, completed_at, result_summary', { count: 'exact' }).eq('company_id', companyId).order('created_at', { ascending: false }).order('id', { ascending: true }).range(from, to);
  if (error) throw error;
  if (count == null) throw new Error('REPORT_QUERY_LIMIT_EXCEEDED: import history count unavailable');
  return { data: (data ?? []).map(mapImportRecord), count, page, page_size: pageSize };
}
export async function fetchImportRecords(page = 0, pageSize = DEFAULT_IMPORT_PAGE_SIZE): Promise<ImportRecord[]> {
  return (await fetchImportRecordPage(page, pageSize)).data;
}
