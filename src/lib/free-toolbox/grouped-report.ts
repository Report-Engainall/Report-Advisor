...
export function toGroupedReport(rows:DetailReportRow[],groups:Record<string,{name:string;members:string[]}>):GroupedReportRow[]{
 ...
 for(const r of rows){
  ...
  if(Number.isFinite(r.netSalesUnits))x.netSalesUnits+=r.netSalesUnits;else state.sales=true;
  if(Number.isFinite(r.dailyDemand))x.dailyDemand+=r.dailyDemand;else state.demand=true;
 ...
 return [...out.values()].map(x=>{
  ...
  return {...x,daysOfCover:Number.isFinite(x.dailyDemand)&&x.dailyDemand>0?x.stockUnits/x.dailyDemand:Number.NaN};
 });
}
