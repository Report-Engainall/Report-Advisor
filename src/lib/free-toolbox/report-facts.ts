...
export function attachEvidence(facts: ReportFact[], ledger: EvidenceLedger) { return facts.map(f => ({ ...f, confidence: boundedConfidence(f.confidence), evidence: f.evidence ?? ledger.items.filter(e => e.field === f.key).map(e => `${e.sourceId}${e.page ? `#page-${e.page}` : ''}`) })) }
