...
function rowVersions(rows: ReconciledCanonicalImportRow[], sourceHash: string): RowVersion<Record<string, unknown>>[] { return rows.map((row) => ({ key: rowKey(row), hash: `${sourceHash}:${row.rowNumber}`, value: row.data })); }
function sourceCandidates(rows: ReconciledCanonicalImportRow[]): SourceCandidate<Record<string, unknown>>[] { const observedAt = new Date().toISOString(); return rows.map((row) => ({ businessKey: rowKey(row), sourceId: row.provenance.sourceId, precedence: 0, observedAt, value: row.data })); }
function evidence(rows: ReconciledCanonicalImportRow[], sourceHash: string): RuntimeEvidence[] { const observedAt = new Date().toISOString(); return [{ key: `canonical-import:${sourceHash}:source`, source: 'CanonicalImportPage/file-engine', observedAt, quality: 1, details: { rowCount: rows.length, sourceHash } }, { key: `canonical-import:${sourceHash}:reconciliation`, source: 'canonical-truth-boundary.reconcileForCanonical', observedAt, quality: 1, details: { rowCount: rows.length, reconciled: true } }]; }

async function failImportJob(importJobId: string, sourceHash: string, error: unknown): Promise<void> {
  try {
    await supabase.rpc('import_finish_job', {
      p_job_id: importJobId,
      p_status: 'failed',
      p_result_summary: { sourceHash },
      p_error_message: error instanceof Error ? error.message : String(error),
    });
  } catch {
    // Preserve the primary validation failure; finalization is best effort.
  }
}
...
export async function runCanonicalProductionImport(input: CanonicalProductionImportInput) {
  ...
  const { data: importJob, error: importJobError } = await supabase.from('import_jobs').select('total_rows,invalid_rows').eq('id', input.importJobId).eq('company_id', companyId).single();
  ...
  try {
    const { data: company, error: companyError } = await supabase.from('companies').select('currency').eq('id', companyId).single();
    ...
    if (input.entityType === 'sales_invoices') {
      for (const row of input.rows) {
        ...
        if (rowCurrency && rowCurrency !== companyCurrency) throw new Error(`IMPORT_CURRENCY_MISMATCH:${rowCurrency}:${companyCurrency}`);
      }
    }
  } catch (error) {
    await failImportJob(input.importJobId, sourceHash, error);
    throw error;
  }
  ...
  const request: ReportExecutionRequest = { reportId: `canonical-import:${input.importJobId}`, tenantId: companyId, requestedBy: userData.user.id, parameters: { entityType: input.entityType, importJobId: input.importJobId, sourceFile: input.fileName }, formats: ['web'], idempotencyKey: `canonical-import:${input.entityType}:${sourceHash}` };
  const durableSourceIdentity = request.idempotencyKey;
  const { data: enqueueData, error: enqueueError } = await supabase.rpc('enqueue_report_execution_job', { p_company_id: companyId, p_job_key: request.idempotencyKey, p_source_path: durableSourceIdentity, p_source_hash: sourceHash, p_evidence_keys: evidence(input.rows, sourceHash).map((item) => item.key), p_max_attempts: 3 });
  ...
  try {
    const lifecycle = await runDurableProductionLifecycle({
      ...
      lifecycle: { previousRows: [], currentRows, sourceCandidates: candidates, scenarioOptions: [], riskBudget: { maxRisk: 0, protectedLiquidity: 0, minimumServiceLevel: 0 }, portfolioCandidates: [], autonomy: { trustHealthy: false, evidenceQuality: 1, confidence: 0, riskBudgetValid: true, criticalDrift: false, rollbackVerified: false, isolationVerified: true }, evidence: runtimeEvidence },
      executeStage: async (stage, stageInput) => {
        ...
        if (stage === 'committed') {
          ...
          const { error } = await supabase.rpc('import_update_job_progress', { p_job_id: input.importJobId, p_processed_rows: totalRows, p_valid_rows: input.rows.length, p_invalid_rows: invalidRows, p_duplicate_rows: 0, p_status: 'processing' });
          if (error) { /* Telemetry only; canonical transaction is already authoritative. */ }
        }
      },
    }, store);

    const { error: finishError } = await supabase.rpc('import_finish_job', { p_job_id: input.importJobId, p_status: 'completed', p_result_summary: { sourceHash, durableExecutionJobId: jobId, committed: input.rows.length, invalidRows, lifecycleCompleted: true }, p_error_message: null });
