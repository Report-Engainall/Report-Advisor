...
export async function processFolderFiles(files:FolderScanFile[],entityType:BatchEntityType,onProgress?:(progress:BatchProgress)=>void):Promise<BatchProgress>{
  ...
  for(let index=0;index<files.length;index++){
    ...
    try{
      ...
      for(let offset=0;offset<validRows.length;offset+=500){const batch=validRows.slice(offset,offset+500);const result=await commitImportBatch(entityType,batch);committed+=result.committed;await updateImportRecord(importRecordId,{progress:Math.round(committed/validRows.length*100)});}
      ...
      if(importRecordId){try{await updateImportRecord(importRecordId,{status:'failed',error_message:message,completed_at:new Date().toISOString()});}catch{ /* preserve primary import failure */ }}
