...
export interface BatchFileResult { name:string; path:string; status:'completed'|'skipped'|'failed'; format:string; rows:number; committed:number; error?:string; duplicate?:boolean; warning?:string; }
...
function requiredFields(entityType: BatchEntityType): string[] { return entityType === 'products' ? ['sku','name','unit','cost_price','selling_price','min_stock','reorder_point','is_active'] : entityType === 'customers' ? ['name','segment','credit_limit','payment_terms_days'] : ['invoice_number','invoice_date','customer_name','subtotal','tax_amount','total','paid_amount','status']; }
...
export async function processFolderFiles(files: FolderScanFile[], entityType: BatchEntityType, onProgress?: (progress: BatchProgress) => void): Promise<BatchProgress> {
  ...
  for (let index=0; index<files.length; index++) {
    const item=files[index]; onProgress?.({processed:index,total:files.length,current:item.relativePath,results:[...results]});
    let importRecord: { id: string } | null = null; let committed = 0;
    try {
      ...
      const hash=await computeSHA256(buffer); const duplicate=await checkDuplicate(hash); if(duplicate.isDuplicate){results.push({name:item.file.name,path:item.relativePath,status:'skipped',format:detection.format,rows:0,committed:0,duplicate:true,error:'تم استيراد الإصدار نفسه سابقاً.'});continue;}
      const datasets=await parseFile(buffer,item.file.name,detection.format); const dataset=datasets[0]; if(!dataset||dataset.rowCount===0) throw new Error('الملف فارغ أو لا يحتوي على بيانات قابلة للقراءة');
      const extraction=await finalizeExtraction(hash,detection.format,canonicalTextFromRows(dataset.rows)); const required=requiredFields(entityType); const validRows:CanonicalImportRow[]=[];
      for(let rowIndex=0;rowIndex<dataset.rows.length;rowIndex++){const data=dataset.rows[rowIndex];const missing=required.filter(field=>{const key=Object.keys(data).find(k=>k===field)??Object.keys(data).find(k=>k.toLowerCase().includes(field.toLowerCase()));const value=key?data[key]:undefined;return value==null||String(value).trim()==='';});if(!missing.length)validRows.push({rowNumber:rowIndex+1,data});}
      ...
      const recordId=importRecord.id;
      for(let offset=0;offset<validRows.length;offset+=50){const batch=validRows.slice(offset,offset+50);const result=await commitImportBatch(entityType,batch);committed+=result.committed;await updateImportRecord(recordId,{progress:Math.round(committed/validRows.length*100)});}
      await updateImportRecord(recordId,{status:'completed',progress:100,completed_at:new Date().toISOString()});
    ...
    } catch(error) { const message=error instanceof Error?error.message:'خطأ غير معروف'; if(importRecord){try{await updateImportRecord(importRecord.id,{status:'failed',progress:committed,error_message:message});}catch{/* preserve original failure */}} results.push({name:item.file.name,path:item.relativePath,status:'failed',format:item.format,rows:0,committed,error:message}); }
  }
  const final={processed:files.length,total:files.length,current:'',results}; onProgress?.(final); return final;
}
