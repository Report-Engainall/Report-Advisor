...
export async function scanDirectory(handle:BrowserDirectoryHandle,prefix=''):Promise<FolderScanFile[]>{
  if(!handle||typeof handle.entries!=='function')throw new Error('لم يتم اختيار مجلد صالح.');
  const files:FolderScanFile[]=[];
  for await(const [name,entry]of handle.entries()){
    const relativePath=prefix?`${prefix}/${name}`:name;
    if(entry.kind==='directory'){
      for(const item of await scanDirectory(entry as BrowserDirectoryHandle,relativePath))files.push(item);
      continue;
    }
...
function requiredFields(entityType:BatchEntityType):string[]{
  return entityType==='products'
    ...
    :entityType==='customers'
      ...
      :['invoice_number','invoice_date','subtotal','tax_amount','total','paid_amount','status'];
}
...
export async function processFolderFiles(files:FolderScanFile[],entityType:BatchEntityType,onProgress?:(progress:BatchProgress)=>void):Promise<BatchProgress>{
  ...
  if(!companyId)throw new Error('TENANT_REQUIRED: لا يمكن تنفيذ الاستيراد بدون شركة مصادق عليها.');
  ...
  for(let index=0;index<files.length;index++){
    ...
    try{
      ...
      const duplicate=await checkDuplicate(hash,companyId);
      ...
      // File-level atomic boundary: one RPC carries the complete valid payload.
      // No earlier chunk is committed, so a failure rolls back the whole file.
      const result=await commitImportBatch(entityType,validRows);
      ...
      const message=error instanceof Error?error.message:'حدث خطأ غير معروف أثناء الاستيراد';
      if(importRecordId){
        try{await updateImportRecord(importRecordId,{status:'failed',error_message:message,completed_at:new Date().toISOString()});}catch{/* Preserve the original import failure when best-effort status finalization also fails. */}}
