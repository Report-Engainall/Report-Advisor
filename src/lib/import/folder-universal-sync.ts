...
export interface UniversalFolderResult {
  name: string;
  path: string;
  status: 'completed' | 'analyzed' | 'skipped' | 'failed';
  format: string;
  rows: number;
  committed: number;
  datasets: number;
...
export interface UniversalFolderProgress {
  processed: number;
  total: number;
  current: string;
  results: UniversalFolderResult[];
  startedAt: string;
  updatedAt: string;
}
...
function classifyValidRows(dataset: Dataset, entityType: Exclude<UniversalFolderEntity, 'document_analysis'>) {
  ...
  dataset.rows.forEach((sourceData, index) => {
    ...
    const valid = required.every((field) => data[field] != null && String(data[field]).trim() !== '');
    if (valid) validRows.push({ rowNumber: index + 1, data });
    else rejected += 1;
  });
...
async function processDataset(
  ...
  dataset: Dataset,
  entityType: UniversalFolderEntity,
): Promise<{ committed: number; warning?: string; importRecordId?: string }> {
  if (entityType === 'document_analysis') {
    return { committed: 0, warning: 'تم تحليل الوثيقة وحفظ بصمتها وملفها التعريفي؛ لم يتم اختراع كيان أعمال أو إسقاط الحقول غير المعروفة.' };
  }

  const extraction = await finalizeExtraction(hash, 'unknown' as FileFormat, canonicalTextFromRows(dataset.rows));
  if (extraction.status !== 'succeeded') {
    return { committed: 0, warning: `تم التحليل دون كتابة: الاستخراج القانوني لم يكتمل (${extraction.status}).` };
  }

  const { validRows, rejected } = classifyValidRows(dataset, entityType);
  if (!validRows.length) {
    return { committed: 0, warning: `تم تحليل الملف لكن لم توجد صفوف مكتملة لمفتاح ${entityType}. الحقول الأصلية محفوظة ولم تُحذف.` };
  }

  const reconciliation = reconcileForCanonical(
    entityType,
    companyId,
    item.relativePath,
    hash,
    hash,
    (data, rowNumber) => `evidence:${hash}:${rowNumber}:${JSON.stringify(data)}`,
    validRows,
  );
  if (reconciliation.rejected.length) {
    return { committed: 0, warning: `تم التحليل دون كتابة بسبب تعارضات في ${reconciliation.rejected.length} صف؛ لم يتم تجاوز التعارض تلقائيًا.` };
  }
  ...
  const rec = await createImportRecord({
    ...
    status: 'processing',
    total_rows: dataset.rowCount,
    valid_rows: canonicalRows.length,
    invalid_rows: rejected + (dataset.rowCount - validRows.length - rejected),
    quarantined_rows: dataset.rowCount - validRows.length,
  ...
  for (let offset = 0; offset < canonicalRows.length; offset += 500) {
    ...
    await updateImportRecord(rec.id, { progress: Math.round(Math.min(1, (offset + batch.length) / canonicalRows.length) * 100) });
  }
  await updateImportRecord(rec.id, { status: 'completed', progress: 100, completed_at: new Date().toISOString() });
  return { committed: canonicalRows.length, importRecordId: rec.id, warning: rejected ? `استوردنا الصفوف الصالحة فقط، ووضعنا ${rejected} صفًا غير مكتمل في حالة مراجعة.` : undefined };
}

export async function processFolderFilesUniversal(
  files: FolderScanFile[],
  onProgress?: (progress: UniversalFolderProgress) => void,
): Promise<UniversalFolderProgress> {
  ...
  for (let index = 0; index < files.length; index += 1) {
    ...
    try {
      ...
      if (duplicate.isDuplicate) {
        results.push({ name: item.file.name, path: item.relativePath, status: 'skipped', format: detection.format, rows: 0, committed: 0, datasets: 0, entityType: 'document_analysis', qualityScore: 100, duplicate: true, warning: 'نفس البصمة موجودة سابقًا؛ تم تجاوز الملف دون إعادة الكتابة.' });
        continue;
      }
      ...
      let committed = 0;
      const entityTypes = new Set<UniversalFolderEntity>();
      const warnings: string[] = [];
      let qualityScore = 0;
      for (const dataset of datasets) {
        const entityType = inferEntity(dataset);
        entityTypes.add(entityType);
        qualityScore += dataset.qualityScore;
        const outcome = await processDataset(companyId, item, hash, dataset, entityType);
        committed += outcome.committed;
        if (outcome.warning) warnings.push(`${dataset.name}: ${outcome.warning}`);
      }
      ...
      results.push({
        ...
        status: committed > 0 ? 'completed' : 'analyzed',
        format: detection.format,
        rows: datasets.reduce((sum, dataset) => sum + dataset.rowCount, 0),
        committed,
  ...
  const final: UniversalFolderProgress = { processed: files.length, total: files.length, current: '', results, startedAt, updatedAt: new Date().toISOString() };
  onProgress?.(final);
  return final;
}
