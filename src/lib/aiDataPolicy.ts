export interface AIDataPolicyInput { capability: 'chat' | 'embedding' | 'document' | 'ocr'; tenantId: string; text?: string; includeRawBusinessRows?: boolean; trustedProvider?: boolean; }
export interface AIDataPolicyDecision { allowed: boolean; reason: string; payloadMode: 'none' | 'redacted-context' | 'approved-context'; }

const hasTenantScope = (tenantId: unknown): tenantId is string => typeof tenantId === 'string' && tenantId.trim().length > 0;

/** Hosted AI is not allowed to receive arbitrary database rows. Callers must first construct a minimal, approved context or use deterministic tools. */
export function evaluateAIDataPolicy(input: AIDataPolicyInput): AIDataPolicyDecision {
  if (!hasTenantScope(input.tenantId)) return { allowed: false, reason: 'Tenant scope is required', payloadMode: 'none' };
