...
export function buildDecisionPortfolio(candidates: PortfolioCandidate[], maxRisk: number): RuntimeDecision[] {
  return rankPortfolio(candidates, maxRisk).map((item) => ({
    ...
    risk: item.riskConsumption,
    evidenceKeys: item.evidenceKeys,
  }));
}

export function chooseBoundedScenario(options: ScenarioOption[], budget: RiskBudget) {
  return selectBoundedScenario(options, budget);
}

export function autonomyDecision(input: Parameters<typeof evaluateAutonomyGate>[0]) {
...
export function assessReleaseReadiness(evidence: LifecycleEvidence[], blockers: string[] = []): ReleaseReadiness {
  const completedPhases = LIFECYCLE_PHASES.filter((phase) => {
    const items = evidence.filter((item) => item.phase === phase);
    return items.length > 0 && items.every((item) => item.passed);
  });
  const blockedPhases = LIFECYCLE_PHASES.filter((phase) => {
    const items = evidence.filter((item) => item.phase === phase);
    return items.some((item) => !item.passed);
  });
  ...
  const allPhasesCertified = completedPhases.length === LIFECYCLE_PHASES.length;
  return { ready: allPhasesCertified && blockers.length === 0, completedPhases, blockedPhases, evidenceScore, blockers: [...blockers] };
}
