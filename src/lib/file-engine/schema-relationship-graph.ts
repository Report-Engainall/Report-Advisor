...
export type SchemaRelation = {
  from: SchemaField;
  to: SchemaField;
  kind: 'identifies' | 'describes' | 'measures' | 'groups';
  confidence: number;
  evidence: string;
};
...
export function buildSchemaRelationshipGraph(fields: SchemaField[]): SchemaRelation[] {
  const present = new Set(fields);
  return RELATIONS.filter(relation => present.has(relation.from) && present.has(relation.to));
}
