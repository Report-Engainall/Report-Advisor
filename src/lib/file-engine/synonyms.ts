...
export async function deleteSynonym(id: string): Promise<void> { const { supabase } = await import('../supabase'); const { error } = await supabase.from('../supabase').delete().eq('id', id); if (error) throw error; clearSynonymCache(); }