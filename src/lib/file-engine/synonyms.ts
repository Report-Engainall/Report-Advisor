...
const BUILTIN_SYNONYMS: Array<[string, string, number]> = [
  ...
  ['code', 'code', 98], ['customer code', 'code', 98], ['customer name', 'customer_name', 98], ['customer code', 'code', 98], ['segment', 'segment', 98], ['credit_limit', 'credit_limit', 98], ['credit limit', 'credit_limit', 98], ['payment_terms_days', 'payment_terms_days', 98], ['payment terms days', 'payment_terms_days', 98],
  ['email', 'email', 96], ['البريد الإلكتروني', 'email', 98], ['date', 'date', 94], ['التاريخ', 'date', 96], ['total', 'total', 94], ['الإجمالي', 'total', 96],
  ['invoice_number', 'invoice_number', 98], ['invoice number', 'invoice_number', 98], ['invoice_date', 'invoice_date', 98], ['invoice date', 'invoice_date', 98], ['customer_id', 'customer_id', 98], ['subtotal', 'subtotal', 98], ['tax_amount', 'tax_amount', 98], ['tax amount', 'tax_amount', 98], ['paid_amount', 'paid_amount', 98], ['paid amount', 'paid_amount', 98],
];
...
export async function loadSynonyms(): Promise<Map<string, { canonical: string; confidence: number }>> {
  ...
  // File parsing is also executed in Node-based deterministic regression tests.
  // Browser production keeps the authoritative database-backed synonym lookup;
  // non-browser execution intentionally stays pure and requires no Vite env.
  if (typeof window === 'undefined') {
    synonymCache = map;
    return map;
  }
  const { supabase } = await import('../supabase');
  const { data, error } = await supabase.from('synonym_dictionary').select('*').eq('is_active', true);
  if (!error && data) for (const entry of data as SynonymEntry[]) {
    const key = normalizeColumnName(entry.synonym);
    const existing = map.get(key);
    if (!existing || entry.confidence > existing.confidence) map.set(key, { canonical: entry.canonical_field, confidence: entry.confidence });
  }
  synonymCache = map;
  return map;
}
...
export async function addSynonym(canonicalField: string, synonym: string, language = 'ar', confidence = 90): Promise<void> { const { supabase } = await import('../supabase'); const { error } = await supabase.from('synonym_dictionary').insert({ canonical_field: canonicalField, synonym, language, confidence, is_active: true }); if (error) throw error; clearSynonymCache(); }
export async function fetchAllSynonyms(): Promise<SynonymEntry[]> { const { supabase } = await import('../supabase'); const { data, error } = await supabase.from('synonym_dictionary').select('*').order('canonical_field', { ascending: true }); if (error) throw error; return data as SynonymEntry[]; }
export async function updateSynonym(id: string, updates: Partial<SynonymEntry>): Promise<void> { const { supabase } = await import('../supabase'); const { error } = await supabase.from('synonym_dictionary').update(updates).eq('id', id); if (error) throw error; clearSynonymCache(); }
export async function deleteSynonym(id: string): Promise<void> { const { supabase } = await import('../supabase'); const { error } = await supabase.from('synonym_dictionary').delete().eq('id', id); if (error) throw error; clearSynonymCache(); }