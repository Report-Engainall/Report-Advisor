...
/** Safe built-in vocabulary keeps schema discovery useful when the dictionary is empty/offline. */
const BUILTIN_SYNONYMS: Array<[string, string, number]> = [
  ...
  ['name', 'name', 96],
  ['item name', 'name', 98],
  ['product name', 'name', 98],
  ['اسم الصنف', 'name', 98],
  ['اسم المنتج', 'name', 98],
  ['description', 'description', 94],
  ['الوصف', 'description', 96],
  ['price', 'price', 96],
  ['sale price', 'price', 96],
  ['selling price', 'price', 96],
  ['السعر', 'price', 98],
  ['سعر البيع', 'price', 98],
  ['cost', 'cost', 94],
  ['التكلفة', 'cost', 96],
  ['quantity', 'quantity', 96],
  ['qty', 'quantity', 96],
  ['stock', 'quantity', 94],
  ['available quantity', 'quantity', 96],
  ['الكمية', 'quantity', 98],
  ['المخزون', 'quantity', 96],
  ['unit', 'unit', 96],
  ['الوحدة', 'unit', 98],
  ...
  ['status', 'status', 94],
  ['الحالة', 'status', 96],
  ['customer number', 'customer_number', 98],
  ['customer id', 'customer_number', 96],
  ['رقم العميل', 'customer_number', 98],
  ['customer name', 'customer_name', 98],
  ['اسم العميل', 'customer_name', 98],
  ['phone', 'phone', 96],
  ['mobile', 'phone', 96],
  ['هاتف', 'phone', 96],
  ['جوال', 'phone', 96],
  ['email', 'email', 96],
  ['البريد الإلكتروني', 'email', 98],
  ['date', 'date', 94],
  ['التاريخ', 'date', 96],
  ['total', 'total', 94],
  ['الإجمالي', 'total', 96],
...
function createBuiltinMap(): Map<string, { canonical: string; confidence: number }> {
  const map = new Map<string, { canonical: string; confidence: number }>();
  for (const [synonym, canonical, confidence] of BUILTIN_SYNONYMS) {
    map.set(normalizeColumnName(synonym), { canonical, confidence });
  }
  return map;
}
