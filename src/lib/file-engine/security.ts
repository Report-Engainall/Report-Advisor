...
export function securityScan(file: File, buffer: ArrayBuffer): SecurityScanResult {
  ...
  if (isZip) {
    ...
    for (let i = 0; i < fileNameBytes.length - 2; i++) {
      if (fileNameBytes[i] === 0x2E && fileNameBytes[i + 1] === 0x2E && fileNameBytes[i + 2] === 0x2F) { isZipTraversal = true; issues.push('تحذير: اسم الملف يحتوي على مسار انتقالي (path traversal)'); break; }
    }
  }
...
/**
 * Duplicate detection is tenant-native: RLS/current_company_id() is the authority.
 * The optional legacy arguments remain source-compatible for one pending UI migration,
 * but are intentionally ignored and never participate in authorization or filtering.
 */
export async function checkDuplicate(hash: string, _legacyCompanyId?: string, _legacyClient?: typeof supabase): Promise<{ isDuplicate: boolean; existing: Record<string, unknown> | null }> {
  const { data, error } = await supabase.from('file_records').select('*').eq('file_hash', hash).order('created_at', { ascending: false }).limit(1).maybeSingle();
