...
export type ReconciliationRow<T> = {
  key: string;
...
export function reconcileByBusinessKey<T>(
  sourceRows: T[],
  targetRows: T[],
  getKey: (row: T) => unknown,
  compareFields: FieldComparator<T>,
  ...
  const summary = rows.reduce<ReconciliationSummary>((acc, row) => {
    ...
    acc[row.status] += 1;
    return acc;
  }, { total: 0, new: 0, updated: 0, unchanged: 0, conflict: 0, error: 0 });
...
export function stableImportFingerprint<T>(rows: T[], getKey: (row: T) => unknown, serialize: (row: T) => string): string {
  const canonical = rows
    .map(row => `${normalizeBusinessKey(getKey(row))}|${serialize(row)}`)
    .sort()
    .join('\n');
  let hash = 2166136261;
