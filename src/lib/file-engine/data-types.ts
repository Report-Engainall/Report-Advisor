...
import { normalizeArabicDigits, isSKU, isPhone, isEmail, parseNumber, parseDate, parseCurrency } from './normalizer';
...
export function detectDataType(values: any[]): DataType {
  ...
  for (const v of sample) {
    ...
    if (typeof v === 'boolean' || /^(true|false|نعم|لا|true|false|صح|خطأ)$/i.test(str)) boolCount++;
  ...
  if (uniqueCount <= Math.min(20, sample.length * 0.3) && uniqueCount > 1) return 'category';

  return 'text';
}

export function detectColumnDataType(values: any[], columnName: string): DataType {
  const normalized = normalizeArabicDigits(columnName).toLowerCase().trim();

  if (/sku|كود|رمز|رقم الصنف|barcode|باركود|item code|product code|product id/i.test(normalized)) {
    if (values.some(v => isSKU(String(v)))) return 'sku';
  }
  if (/phone|هاتف|جوال|tel|mobile|رقم/i.test(normalized)) {
    if (values.some(v => isPhone(String(v)))) return 'phone';
  }
  ...
  if (/qty|كمية|عدد|quantity|count/i.test(normalized)) return 'integer';
  if (/unit|وحدة/i.test(normalized)) return 'unit';
  if (/status|حالة|segment|شريحة|category|فئة|تصنيف/i.test(normalized)) return 'category';
...
export function cleanValue(value: any, dataType: DataType): any {
  ...
  switch (dataType) {
    ...
    case 'currency':
    case 'percentage': {
      const n = parseCurrency(value) ?? parseNumber(value);
      return n;
    }
    case 'date': {
