...
export interface CanonicalIntelligenceInput { sales: IntelligenceInvoice[]; purchases: IntelligencePurchase[]; inventory: IntelligenceInventoryRow[]; salesHistory: number[]; costOfSales?: number; alternativeGroups?: AlternativeGroupInput[]; receivablePriorities?: Parameters<typeof prioritizeReceivables>[0]; supplierPaymentPriorities?: Parameters<typeof prioritizeSupplierPayments>[0]; openingLiquidity?: number; dailyInflow?: number; dailyOutflow?: number; committedOutflow?: number; periodDays?: number; }
export interface CanonicalIntelligence { metrics: MetricEvaluation[]; trend: ReturnType<typeof analyzeTrend>; forecast: ReturnType<typeof forecastSeries>; backtest: ReturnType<typeof backtestForecast>; cashConversionCycle: CashConversionCycle; liquidity: LiquidityProjection[]; reserveProtection: ReserveProtection; collections: ReceivablePriority[]; supplierPayments: PaymentPriority[]; replenishment: InventoryDecision[]; stochasticInventory: StochasticInventoryDecision[]; alternativeGroups: AlternativeGroupDecision[]; confidence: number; warnings: string[]; }
const finite = (value: unknown) => typeof value === 'number' && Number.isFinite(value) ? value : 0;
const nonNegative = (value: unknown) => Math.max(0, finite(value));
const average = (values: number[]) => values.length ? values.reduce((a, b) => a + b, 0) / values.length : 0;
const sanitizeHistory = (values: number[]) => values.filter(Number.isFinite).map(value => Math.max(0, value));
const outstanding = (total: number, paidAmount: number) => Math.max(0, nonNegative(total) - nonNegative(paidAmount));
export function buildCanonicalIntelligence(input: CanonicalIntelligenceInput): CanonicalIntelligence {
  const sales = input.sales.filter(row => Number.isFinite(row.total)); const purchases = input.purchases.filter(row => Number.isFinite(row.total)); const inventory = input.inventory.filter(row => Number.isFinite(row.stock) && Number.isFinite(row.unitCost));
  const salesHistory = sanitizeHistory(input.salesHistory);
  const salesTotal = sales.reduce((sum, row) => sum + nonNegative(row.total), 0); const purchaseTotal = purchases.reduce((sum, row) => sum + nonNegative(row.total), 0);
  const receivables = sales.reduce((sum, row) => sum + outstanding(row.total, row.paidAmount), 0); const payables = purchases.reduce((sum, row) => sum + outstanding(row.total, row.paidAmount), 0);
  ...
  const reserveProtection = protectCashReserve({ openingCash: input.openingLiquidity === undefined ? 0 : input.openingLiquidity, committedOutflow: input.committedOutflow === undefined ? 0 : input.committedOutflow, collectibleInflow: input.dailyInflow === undefined ? 0 : input.dailyInflow });
  ...
  const supplierPayments = prioritizeSupplierPayments(input.supplierPaymentPriorities ?? purchases.map((row, index) => ({ id: `purchase-${index}`, amount: outstanding(row.total, row.paidAmount), overdueDays: 0 })), reserveProtection);
  const replenishment = inventory.map(row => decideReplenishment({ onHand: nonNegative(row.stock), avgDailyDemand: average((row.dailySales ?? []).filter(Number.isFinite).map(value => Math.max(0, value))), leadTimeDays: nonNegative(row.leadTimeDays ?? 7), safetyDays: 7, reserved: 0, onOrder: 0 }));
  const stochasticInventory = inventory.map(row => calculateInventoryDecision({ sku: row.sku, stock: nonNegative(row.stock), dailySales: (row.dailySales ?? []).filter(Number.isFinite).map(value => Math.max(0, value)), leadTimeDays: nonNegative(row.leadTimeDays ?? 7), safetyDays: 2, reviewPeriodDays: 7, unitCost: nonNegative(row.unitCost) }));
  ...
  const metrics = [
    ...
    evaluateMetric({ key: 'inventory_velocity', value: average(salesHistory), sourceRows: salesHistory.length, confidence: salesHistory.length >= 3 ? 0.9 : 0 }),
    evaluateMetric({ key: 'stock_coverage', value: inventory.length && salesHistory.length ? inventory.reduce((sum, row) => sum + nonNegative(row.stock), 0) / Math.max(average(salesHistory), 0.000001) : null, sourceRows: inventory.length && salesHistory.length ? inventory.length + salesHistory.length : 0, confidence: salesHistory.length >= 7 ? forecast.confidence : 0, status: 'FORECAST' }),
  ];
