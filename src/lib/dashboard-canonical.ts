...
export async function fetchInventoryReportSnapshot(page = 0, pageSize = 25, filter: 'all'|'low'|'out' = 'all'): Promise<InventoryReportSnapshot> {
  ...
  const row = data as Record<string, unknown>;
  return {
    rows: requiredArray<InventoryReportRow>(row.rows), page: Number(row.page ?? page), pageSize: Number(row.pageSize ?? pageSize),
    filter: row.filter === 'low' || row.filter === 'out' ? row.filter : 'all', totalRows: Number(row.totalRows ?? 0), filteredRows: Number(row.filteredRows ?? 0),
    lowStock: Number(row.lowStock ?? 0), outOfStock: Number(row.outOfStock ?? 0), unknownRows: Number(row.unknownRows ?? 0),
    totalValue: finiteOrNull(row.totalValue), dataStatus: row.dataStatus === 'INSUFFICIENT_DATA' ? 'INSUFFICIENT_DATA' : row.dataStatus === 'NO_DATA' ? 'NO_DATA' : 'CALCULATED'
  };
}
