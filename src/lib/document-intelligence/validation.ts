...
export type ConfidenceInput = { header: number; content: number; pattern: number; relationship: number; math: number; context: number };

function finiteScore(value: number): number { return Number.isFinite(value) ? Math.max(0, Math.min(1, value)) : 0; }
...
export function combineEvidence(input: ConfidenceInput): number {
  ...
  return finiteScore(finiteScore(input.header) * weights.header + finiteScore(input.content) * weights.content + finiteScore(input.pattern) * weights.pattern + finiteScore(input.relationship) * weights.relationship + finiteScore(input.math) * weights.math + finiteScore(input.context) * weights.context);
}
...
export function reconcileNumbers(actual: number, expected: number, absoluteTolerance = 0.01, relativeTolerance = 0.0001): ReconciliationResult {
  if (!Number.isFinite(actual) || !Number.isFinite(expected)) return { passed: false, difference: Number.NaN, tolerance: Number.NaN, issues: [{ code: 'RECONCILIATION_INPUT_INVALID', status: 'UNKNOWN', message: 'Actual and expected values must both be finite numbers before reconciliation.', evidence: { actual, expected } }] };
  ...
  return { passed, difference, tolerance, issues: passed ? [] : [{ code: 'RECONCILIATION_MISMATCH', status: 'FAIL', message: 'Actual and expected values exceed the configured tolerance.', evidence: { actual, expected, difference, tolerance } }] };
}

function missingValidation(field: string, required: string[]): ValidationIssue[] { return [{ code: 'VALIDATION_INPUT_INCOMPLETE', status: 'UNKNOWN', message: `Required validation inputs are missing or non-numeric: ${required.join(', ')}.`, field, criticality: 'HIGH' }]; }
...
export function validateLineMath(row: Record<string, unknown>): ValidationIssue[] {
  const q = strictNumber(row.quantity);
...
export function validateInvoiceTotals(row: Record<string, unknown>): ValidationIssue[] {
  const subtotal = strictNumber(row.subtotal);
  const tax = row.tax == null ? 0 : strictNumber(row.tax);
  const discount = row.discount == null ? 0 : strictNumber(row.discount);
