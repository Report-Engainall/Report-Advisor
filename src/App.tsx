...
const RecommendationsPage = lazy(() => import('@/pages/IntelligencePages').then(m => ({ default: m.RecommendationsPage })));
const ForecastsPage = lazy(() => import('@/pages/IntelligencePages').then(m => ({ default: m.ForecastsPage })));
...
function AppShell() { const { language, direction } = useLanguage(); const navigate = useNavigate(); const landingAppliedRef = useRef(false); const [alerts, setAlerts] = useState<Alert[]>([]); const [sidebarOpen, setSidebarOpen] = useState(false); const [commandOpen, setCommandOpen] = useState(false); const [advisorOpen, setAdvisorOpen] = useState(false); const [user, setUser] = useState<User | null>(null); const location = useLocation(); const loadAlerts = useCallback(async () => { try { const intelligence = await fetchDashboardIntelligence(); setAlerts(intelligence.alerts); } catch (error) { console.error('[AppShell] Failed to load alerts', error); setAlerts([]); } }, []); useEffect(() => { void getAuthenticatedUser().then(setUser); void loadAlerts(); }, [loadAlerts]); useEffect(() => { setSidebarOpen(false); }, [location.pathname]); useEffect(() => { if (landingAppliedRef.current) return; landingAppliedRef.current = true; const preferences = readWorkspacePreferences(); if (location.pathname === '/' && preferences.defaultLandingPath !== '/' && isWorkspacePathVisible(preferences.defaultLandingPath, readWorkspaceMode(), preferences)) navigate(preferences.defaultLandingPath, { replace: true }); }, [location.pathname, navigate]); useEffect(() => { const onKeyDown = (event: KeyboardEvent) => { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'k') { event.preventDefault(); setCommandOpen(value => !value); } }; window.addEventListener('keydown', onKeyDown); return () => window.removeEventListener('keydown', onKeyDown); }, []); const handleMarkAlert = useCallback(async (id: string) => { try { await markAlertRead(id); setAlerts(prev => prev.map(a => a.id === id ? { ...a, is_read: true } : a)); notifyFeedback({ tone: 'success', title: 'تم تحديث التنبيه', message: 'أزيل من قائمة الانتباه.' }); } catch (error) { notifyFeedback({ tone: 'danger', title: 'تعذر تحديث التنبيه', message: 'حاول مرة أخرى.' }); console.error('[AppShell] Failed to mark alert as read', error); } }, []); const alertCount = alerts.filter(a => !a.is_read).length; return <div dir={direction} className="ag-app-shell flex min-h-screen flex-row bg-transparent"><Suspense fallback={<div className="hidden w-[238px] shrink-0 bg-[#0d1510] lg:block" aria-hidden="true" />}><div className="hidden lg:block"><Sidebar alertCount={alertCount} user={user} /></div></Suspense>{sidebarOpen && <div className="lg:hidden fixed inset-0 z-50 flex"><div className="fixed inset-0 bg-ink-900/50 backdrop-blur-sm" onClick={() => setSidebarOpen(false)} /><div className={"relative h-full animate-slide-in " + (language === "ar" ? "ml-auto" : "mr-auto")}><Suspense fallback={<div className="relative h-full w-[296px] bg-[#0d1510]" aria-hidden="true" />}><Sidebar alertCount={alertCount} user={user} onNavigate={() => setSidebarOpen(false)} /></Suspense><button onClick={() => setSidebarOpen(false)} className="absolute top-4 left-4 text-ink-400 hover:text-ink-600" aria-label="إغلاق القائمة"><X size={20} /></button></div></div>}<div className="flex-1 flex flex-col min-w-0"><FeedbackHost /><Header alerts={alerts} onMarkAlertRead={handleMarkAlert} onMenuClick={() => setSidebarOpen(true)} onOpenCommandPalette={() => setCommandOpen(true)} /><button type="button" onClick={() => setAdvisorOpen(value => !value)} className="fixed bottom-5 left-5 z-50 hidden items-center gap-2 rounded-full border border-primary-300 bg-ink-950 px-4 py-3 text-xs font-black text-white shadow-elevated lg:inline-flex" aria-expanded={advisorOpen} aria-controls="ag-global-advisor"><span className="flex h-6 w-6 items-center justify-center rounded-full bg-primary-500">✦</span>المستشار الأغبري</button>{advisorOpen && <aside id="ag-global-advisor" className="fixed bottom-20 left-5 z-50 w-[min(430px,calc(100vw-2rem))] max-h-[78vh] overflow-y-auto rounded-3xl border border-ink-200 bg-white p-3 shadow-elevated" aria-label="المستشار الأغبري"><DeterministicIntelligenceAssistant recommendationsCount={0} activeAlertsCount={alerts.filter(a => !a.is_read).length} forecastsCount={0} /></aside>}<main className="ag-main flex-1 min-w-0 w-full max-w-[1600px] p-3 pb-24 sm:p-4 sm:pb-24 lg:p-5 lg:pb-5 2xl:p-6 2xl:pb-6 mx-auto"><ProductJourneyNav/><Suspense fallback={<div className="flex items-center justify-center py-20" role="status" aria-live="polite"><div className="animate-spin rounded-full h-8 w-8 border-2 border-primary-500 border-t-transparent" /><span className="sr-only">جارٍ تحميل الصفحة</span></div>}><Routes><Route path="/" element={<DashboardPage />} /><Route path="/onboarding" element={<OnboardingPage />} /><Route path="/work-center" element={<WorkCenterPage />} />
      <Route path="/connections" element={<ConnectionsPage />} /><Route path="/command-center" element={<ExecutiveCommandCenterPage />} /><Route path="/decision-experience" element={<DecisionExperiencePage />} /><Route path="/metrics" element={<MetricInspectorPage />} /><Route path="/proposal-demo" element={<ProposalDemoPage />} /><Route path="/reports" element={<ReportsCenterPage />} /><Route path="/reports/executive" element={<ExecutiveReportPage />} /><Route path="/reports/sales" element={<SalesReportPage />} /><Route path="/reports/purchases" element={<PurchasesReportPage />} /><Route path="/reports/inventory" element={<InventoryReportPage />} /><Route path="/reports/inventory-intelligence" element={<InventoryIntelligencePage />} /><Route path="/reports/demand-velocity" element={<DemandVelocityPage />} /><Route path="/reports/receivables" element={<ReceivablesReportPage />} /><Route path="/reports/profitability" element={<ProfitabilityReportPage />} /><Route path="/import" element={<ImportPage />} /><Route path="/import/analyze" element={<ExternalFileAnalysisPage />} /><Route path="/data-quality" element={<DataQualityPage />} /><Route path="/analytics" element={<AnalyticsCenterPage />} /><Route path="/analytics/rfm" element={<RFMAnalysisPage />} /><Route path="/analytics/abc" element={<ABCAnalysisPage />} /><Route path="/analytics/aging" element={<AgingAnalysisPage />} /><Route path="/intelligence" element={<IntelligenceCenterPage />} /><Route path="/intelligence/recommendations" element={<RecommendationsPage />} /><Route path="/intelligence/forecasts" element={<ForecastsPage />} /><Route path="/intelligence/scenarios" element={<ScenarioTruthGuardPage />} /><Route path="/customers" element={<CustomersPage />} /><Route path="/products" element={<ProductsPage />} /><Route path="/inventory" element={<InventoryPage />} /><Route path="/alternative-groups" element={<AlternativeGroupsPage />} /><Route path="/settings" element={<CompanySettingsPage />} /><Route path="/settings/profile" element={<ProfileSettingsPage />} /><Route path="*" element={<NotFoundPage />} /></Routes></Suspense></main></div><MobileActionBar onOpenCommandPalette={() => setCommandOpen(true)} /><CommandPalette open={commandOpen} onClose={() => setCommandOpen(false)} /></div>; }
