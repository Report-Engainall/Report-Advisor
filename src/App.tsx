    ...
    return (
      <div dir="rtl" className="min-h-screen bg-ink-50 flex items-center justify-center p-6">
        <div className="w-full max-w-lg rounded-2xl border border-ink-100 bg-white p-8 shadow-sm text-center">
          <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-red-50 text-red-600"><AlertTriangle size={24} /></div>
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-3 justify-center">
            <button type="button" onClick={this.handleRetry} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-primary-700"><RefreshCw size={16} /> المحاولة مرة أخرى</button>
            <Link to="/" className="inline-flex items-center justify-center gap-2 rounded-xl border border-ink-200 px-4 py-2.5 text-sm font-medium text-ink-700 hover:bg-ink-50"><Home size={16} /> لوحة القيادة</Link>
          </div>
        </div>
      </div>
    );
...
function NotFoundPage() {
  return <div dir="rtl" className="min-h-[60vh] flex items-center justify-center p-6"><div className="w-full max-w-lg rounded-2xl border border-ink-100 bg-white p-8 shadow-sm text-center"><div className="text-5xl font-black text-primary-600">404</div><h1 className="mt-3 text-xl font-bold text-ink-900">الصفحة غير موجودة</h1><p className="mt-2 text-sm leading-6 text-ink-500">الرابط الذي طلبته غير موجود أو تم نقله.</p><Link to="/" className="mt-6 inline-flex items-center justify-center gap-2 rounded-xl bg-primary-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-primary-700">العودة للرئيسية</Link></div></div>;
}
...
function AppShell() {
  ...
  const loadAlerts = useCallback(async () => {
    ...
    catch (error) { console.error('[AppShell] Failed to load alerts', error); setAlerts([]); }
  }, []);
  ...
  const handleMarkAlert = useCallback(async (id: string) => {
    try { await markAlertRead(id); setAlerts(prev => prev.map(a => a.id === id ? { ...a, is_read: true } : a)); }
    catch (error) { console.error('[AppShell] Failed to mark alert as read', error); }
  }, []);
  ...
  return (
    <div className="flex min-h-screen bg-ink-50">
      <div className="hidden lg:block"><Sidebar alertCount={alertCount} user={user} /></div>
      {sidebarOpen && <div className="lg:hidden fixed inset-0 z-50 flex"><div className="fixed inset-0 bg-ink-900/50 backdrop-blur-sm" onClick={() => setSidebarOpen(false)} /><div className="relative animate-slide-in"><Sidebar alertCount={alertCount} user={user} onNavigate={() => setSidebarOpen(false)} /><button onClick={() => setSidebarOpen(false)} className="absolute top-4 left-4 text-ink-400 hover:text-ink-600" aria-label="إغلاق القائمة"><X size={20} /></button></div></div>}
      <div className="flex-1 flex flex-col min-w-0">
        <Header alerts={alerts} onMarkAlertRead={handleMarkAlert} onMenuClick={() => setSidebarOpen(true)} onOpenCommandPalette={() => setCommandOpen(true)} />
        <main className="flex-1 p-4 lg:p-6 max-w-[1600px] w-full mx-auto">
          <Suspense fallback={<div className="flex items-center justify-center py-20" role="status" aria-live="polite"><div className="animate-spin rounded-full h-8 w-8 border-2 border-primary-500 border-t-transparent" /><span className="sr-only">جارٍ تحميل الصفحة</span></div>}>
            <Routes>
              ...
              <Route path="/customers" element={<CustomersPage />} /><Route path="/products" element={<ProductsPage />} /><Route path="/inventory" element={<InventoryPage />} /><Route path="/alternative-groups" element={<AlternativeGroupsPage />} /><Route path="/settings" element={<CompanySettingsPage />} /><Route path="/settings/profile" element={<ProfileSettingsPage />} />
...
export default function App() {
  return <BrowserRouter><AppErrorBoundary><AuthGate><AppShell /></AuthGate></AppErrorBoundary></BrowserRouter>;
}