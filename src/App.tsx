...
function AppShell() {
  ...
  const handleMarkAlert = useCallback(async (id: string) => {
    try {
      ...
      notifyFeedback({ tone: 'success', title: 'تم تحديث التنبيه', message: 'أزيل من قائمة الانتباه.' });
    } catch (error) {
      notifyFeedback({ tone: 'danger', title: 'تعذر تحديث التنبيه', message: 'حاول مرة أخرى.' });
  ...
  return (
    <div dir={direction} className={"flex min-h-screen bg-transparent " + (language === "ar" ? "flex-row-reverse" : "")}>
      <Suspense fallback={<div className="hidden w-[238px] shrink-0 bg-[#0d1510] lg:block" aria-hidden="true" />}>
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <div className={"relative h-full animate-slide-in " + (language === "ar" ? "ml-auto" : "mr-auto")}>
            <Suspense fallback={<div className="relative h-full w-[296px] bg-[#0d1510]" aria-hidden="true" />}>
              <Sidebar alertCount={alertCount} user={user} onNavigate={() => setSidebarOpen(false)} />
            </Suspense>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 min-w-0 w-full max-w-[1600px] p-3 pb-20 sm:p-4 sm:pb-20 lg:p-5 lg:pb-5 2xl:p-6 2xl:pb-6 mx-auto">
          <ProductJourneyNav/>
          <Suspense fallback={<div className="flex items-center justify-center py-20" role="status" aria-live="polite"><div className="animate-spin rounded-full h-8 w-8 border-2 border-primary-500 border-t-transparent" /><span className="sr-only">جارٍ تحميل الصفحة</span></div>}>
            <Routes>
              ...
              <Route path="/decision-experience" element={<DecisionExperiencePage />} />
              <Route path="/metrics" element={<MetricInspectorPage />} />
              <Route path="/proposal-demo" element={<ProposalDemoPage />} />
              ...
              <Route path="/import/analyze" element={<ExternalFileAnalysisPage />} />
              ...
              <Route path="/intelligence/scenarios" element={<ScenarioTruthGuardPage />} />
