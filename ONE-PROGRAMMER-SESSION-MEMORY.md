...
- CI STATUS: The governance-head Final Certification exposed two stale work-item migration references after watched-folder paths were fixed. Both contract scripts now pass locally, and a full scan of all script migration references reports zero missing filenames. Candidate `51d589a...` is ready for fresh Final Certification/Phase-F.
- FAILED / BLOCKED: The last live Phase-F result remains 1/4 on 88a27eb (`DEPLOYMENT_SHA_MISMATCH`, restore UUID/text failure, rollback-forward 503). Candidate 51d589a contains the subsequent certification-path fixes; fresh live evidence is required and production identity remains separate.
...
- CURRENT RESUME POINTER: code candidate 51d589a... → governance rebind → fresh Final Certification/Phase-F → first failure → production identity alignment → measured recovery.
