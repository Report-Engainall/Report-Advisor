...
- CI STATUS: Phase-F run 36069941030 on 88a27eb... returned 1/4: production deployment SHA mismatch (`7be9f014...`), canary PASS, restore replay advanced to later migration `20260831061000_dashboard_top_entities_truth.sql` and failed on the same UUID/text `COALESCE`; rollback-forward 503. Current candidate 501013ad... fixes both top-entity casts. Static contracts pass locally; fresh Phase-F is pending.
- FAILED / BLOCKED: Phase-F run 36069941030 on 88a27eb reported production deployment SHA 7be9f014... (candidate mismatch), restore replay failure in `20260831061000_dashboard_top_entities_truth.sql`, and rollback-forward HTTP 503; result 1/4. Candidate 501013ad fixes both casts in that later migration. Production identity remains a separate blocker.
...
- NEXT EXECUTABLE ACTION: commit governance rebind to candidate 501013ad...; verify exact GitHub HEAD; consume fresh Phase-F; fix only the first reproduced restore failure; if restore passes, promote/deploy exact candidate so `/api/health` reports the same SHA; then consume Enforcement/Final Certification.
...
- CURRENT RESUME POINTER: code candidate 501013ad... → governance rebind → fresh Phase-F → first restore failure → production identity alignment → exact-head Enforcement/Final Certification → measured recovery.
