...
- CURRENT CODE/TEST CANDIDATE: 02d849e6895481c6b4e369b9b092ca2490e1495b (PR #628 branch).
- DONE: corrected three stale migration-contract paths; added duplicate-FK replay guards; qualified tenant columns in historical decision work-item/action-receipt RLS policies; added DROP FUNCTION IF EXISTS before recreating import_update_job_progress; and restored the missing enforce_same_company_reference() helper before its warehouse/branch trigger. Pushed code candidate 02d849e6895481c6b4e369b9b092ca2490e1495b to PR #628.
...
- CI STATUS: Phase-F run 36068854348 on a332c3e... returned 1/4: deployment SHA mismatch (`7be9f014...`), canary PASS, restore replay failed because enforce_same_company_reference() was absent from its migration, rollback-forward 503. Current candidate 02d849e... adds that missing helper. Targeted local contracts pass; fresh Phase-F is pending.
- FAILED / BLOCKED: Phase-F run 36068854348 on a332c3e reported production deployment SHA 7be9f014... (candidate mismatch), restore replay failure at enforce_same_company_reference(), and rollback-forward HTTP 503; result 1/4. Candidate 02d849e adds the helper before trigger creation. Live replay remains NOT VERIFIED. Local Supabase CLI and Docker are unavailable.
...
- NEXT EXECUTABLE ACTION: update governance index/memory to candidate 02d849e...; verify exact GitHub HEAD; consume fresh Phase-F; if restore passes, resolve the production deployment SHA mismatch by promoting/deploying the exact candidate; then consume Enforcement/Final Certification.
...
- CURRENT RESUME POINTER: code candidate 02d849e... → governance rebind → fresh Phase-F → first reproduced restore failure or deployment identity issue → promote exact candidate → exact-head gates → measured recovery.
