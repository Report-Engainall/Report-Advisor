...
- CURRENT CODE/TEST CANDIDATE: a332c3e770477636f2a2898cecabc667f0acac5a (PR #628 branch).
- DONE: corrected three stale migration-contract paths; added duplicate-FK replay guards; qualified tenant columns in historical decision work-item/action-receipt RLS policies; and added DROP FUNCTION IF EXISTS before recreating import_update_job_progress to handle a restored legacy signature. Pushed code candidate a332c3e770477636f2a2898cecabc667f0acac5a to PR #628.
- LOCAL PROOF on code ancestor dfbe1fbd...: typecheck PASS; Vite production build PASS; ESLint 0 errors / 63 warnings; route/sidebar parity PASS (39 routes / 37 canonical links); Product WOW PASS; knowledge architecture PASS; execution enforcement contract PASS; Phase-F static closure PASS; production readiness PASS; storage isolation PASS; migration audit PASS (254 / 0 findings); diff check clean. Current candidate a226aba... additionally passes decision/intelligence closure, tenant security, migration audit, Phase-F static closure, decision approval TOCTOU, and diff check.
- CI STATUS: Phase-F run 36068478991 on a226aba... returned 1/4: deployment SHA mismatch (`7be9f014...`), canary PASS, restore replay failed in import progress RPC migration, rollback-forward 503. Candidate a332c3e... adds a guarded function drop before recreation and local import lifecycle/transaction, migration audit, and Phase-F static contracts pass. Phase-F run 36068854348 on a332c3e... has been queued and is currently in progress; exact-head CI gates remain to be consumed after governance rebind.
- FAILED / BLOCKED: Phase-F run 36068478991 on a226aba reported deployment SHA 7be9f014... (candidate mismatch), restore replay failure at import_update_job_progress migration, and rollback-forward HTTP 503; result 1/4. Candidate a332c3e adds a guarded DROP FUNCTION before recreate; live replay remains NOT VERIFIED. Local Supabase CLI and Docker are unavailable.
...
- NEXT EXECUTABLE ACTION: monitor Phase-F run 36068854348 on candidate a332c3e...; inspect the exact restore result; fix only the first reproduced issue; then obtain exact-head Enforcement/Final Certification on the current governance HEAD.
...
- CURRENT RESUME POINTER: code candidate a332c3e... → Phase-F run 36068854348 → first reproduced restore failure → exact-head gates → deployment SHA alignment → measured recovery.
