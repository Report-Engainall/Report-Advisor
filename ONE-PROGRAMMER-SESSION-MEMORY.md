...
- CURRENT CODE/TEST CANDIDATE: a226aba64d3ed367c0b10530a652b6afe87caa96 (PR #628 branch).
- DONE: corrected three stale migration-contract paths; added replay guards for the two duplicate import-lineage FKs; qualified tenant columns in the historical decision work-item/action-receipt RLS policies to match the later tenant-safe migration and remove ambiguous/unqualified predicates. Pushed code candidate a226aba64d3ed367c0b10530a652b6afe87caa96 to PR #628.
...
- CI STATUS: code candidate 733cfb... produced broad successes but Phase-F run 36068182084 returned 1/4: deployment SHA mismatch, tenant canary PASS, backup/restore failed while replaying the decision action-receipt policy SQL, and rollback-forward returned 503. Enforcement/Final Certification on 733cfb failed at candidate-boundary validation because the index then pointed to an older candidate. New code candidate a226aba... contains the policy qualification repair; fresh CI/Phase-F evidence is pending after index rebind.
...
- NEXT EXECUTABLE ACTION: commit governance-only index/memory rebind to candidate a226aba...; verify exact GitHub HEAD; consume fresh exact-head CI/Phase-F; repair only the first reproducible restore or deployment-identity failure.
...
- CURRENT RESUME POINTER: code candidate a226aba... → governance-only index/memory rebind → fresh exact-head gates → verify policy/FK replay → align production deployment SHA → measured backup/restore, RPO/RTO and rollback.
