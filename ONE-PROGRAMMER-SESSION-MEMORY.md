...
- CURRENT CODE/TEST CANDIDATE: 7fdbf61ac8d84afece7b9faddca5025045ba6549 (PR #628 branch).
- DONE: corrected three stale migration-contract paths; added duplicate-FK replay guards; qualified tenant columns in historical decision work-item/action-receipt RLS policies; added DROP FUNCTION IF EXISTS before recreating import_update_job_progress; and restored enforce_same_company_reference() as a valid zero-argument trigger function using TG_ARGV. Pushed code candidate 7fdbf61ac8d84afece7b9faddca5025045ba6549 to PR #628.
...
- CI STATUS: Phase-F run 36069252323 on 02d849e... returned 1/4: deployment SHA mismatch (`7be9f014...`), canary PASS, restore replay failed because the added trigger helper declared SQL parameters, which PostgreSQL rejects for trigger functions; rollback-forward 503. Current candidate 7fdbf61... uses zero SQL arguments + TG_ARGV. Staging DB verification confirms the canonical live helper already uses zero arguments, and the fixed helper/trigger compiled successfully in a rolled-back transaction.
- FAILED / BLOCKED: Phase-F run 36069252323 on 02d849e reported production deployment SHA 7be9f014... (candidate mismatch), restore replay failure because trigger functions cannot declare SQL parameters, and rollback-forward HTTP 503; result 1/4. Candidate 7fdbf61 applies the correct PostgreSQL trigger semantics. Live Phase-F on the fixed candidate is pending. Production identity remains the separate blocker.
...
- NEXT EXECUTABLE ACTION: commit governance rebind to candidate 7fdbf61...; verify exact GitHub HEAD; consume fresh Phase-F; if restore passes, promote/deploy the exact candidate to production so `/api/health` reports the same SHA; then consume Enforcement/Final Certification.
...
- CURRENT RESUME POINTER: code candidate 7fdbf61... → governance rebind → fresh Phase-F → resolve production deployment identity → exact-head Enforcement/Final Certification → measured recovery.
