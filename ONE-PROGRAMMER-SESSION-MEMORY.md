...
- CI STATUS: Phase-F run 36069668188 on 7fdbf61... returned 1/4: production deployment SHA mismatch (`7be9f014...`), canary PASS, restore replay failed on dashboard top-customer aggregation because `COALESCE(text, uuid)` was incompatible; rollback-forward 503. Current candidate 88a27eb... casts customer/product UUID labels to text. Static contracts pass locally; fresh Phase-F is pending.
- FAILED / BLOCKED: Phase-F run 36069668188 on 7fdbf61 reported production deployment SHA 7be9f014... (candidate mismatch), restore replay failure on dashboard entity UUID/text aggregation, and rollback-forward HTTP 503; result 1/4. Candidate 88a27eb applies the sibling casts in the same canonical dashboard aggregation migration. Production identity remains a separate blocker.
...
- NEXT EXECUTABLE ACTION: commit governance rebind to candidate 88a27eb...; verify exact GitHub HEAD; consume fresh Phase-F; fix only the first reproduced restore failure; if restore passes, promote/deploy exact candidate so `/api/health` reports the same SHA; then consume Enforcement/Final Certification.
...
- CURRENT RESUME POINTER: code candidate 88a27eb... → governance rebind → fresh Phase-F → first restore failure → production identity alignment → exact-head Enforcement/Final Certification → measured recovery.
