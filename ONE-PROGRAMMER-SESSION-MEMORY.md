...
- CI STATUS: On governance head `17d00f...`, Execution Enforcement passed. Final Certification failed only at `check-import-folder-foundation` because two certification scripts referenced stale migration filename `20260825110000_watched_report_folders.sql`. Both scripts now pass locally after correction. Candidate `6f9948c...` is the corrected head; fresh Final Certification/Phase-F is pending.
- FAILED / BLOCKED: The last live Phase-F result remains 1/4 on 88a27eb (`DEPLOYMENT_SHA_MISMATCH`, restore UUID/text failure, rollback-forward 503). Candidate 6f9948c fixes the next certification contract failure; fresh live evidence is required and production identity remains separate.
...
- CURRENT RESUME POINTER: code candidate 6f9948c... → governance rebind → fresh Final Certification/Phase-F → first failure → production identity alignment → measured recovery.
