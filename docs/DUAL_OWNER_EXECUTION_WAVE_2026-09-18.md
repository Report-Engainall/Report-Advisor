...
- Wave 2 UI execution (2026-09-18): `src/pages/IntelligencePages.tsx` now frames recommendations as source → evidence → human decision → outcome and adds semantic filter state; `src/pages/CanonicalScenarioPage.tsx` now labels scenario output as deterministic local simulation rather than forecast/operational truth; `src/pages/ExternalFileAnalysisPage.tsx` now exposes staged analysis progression and makes the file drop zone keyboard accessible; regression guard added at `scripts/check-aghbari-command-wave2-ui.mjs`. These are product-development changes only and are not certification evidence.
- Wave 2 follow-up (2026-09-18): Company Settings workspace modes now expose `aria-pressed`; Profile Settings uses the shared `LoadingState`. These remain UI-only and exact-head regression guarded.
- Wave 3 UI execution (2026-09-18): `SavedViewMenu.tsx` now uses unique popover IDs, dialog semantics, open-focus and Escape-return focus behavior, an accessible name field, and visible keyboard focus states; `scripts/check-aghbari-command-wave2-ui.mjs` now guards these semantics. No database/RPC/runner behavior changed.
- Wave 4 UI execution (2026-09-18): `Header.tsx` and `App.tsx` now expose mobile menu state and alert popover relationships, semantic dialog ownership, and visible keyboard focus states for the mobile command shell. No data-path, RPC, runner, or persistence behavior changed.
- Wave 5 UI execution (2026-09-18): `CommandPalette.tsx` now exposes true combobox/listbox/option semantics with an active-result relationship and visible keyboard focus states. Exact-head PC01 verification passed: UI contract PASS, TypeScript typecheck PASS, production build PASS, performance budget PASS.
- Wave 6 UI execution (2026-09-18): `Sidebar.tsx` now exposes explicit expandable-section button/panel relationships and visible keyboard focus; the mobile navigation dialog is explicitly modal. Exact-head PC01 verification passed again: UI contract PASS, TypeScript typecheck PASS, production build PASS, performance budget PASS.
- `40765f57518f46b7e0d05d5c05a158b7c01d367a` produced a real Netlify build failure (exit code 2). The failure was isolated to the newly added saved-view syntax surface; `f3fc3b6306a8e1b3753b0ccd772a99ad5ae0fc19` removed escaped template syntax. The affected historical head is retained only as failure provenance; exact-head build/runtime proof remains required.
...
- Latest product-code commit issued by this accessibility wave before integration was `6bca1a93644c5b43a6ba2f780fd1e6c69fe1111d`; the integrated exact head must be re-proven after this merge.
