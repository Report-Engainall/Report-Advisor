...
- Latest UI closure wave (2026-09-18): `src/pages/DataQualitySnapshotPage.tsx` now keeps `EMPTY` quality explicitly unscored (`—`) instead of collapsing no-data to `0%`; `src/pages/AlternativeGroupsPage.tsx` now frames alternative groups as a tenant-scoped business-analysis lens with accessible controls and explicit decision context; `scripts/check-aghbari-command-ui-closure.mjs` guards these invariants. This is product-development evidence, not certification evidence.
