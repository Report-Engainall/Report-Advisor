...
Current branch: `wave-final-exact-ci-9`.
...
8. File-security regression alias coupling → pure file identity core.
9. Workflow integrity failure → `batch-integrity-guards.yml` concurrency now includes `${{ github.workflow }}`.
10. Quality-chain failure → restored `npm run test:document-resilience` in the quality Full resilience gate.

## Exact CI evidence
- Historical PASS: Run `32910806786` on `25eef5212dbc63d2255ad7998e76c3d02a5191cf`.
- Run `32919127516` on `f8d1c024ea9fc59b13f51d9edf2488e131ba6f9c` = QUALITY PASS, but two independent workflow guards exposed real workflow-contract issues now fixed: batch concurrency scope and missing document-resilience command in quality.
- Current candidate: `24f7b2d5782ddaad981e2979bd310496fe5be734` plus this index update.
- Fresh exact-head CI for the final candidate is PENDING.
