...
- The production-gate runtime Windows fix is part of the current remediation lineage; the previous `C:\Program` process-launch defect is closed. The current candidate also includes the source-preservation contract rebind and the worker-boundary contract rebind to the authenticated Supabase helper.
- Staging schema-drift reconciliation `20260918060000_reconcile_import_job_row_tenant_schema.sql` is bound to the exact predecessor code candidate and verified live on Staging (migration `20260918053906`, compound tenant FK, required indexes, zero null tenant rows). No earlier runtime/browser/production evidence is promoted by this binding.
