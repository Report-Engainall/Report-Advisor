...
- Exact active remediation HEAD before this documentation commit: `fc8512835e71e723f902f7e0169ef0c56d54ea95`
...
| Migration/schema provenance parity | 10% | ~70% | 7.0 |
...
The forward-only migration `20260906202000_harden_worker_checkpoint_admission` was applied to Staging and its canonical function definition was re-read successfully. This closes the previously identified source-vs-live checkpoint-admission gap.
...
The live Staging ledger contains the worker/reconciliation migration chain, including execution-timestamped applications. Historical migration records were not rewritten.
...
Canonical truth boundary, provenance requirements, reconciliation states, and import commit path exist. Full adversarial business-corpus runtime proof remains open.
...
RLS performance work has materially improved the policy shape and reduced prior advisor noise. Remaining work includes production-scale measurements, failure-injection observability, SECURITY DEFINER least-privilege review, and final UX/business acceptance.

## CI / RELEASE TRUTH

- Vercel preview deployments have reached READY; this is deployment/build evidence only and does not certify production.
- GitHub Actions have repeatedly produced `failure` records whose jobs expose `steps=null`; these records do not prove test-step execution.
