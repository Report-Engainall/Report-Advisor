...
### CURRENT HEAD
- Branch: `main`
- **Exact current candidate:** `5ee6e8b10fcf311ab876855360e9a503f7690313`
- Parent: `3732cf4bdc8f9f9403d890094d281868e60f198d`
- Latest mutation: restore authenticated execution contract for report-export RPCs.
...
- Export RPC repair: `5ee6e8b10fcf311ab876855360e9a503f7690313`
...
| Reporting/export | PARTIAL | live RPC repair done; browser output still unproven |
...
4. **Real defect:** authenticated users could not execute `get_sales_export_rows`, `get_purchase_export_rows`, `get_inventory_export_rows`, or `get_receivables_export_rows`, while the authenticated Reports UI consumes export RPCs.
5. **Live repair:** authenticated EXECUTE restored for all four; anon EXECUTE explicitly denied.
...
8. Supabase security advisor still reports several authenticated-callable SECURITY DEFINER helpers and leaked-password protection disabled. Major mutation helpers inspected include tenant/auth checks and secure search path; no exploit proven, so no blind revoke was performed.
...
All seven now have explicit expected-disposition contract coverage. Runtime source→parse→normalize→DB→reconcile→analytics→evidence→decision→output proof remains NOT PROVEN.
...
- RPC caller/signature/migration parity.
...
- Performance scale, Electron/native Windows, migration lineage and SECURITY DEFINER least-privilege review.
