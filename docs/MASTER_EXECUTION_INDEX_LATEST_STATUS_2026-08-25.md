...
`main` currently advances through the proactive closure sequence ending with the latest index commit after tenant/import/reconciliation hardening. The newest implementation commits in this cycle are:
- `3a942154` — authoritative tenant filters distinguished from legacy consumers.
- `fea5e382` — duplicate import lookup made tenant-authoritative and typed while preserving old callers without trusting their tenant argument.
- `f67c8ac` — folder import uses tenant-authoritative duplicate lookup.
- `7852ed05` — reconciliation exposes target-only/deleted keys without implicit deletion.
- `82538b5d` — reconciliation regression fixture covers deleted-row detection and explicit tombstone semantics.

Earlier verified closure work remains part of this state: authoritative KPI tenant resolution, evidence-bound decisions, fail-closed batch decisions, atomic import chunks, terminal failed-import state, report retry/recovery hardening, and K/L integration contracts.
...
| Deleted-row reconciliation | IMPLEMENTED | YES | PENDING CI | NOT PROVEN | NO |
...
| Import lineage/idempotency | IMPLEMENTED | YES | REVIEW | NOT PROVEN | NO |
...
- Duplicate-file lookup now resolves the tenant internally; legacy caller parameters are ignored and cannot select a tenant.
...
- Reconciliation now reports target-only keys as `deletedKeys`; this is observation only and never performs implicit deletion.
...
## CI truth
Recent CI has repeatedly reached the Tenant boundary before failing on remaining contract/consumer issues. Run `32878245777` on `bcdb47fe` failed at the Tenant legacy-consumer gate; subsequent commits explicitly corrected authoritative-filter detection and duplicate-lookup semantics. A new Quality run was automatically queued from the latest commit and remains the source of truth for the next gate result.
