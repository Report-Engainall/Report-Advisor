...
`Actual Verified Closure / Actionable Work Available`
...
Record an `UNDER-EXECUTION EVENT` when any of these occur:

- async work is running while independent actionable work exists and is not executed;
- NEXT is complete while executable NEXT+1 is skipped;
- NEXT+1 is complete while executable NEXT+2 is skipped;
- an external blocker exists while independent local work is left unexecuted;
- a discovery has an executable fix but only a report is produced.
...
`RELEASE-RELEVANT CLOSURE + VERIFIED EVIDENCE + REMAINING WORK REDUCTION + CERTIFICATION UNLOCKS`
...
|---|---|---|---|---|---|---|---|
| v4.0 | 2026-09-02 | v3.2 improved throughput but lacked explicit strategy-performance feedback and controlled layer separation | Execution could improve without measuring why; activity could be mistaken for progress | Add performance ledger, effectiveness, under/over-execution detection, command feedback, precedence, strategy memory, smart prioritization, and controlled evolution | v3.2 enforced execution behavior but did not formally govern strategy quality | Governance contract + adversarial anchor tests + exact-SHA index update | IN-PROGRESS until fresh exact-head CI is consumed |
