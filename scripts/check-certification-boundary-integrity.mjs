...
export function validateCertificationBoundary({ index, head, parent, changedFiles }) {
  const codeMatch = normalize(index).match(/CURRENT PROJECT STATE[\s\S]{0,1200}?Current code\/test candidate[^`]*`([0-9a-f]{40})`/i);
  const indexed = codeMatch?.[1]?.toLowerCase();
  ...
  try {
    execFileSync('git', ['merge-base', '--is-ancestor', indexed, head], { stdio: 'ignore' });
  } catch {
...
if (process.argv[1]?.endsWith('check-certification-boundary-integrity.mjs')) {
  ...
  const indexed = index.match(/CURRENT PROJECT STATE[\s\S]{0,1200}?Current code\/test candidate[^`]*`([0-9a-f]{40})`/i)?.[1];
