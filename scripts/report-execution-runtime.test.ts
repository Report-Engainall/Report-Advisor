...
for (const token of ['attempt >= max_attempts', "'dead_letter'", "status IN ('leased','processing')", 'lease_expires_at > now()', 'company_id=public.current_company_id()']) {
...
for (const rpc of ["rpc('advance_report_execution_checkpoint'", "rpc('complete_report_execution_job'", "rpc('retry_report_execution_job'"]) {
...
for (const invariant of [
  'loadSourceSnapshot',
  'sourceSnapshotId',
  'source.sourceHash !== input.sourceHash',
  'source.currentRows',
  ...
  assert.ok(runner.includes(invariant), `missing durable production invariant: ${invariant}`);
}
