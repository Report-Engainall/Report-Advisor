...
// Deterministic, synthetic authorization corpus. This harness is deliberately
// environment-neutral: live Supabase credentials are injected only by the
// authenticated-live runner and are never committed.
const corpus = {
...
// Contract vectors for the future authenticated-live adapter.
const cases = [
...
console.log('- SELECT/UPDATE/DELETE/RPC/export/storage/realtime boundary vectors');
...
console.log('- executed-decision + completed-work-item prerequisite vectors');
console.log('- corpus evidence kind is SYNTHETIC and is never production certification');
