...
async function currentTenant(page) { const token = await accessToken(page); let last = ''; for (let attempt = 1; attempt <= 8; attempt += 1) { const response = await fetch(`${supabaseURL}/rest/v1/rpc/current_company_id`, { method: 'POST', headers: { apikey: anonKey, Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: '{}' }); const body = await response.text(); if (response.ok) { const tenantId = body.replaceAll('"', '').trim(); assert.ok(tenantId, 'current_company_id must resolve a tenant'); return tenantId; } last = `current_company_id HTTP ${response.status}: ${body}`; if (![502, 503, 504, 544].includes(response.status) || attempt === 8) break; await page.waitForTimeout(1500 * attempt); } throw new Error(last || 'current_company_id failed'); }
