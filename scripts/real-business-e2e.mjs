...
async function importOne(page, entity, fields, marker) { await page.goto(`${baseURL}/import`, { waitUntil: 'networkidle', timeout: 30000 }); const entityButtonLabel = entity === 'customers' ? 'العملاء' : entity === 'products' ? 'المنتجات' : 'فواتير المبيعات'; await page.locator('button').filter({ hasText: new RegExp('^' + entityButtonLabel) }).first().click(); await page.locator('input[type="file"]').first().setInputFiles({ name: `${marker}.csv`, mimeType: 'text/csv', buffer: csvBuffer(fields) }); await page.getByText('المراجعة', { exact: true }).waitFor({ state: 'visible', timeout: 30000 }); const commit = page.getByRole('button', { name: /تأكيد الاستيراد/ }); const qualityApproval = page.getByRole('checkbox', { name: /موافقة جودة صريحة/ }); if (await qualityApproval.count() === 1 && await qualityApproval.isVisible()) { await qualityApproval.check(); await page.waitForTimeout(100); evidence.steps.push({ step: `import-quality-approval:${entity}`, status: 'PASS' }); } assert.equal(await commit.isEnabled(), true, `${entity} valid import must be enabled`); await commit.click(); try { await page.getByRole('heading', { name: 'اكتملت عملية الاستيراد', exact: true }).waitFor({ state: 'visible', timeout: 30000 }); } catch (error) { const visibleError = await page.locator('.bg-danger-50').allTextContents().catch(() => []); throw new Error(`${error instanceof Error ? error.message : String(error)} UI_ERRORS=${visibleError.join(' | ')}`); } evidence.steps.push({ step: `import:${entity}`, status: 'PASS' }); }
