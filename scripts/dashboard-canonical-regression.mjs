...
assert.equal(authoritativeRows.length,21); assert.equal(authoritativeRows.reduce((sum,row)=>sum+row.subtotal,0),210);
...
assert.match(executivePage,/fetchDashboardSnapshot/); assert.doesNotMatch(executivePage,/fetchDashboardKPIs/);
...
assert.match(inventorySql,/DROP FUNCTION IF EXISTS public\.get_inventory_report_snapshot\(integer,integer\)/);
assert.match(inventorySql,/p_filter text DEFAULT 'all'/); assert.match(inventorySql,/FROM paged/); assert.match(inventorySql,/ORDER BY updated_at DESC, id DESC OFFSET v_page\*v_page_size LIMIT v_page_size/);
assert.match(inventorySql,/'totalRows'/);assert.match(inventorySql,/'filteredRows'/);assert.match(inventorySql,/'lowStock'/);assert.match(inventorySql,/'outOfStock'/);assert.match(inventorySql,/'unknownRows'/);assert.match(inventorySql,/'totalValue'/);assert.match(inventorySql,/'INSUFFICIENT_DATA'/);
assert.match(inventorySql,/current_company_id\(\)/);assert.match(inventorySql,/SECURITY INVOKER/);
assert.match(adapter,/supabase\.rpc\('get_inventory_report_snapshot'/);assert.match(adapter,/p_filter: filter/);
assert.match(fs.readFileSync('src/pages/ReportsPage.tsx','utf8'),/fetchInventoryReportSnapshot\(0,25/);assert.doesNotMatch(fs.readFileSync('src/pages/ReportsPage.tsx','utf8'),/fetchInventoryBalances\(/);
assert.match(inventoryPage,/fetchInventoryReportSnapshot\(page, pageSize, filter\)/);assert.doesNotMatch(inventoryPage,/fetchInventoryBalances\(/);assert.match(inventoryPage,/snapshot\.totalValue/);assert.match(inventoryPage,/snapshot\.filteredRows/);assert.match(inventoryPage,/snapshot\.lowStock/);assert.match(inventoryPage,/snapshot\.outOfStock/);assert.doesNotMatch(inventoryPage,/balances\.reduce/);assert.doesNotMatch(inventoryPage,/balances\.filter/);

// Compatibility exports remain intentional infrastructure, but their dashboard functions delegate to the canonical adapter.
...
console.log('PASS InventoryPage and ReportsPage no longer consume unbounded inventory balances');
