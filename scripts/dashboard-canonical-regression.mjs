...
assert.deepEqual(offenders,[]);assert.match(fs.readFileSync('src/pages/DashboardPage.tsx','utf8'),/fetchDashboardSnapshot/);assert.match(reportsPage,/fetchDashboardSnapshot/);assert.match(executivePage,/fetchDashboardSnapshot/);assert.doesNotMatch(executivePage,/fetchDashboardKPIs/);assert.match(adapter,/supabase\.rpc\('get_dashboard_snapshot'/);assert.match(adapter,/status: row\.status === 'CONFIRMED' \? 'CONFIRMED' : row\.status === 'CALCULATED' \? 'CALCULATED' : 'INSUFFICIENT_DATA'/);assert.match(adapter,/dataStatus: row\.dataStatus === 'CALCULATED' \? 'CALCULATED' : row\.dataStatus === 'INSUFFICIENT_DATA' \? 'INSUFFICIENT_DATA' : 'NO_DATA'/);assert.match(queries,/fetchDashboardSnapshot/);assert.doesNotMatch(queries,/supabase\.from\('sales_invoices'\)\.select\('id,total,paid_amount/);
