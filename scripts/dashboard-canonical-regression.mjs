...
assert.match(dashboardPage,/const metricStatus=\(value:\s*number\s*\|\s*null\)/);for(const field of ['totalSales','grossProfit','totalReceivables','inventoryValue','totalCustomers','totalProducts','invoiceCount','collectionRate'])assert.match(dashboardPage,new RegExp(`status=\{metricStatus\\(kpis\\.${field}\\)\\}`));
