...
assert.deepEqual(offenders,[]);assert.match(fs.readFileSync('src/pages/DashboardPage.tsx','utf8'),/fetchDashboardSnapshot/);assert.match(reportsPage,/fetchDashboardSnapshot/);assert.match(executivePage,/fetchDashboardSnapshot/);assert.doesNotMatch(executivePage,/fetchDashboardKPIs/);assert.match(adapter,/supabase\.rpc\('get_dashboard_snapshot'/);const assertStatusContract=(source)=>{assert.match(source,/status: row\.status === 'CONFIRMED' \? 'CONFIRMED' : row\.status === 'CALCULATED' \? 'CALCULATED' : 'INSUFFICIENT_DATA'/);assert.match(source,/dataStatus: row\.dataStatus === 'CALCULATED' \? 'CALCULATED' : row\.dataStatus === 'INSUFFICIENT_DATA' \? 'INSUFFICIENT_DATA' : 'NO_DATA'/);};assertStatusContract(adapter);const tamperedStatus=adapter.replace(/status: row\.status === 'CONFIRMED' \? 'CONFIRMED' : row\.status === 'CALCULATED' \? 'CALCULATED' : 'INSUFFICIENT_DATA'/,'status: row.status === \'CALCULATED\' ? \'CALCULATED\' : \'INSUFFICIENT_DATA\'');assert.throws(()=>assertStatusContract(tamperedStatus));
// Regression lock for the proven live RPC shape: aging is an array and trend/category
// rows omit UI-only status/wrapper fields. The adapter must normalize, not reinterpret,
// the canonical payload. This test is intentionally source-level so it cannot require
// a live user session; its adversarial twin below proves removal of the normalizer fails.
assert.match(adapter,/function normalizeAging\(/);assert.match(adapter,/function normalizeTrend\(/);assert.match(adapter,/function normalizeCategories\(/);assert.match(adapter,/aging: normalizeAging\(row\.aging,kpis\)/);assert.match(adapter,/trend: normalizeTrend\(row\.trend,kpis\.status\)/);assert.match(adapter,/categories: normalizeCategories\(row\.categories\)/);
const tamperedShape=adapter.replace(/aging: normalizeAging\(row\.aging,kpis\)/,'aging: row.aging as unknown as AgingDashboard');assert.throws(()=>{assert.match(tamperedShape,/aging: normalizeAging\(row\.aging,kpis\)/);});
assert.match(queries,/fetchDashboardSnapshot/);assert.doesNotMatch(queries,/supabase\.from\('sales_invoices'\)\.select\('id,total,paid_amount/);
...
console.log('PASS canonical CONFIRMED/CALCULATED/NO_DATA statuses cannot be silently downgraded or upgraded');
console.log('PASS dashboard RPC payload shape is normalized at the frontend boundary');
console.log('PASS dashboard payload-normalization regression fails if the aging normalizer is removed');
