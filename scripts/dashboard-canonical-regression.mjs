...
// Inventory valuation legacy consumer proof: only the compatibility implementation may retain the legacy symbol.
const valuationOffenders=[];for(const file of sourceFiles('src')){const rel=path.relative(process.cwd(),file).replaceAll('\\','/');if(allowedLegacyFiles.has(rel))continue;const content=fs.readFileSync(file,'utf8');if(/\bfetchInventoryValuation\b/.test(content))valuationOffenders.push(rel);}assert.deepEqual(valuationOffenders,[]);
