...
assert.match(dashboardPage,/const metricStatus=\(value:number\|null\)/);for(const field of ['totalSales','grossProfit','totalReceivables','inventoryValue','totalCustomers','totalProducts','invoiceCount','collectionRate'])assert.match(dashboardPage,new RegExp(`status=\{metricStatus\\(kpis\\.${field}\\)\\}`));
