...
for(const command of ['npm run typecheck','npm run lint','npm run build','npm run perf:budget'])if(!workflow.includes(command))failures.push(`quality workflow missing ${command}`);
...
console.log(`Architecture contract passed: ${required.length} core modules and CI gates verified.`);
