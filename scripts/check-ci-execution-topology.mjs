...
const requiredQualityGates = [
  'test:phase-k-runtime', 'test:phase-l-runtime', 'test:phase-l-resumable-execution',
  'test:phase-m-certification', 'test:k-to-s-closure', 'test:k-to-s-deep-closure',
  'test:production-release-blockers', 'test:production-certification-contract',
];
for (const gate of requiredQualityGates) if (!quality.includes(gate)) throw new Error(`Canonical quality path missing: ${gate}`);
if (!quality.includes('push: {branches: [main, hardening/safe-prune-and-proof, hardening/gross-profit-truth-closure-f5]}')) throw new Error('Quality must remain the canonical main push gate and exact-head hardening vehicle');
...
function pushTrigger(text) {
  ...
  const start = match.index + match[0].length, rest = text.slice(start), nextTopLevel = rest.search(/^\S/m);
  return { present: true, config: nextTopLevel >= 0 ? rest.slice(0, nextTopLevel) : rest };
}
const pushWorkflows = [], canonicalMainPushWorkflows = [], broadPushWorkflows = [];
for (const file of names) {
  ...
  const targetsMain = /branches\s*:\s*\[?\s*main(?:\s*,[^\]]*)?\s*\]?/.test(config);
...
console.log(JSON.stringify({ contract: 'ci-execution-topology', canonicalPushGate: 'quality.yml', pushWorkflows, canonicalMainPushWorkflows, scopedPushWorkflows: pushWorkflows.filter((file) => !canonicalMainPushWorkflows.includes(file)), manualWaves: ['j-k-l-runtime-wave.yml', 'autonomy-safety-wave.yml', 'phase-f-live-resilience.yml'] }));
