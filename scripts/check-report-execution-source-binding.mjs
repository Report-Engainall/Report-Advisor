...
if (!/job\.tenantId !== input\.request\.tenantId/.test(source)) findings.push('tenant binding check is missing');
if (!/if \(!input\.sourceHash\.trim\(\)\)/.test(source)) findings.push('empty source hash is not rejected');

if (findings.length) {
  console.error('Report execution source binding contract: FAIL');
