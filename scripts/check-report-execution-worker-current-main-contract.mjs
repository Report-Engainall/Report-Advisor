...
assert.match(serverRunner, /durable-worker-adapter/);
assert.match(serverRunner, /commitImportBatchWithClient/);
assert.doesNotMatch(serverRunner, /SUPABASE_SERVICE_ROLE_KEY|serviceClient\s*\(/, 'canonical import server boundary must not depend on service-role credentials');
assert.match(serverRunner, /function authenticatedClient\(token: string\)/);
assert.match(serverRunner, /VITE_SUPABASE_ANON_KEY/);
assert.match(serverRunner, /global:\s*\{\s*headers:\s*\{\s*Authorization:\s*`Bearer \$\{token\}`/);
