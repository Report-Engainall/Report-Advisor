...
for (const [name, value] of Object.entries({ supabaseURL, anonKey, bucket })) if (!value) throw new Error(`STORAGE_RUNTIME_ENV_MISSING:${name}`);
for (const user of users) for (const [name, value] of Object.entries({ [`USER_${user.label}_EMAIL`]: user.email, [`USER_${user.label}_PASSWORD`]: user.password })) if (!value) throw new Error(`STORAGE_RUNTIME_ENV_MISSING:${name}`);

await fs.mkdir(reportDir, { recursive: true });
const browser = await chromium.launch({ headless: true });
const evidence = { exactHead, bucket, status: 'NOT_PROVEN', users: {}, checks: [], startedAt: new Date().toISOString() };
...
async function browserSession(user) {
  ...
  const tenantResponse = await page.evaluate(async ({ url, anonKey }) => {
    const raw = Object.entries(localStorage).find(([key]) => key.endsWith('-auth-token'))?.[1];
    const session = JSON.parse(raw);
...
try {
  ...
  const content = Buffer.from(`storage-e2e ${exactHead} ${Date.now()}`);
  cleanup.push(['A', aPath], ['B', bPath]);

  const uploadA = await storageRequest(sessionA.token, 'POST', `object/${encodeURIComponent(bucket)}/${aPath}`, content, { 'content-type': 'text/plain', 'x-upsert': 'false' });
  ...
  evidence.status = message.includes('Bucket not found') || /STORAGE_BUCKET|bucket/i.test(message) ? 'BLOCKED EXTERNAL' : 'FAIL';
  evidence.error = message;
  await fs.writeFile(`${reportDir}/failure.json`, JSON.stringify(evidence, null, 2));
  if (evidence.status === 'FAIL') process.exitCode = 1;
  else process.exitCode = 2;
} finally {
  if (sessionA && sessionB) {
    for (const [label, path] of cleanup.reverse()) {
      const session = label === 'A' ? sessionA : sessionB;
      await storageRequest(session.token, 'DELETE', `object/${encodeURIComponent(bucket)}`, JSON.stringify({ prefixes: [path] }), { 'content-type': 'application/json' }).catch(() => {});
    }
  }
