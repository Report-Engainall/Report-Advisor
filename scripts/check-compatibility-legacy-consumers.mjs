...
if (!compatibility.includes('const MAX_IMPORT_RECORD_ROWS = 500')) failures.push('import history must remain bounded at 500 rows');
if (!compatibility.includes("range(0, MAX_IMPORT_RECORD_ROWS - 1)")) failures.push('import history bound must be enforced in the query');
if (!compatibility.includes("count > MAX_IMPORT_RECORD_ROWS")) failures.push('import history overflow must fail closed');
if (!compatibility.includes('p_max_rows: 10000')) failures.push('export rows must remain bounded at 10,000');
