...
const failClosed = fs.readFileSync('supabase/migrations/20260827161000_dashboard_secondary_truth_fail_closed.sql', 'utf8');
...
for (const marker of ["CASE WHEN q.incomplete_rows = 0 THEN x.value ELSE NULL END", "CASE WHEN q.incomplete_rows = 0 THEN x.quantity ELSE NULL END", "CASE WHEN q.incomplete_rows = 0 THEN x.sales ELSE NULL END", 'report_dashboard_secondary_truth']) if (!failClosed.includes(marker)) throw new Error(`Dashboard secondary fail-closed refinement missing invariant: ${marker}`);
...
console.log('DASHBOARD_SECONDARY_TRUTH: PASS (server-side tenant-authoritative trend/ranking/category truth; incomplete financial evidence fails closed)');
