...
const hasRuntimeGate = /autonomyGate\s*\(/.test(supabase) && /autonomy_runtime_gate/.test(supabase);
const hasLegacyGate = /canAutonomouslyExecute/.test(supabase) || /canAutonomouslyExecute/.test(runtime);
if (!hasRuntimeGate && !hasLegacyGate) throw new Error('Autonomy runtime link missing: autonomyGate/autonomy_runtime_gate');

for (const token of ['is_continuous_trust_healthy', 'current_company_id()', 'critical_drift']) {
