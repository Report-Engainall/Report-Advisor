...
if (/COALESCE\(r\.(risk_budget|liquidity_reserved|service_level_target)/i.test(gateMatch)) {
  throw new Error('Control-plane execution gate must not coerce missing numeric constraints to zero');
}
if (!/r\.status='approved'/i.test(gateMatch) || !/is_continuous_trust_healthy/i.test(gateMatch)) {
  throw new Error('Control-plane execution gate is not fail-closed on approval/trust state');
}
...
for (const variants of roadmapRequirements) {
  ...
  if (!accepted.some(item => roadmap.includes(item))) throw new Error(`Control-plane roadmap item missing: ${accepted.join(' | ')}`);
}
