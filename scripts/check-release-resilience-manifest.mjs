...
if (!migrations.includes('20260825050000_operational_resilience_trust.sql')) throw new Error('Resilience migration is not present');
for (let i=1;i<migrations.length;i++) if (migrations[i] < migrations[i-1]) throw new Error('Migration ordering is not deterministic');
...
if (!workflow.includes('check-operational-resilience-contract.mjs')) throw new Error('Quality workflow missing operational resilience gate');
