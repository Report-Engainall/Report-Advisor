...
if (secondary.includes(".from('sales_invoices')") || secondary.includes(".from('sale_items')") || secondary.includes(".from('customers')") || secondary.includes(".from('products')")) throw new Error('secondary consumers must not query business tables directly');
...
const requiredSql = ['public.current_company_id()',"raise exception 'TENANT_CONTEXT_MISMATCH'","status not in ('cancelled', 'void')",'(p_from is null or s.invoice_date >= p_from)','(p_to is null or s.invoice_date <= p_to)','limit v_limit','generate_series','group by bucket, bucket_order',"revoke all on function public.get_sales_secondary_metrics",'grant execute on function public.get_sales_secondary_metrics'];
