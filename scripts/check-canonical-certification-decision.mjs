...
assert.equal(evaluateCanonicalCertificationDecision({
  decision: valid,
  expectedSourceSha,
  expectedManifestId,
  expectedCertificationRunId,
}), true);
...
for (const [name, patch, reason] of rejects) {
  assert.throws(() => evaluateCanonicalCertificationDecision({
    decision: { ...valid, ...patch },
    expectedSourceSha,
    expectedManifestId,
    expectedCertificationRunId,
  }), new RegExp(`CANONICAL_CERTIFICATION_REJECTED:${reason}`), name);
}
...
assert.throws(() => evaluateCanonicalCertificationDecision({
  ...
  expectedManifestId,
  expectedCertificationRunId,
}), /CANONICAL_CERTIFICATION_REJECTED:MISSING_DECISION/);

assert.throws(() => evaluateCanonicalCertificationDecision({
  decision: { ...valid, required_contracts: [...valid.required_contracts, 'tenant'] },
  expectedSourceSha,
  expectedManifestId,
  expectedCertificationRunId,
}), /CANONICAL_CERTIFICATION_REJECTED:MANDATORY_CONTRACT_SET_MISMATCH/);
