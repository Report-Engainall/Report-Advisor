...
assert.match(adapter,/topCustomers: requiredArray<TopEntity>\(row\.topCustomers\)\.slice\(0,10\)/);
assert.match(adapter,/topProducts: requiredArray<TopEntity>\(row\.topProducts\)\.slice\(0,10\)/);
assert.doesNotMatch(adapter,/get_dashboard_top_entities/);
...
console.log('PASS top customers/products are consumed from canonical dashboard snapshot; no secondary top-entities RPC');
