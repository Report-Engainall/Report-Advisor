...
page.on('response', async (response) => {
  ...
  if (response.status() >= 400 || /\/rpc\/(enqueue_report_execution_job|claim_report_execution_job|advance_report_execution_checkpoint|complete_report_execution_job|fail_report_execution_job|import_commit_batch)/.test(url)) {
    let body = '';
    try { body = (await response.text()).slice(0, 6000); } catch {}
...
try {
  ...
  await commit.click();
  evidence.lifecycle.push({ stage: 'commit-clicked', at: new Date().toISOString() });

  const deadline = Date.now() + 90000;
  while (Date.now() < deadline) {
    const bodyText = await page.locator('body').innerText();
    const failed = bodyText.includes('فشل الاستيراد:');
    const done = bodyText.includes('اكتملت عملية الاستيراد') || bodyText.includes('تم الاستيراد بنجاح') || bodyText.includes('إعادة استيراد');
    if (failed || done) {
      evidence.ui = {
        failed,
        done,
        errorText: failed ? (bodyText.match(/فشل الاستيراد:[^\n]*/)?.[0] || null) : null,
        stepText: bodyText.includes('النتيجة') ? 'done' : bodyText.includes('الكتابة') ? 'committing' : 'unknown',
      };
      break;
    }
    await page.waitForTimeout(1000);
