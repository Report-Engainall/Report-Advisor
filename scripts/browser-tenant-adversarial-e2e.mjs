...
async function login(email,password){
  ...
  await p.goto(baseURL,{waitUntil:'networkidle',timeout:30000});
  await p.locator('#login-email').fill(email); await p.locator('#login-password').fill(password); await p.getByRole('button',{name:'تسجيل الدخول'}).click();
  await p.waitForLoadState('networkidle').catch(()=>{}); await p.waitForTimeout(800);
  const token=await p.evaluate(()=>{ const x=Object.entries(localStorage).find(([k])=>k.endsWith('-auth-token'))?.[1]; return x?JSON.parse(x).access_token:null; });
  assert.ok(token,'browser access token missing'); return {p,token};
}
...
const tenantA=await rpc(A.p,A.token,'current_company_id'), tenantB=await rpc(B.p,B.token,'current_company_id');
