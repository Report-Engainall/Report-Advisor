...
for(const token of ['fs.watch','recursive:true','WATCH_FOLDER_PATH_OUTSIDE_ROOT','read-file','desktop-folder-watch:file'])if(!main.includes(token))throw new Error(`Missing native watcher safety token: ${token}`);
if(!main.includes('setInterval(rescan,30000)')&&!main.includes('setInterval(()=>void rescan(),30000)'))throw new Error('Missing native watcher polling token');
