...
const full=calculateGrossProfitTruth(rows);if(full.revenue!==320||full.cost!==null||full.grossProfit!==null)throw new Error(`full truth mismatch: ${JSON.stringify(full)}`);
const bounded=calculateGrossProfitTruth(rows,{startDate:'2026-01-01',endDate:'2026-01-31'});if(bounded.revenue!==240||bounded.cost!==140||bounded.grossProfit!==100)throw new Error(`inclusive boundary mismatch: ${JSON.stringify(bounded)}`);
