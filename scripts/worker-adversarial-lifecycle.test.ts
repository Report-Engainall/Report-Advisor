...
function pass(name: string, expected: unknown, actual: unknown, evidence: string) { assert.deepEqual(actual, expected, name); cases.push({ CASE: name, EXPECTED: expected, ACTUAL: actual, RESULT: 'PASS', EVIDENCE: evidence }); }
function reject(name: string, fn: () => unknown, pattern: RegExp, evidence: string) { assert.throws(fn, pattern); cases.push({ CASE: name, EXPECTED: 'reject', ACTUAL: 'reject', RESULT: 'PASS', EVIDENCE: evidence }); }
function blocked(name: string, reason: string) { cases.push({ CASE: name, RESULT: 'BLOCKED', BLOCKER: reason }); }
...
const first = queue.enqueue(base, 'run-a', 3); const duplicate = queue.enqueue(base, 'run-ignored', 3);
pass('duplicate delivery / same tenant idempotency', first.runId, duplicate.runId, 'queue runtime');
const tenantB = queue.enqueue({ ...base, tenantId: 'tenant-b' }, 'run-b', 3); pass('idempotency collision across tenants', true, tenantB.runId !== first.runId, 'queue runtime');
const leaseA = queue.claim('worker-a', 60_000); assert.ok(leaseA?.leaseToken); pass('worker A lease acquisition', 'running', queue.get('run-a')?.status, 'queue runtime');
...
try {
  ...
  const leaseB = queue.claim('worker-b', 60_000); assert.ok(leaseB?.leaseToken); pass('stale lease takeover', 'worker-b', leaseB.leaseOwner, 'queue runtime'); pass('fencing token rotation', true, leaseB.leaseToken !== leaseA.leaseToken, 'queue runtime');
  Date.now = realNow; reject('stale worker completion after takeover', () => queue.complete('run-a', 'worker-a', leaseA.leaseToken), /fencing token is stale/, 'queue runtime');
  queue.cancel('run-a', 'worker-b', leaseB.leaseToken); pass('terminal cancellation', 'cancelled', queue.get('run-a')?.status, 'queue runtime');
...
queue.enqueue({ ...base, idempotencyKey: 'retry' }, 'run-retry', 2); const r1 = queue.claim('worker-a', 60_000); assert.ok(r1?.leaseToken); queue.fail('run-retry', 'worker-a', r1.leaseToken, 'first failure'); const r2 = queue.claim('worker-b', 60_000); assert.ok(r2?.leaseToken); pass('retry attempt increment', 2, r2.attempts, 'queue runtime'); queue.fail('run-retry', 'worker-b', r2.leaseToken, 'terminal failure'); pass('maxAttempts terminalization', 1, queue.listDeadLetters().length, 'queue runtime'); pass('claim after terminal failure', undefined, queue.claim('worker-c', 60_000), 'queue runtime');
