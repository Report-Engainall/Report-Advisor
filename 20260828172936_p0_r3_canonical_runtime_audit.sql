...
BEGIN
  ...
  VALUES (v_company, v_action, TG_TABLE_NAME, v_entity_id,
...
CREATE TRIGGER trg_decision_approval_audit AFTER INSERT OR UPDATE OR DELETE ON public.decision_approvals FOR EACH ROW EXECUTE FUNCTION public.audit_decision_runtime_change();
DROP TRIGGER IF EXISTS trg_decision_work_item_audit ON public.decision_work_items;
CREATE TRIGGER trg_decision_work_item_audit AFTER INSERT OR UPDATE OR DELETE ON public.decision_work_items FOR EACH ROW EXECUTE FUNCTION public.audit_decision_runtime_change();
DROP TRIGGER IF EXISTS trg_decision_action_receipt_audit ON public.decision_action_receipts;
CREATE TRIGGER trg_decision_action_receipt_audit AFTER INSERT OR UPDATE OR DELETE ON public.decision_action_receipts FOR EACH ROW EXECUTE FUNCTION public.audit_decision_runtime_change();
