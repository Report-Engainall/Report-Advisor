...
CREATE TABLE IF NOT EXISTS report_source_versions (
  ...
  source_path text NOT NULL,
  source_hash text NOT NULL,
  previous_version_id uuid REFERENCES report_source_versions(id) ON DELETE SET NULL,
  detected_at timestamptz NOT NULL DEFAULT now(),
  change_kind text NOT NULL CHECK (change_kind IN ('new','changed','renamed','deleted','unchanged')),
...
CREATE TABLE IF NOT EXISTS canonical_text_artifacts (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id uuid NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
  source_version_id uuid NOT NULL REFERENCES report_source_versions(id) ON DELETE CASCADE,
  source_hash text NOT NULL,
  text_hash text NOT NULL,
  processing_version text NOT NULL,
  extraction_route text NOT NULL CHECK (extraction_route IN ('pdf_text','table','ocr','structured','fallback')),
...
CREATE TABLE IF NOT EXISTS report_row_lineage (
  ...
  prior_row_hash text,
  state text NOT NULL CHECK (state IN ('new','changed','unchanged','deleted','quarantined')),
...
CREATE TABLE IF NOT EXISTS report_consolidation_runs (
  ...
  status text NOT NULL CHECK (status IN ('proposed','running','completed','blocked','failed')) DEFAULT 'proposed',
...
CREATE TABLE IF NOT EXISTS report_consolidation_items (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id uuid NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
  run_id uuid NOT NULL REFERENCES report_consolidation_runs(id) ON DELETE CASCADE,
  business_key text NOT NULL,
  source_version_id uuid NOT NULL REFERENCES report_source_versions(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS bounded_scenario_runs (
  ...
  protected_liquidity numeric NOT NULL DEFAULT 0 CHECK (protected_liquidity >= 0),
  input_evidence jsonb NOT NULL DEFAULT '{}'::jsonb,
  result jsonb NOT NULL DEFAULT '{}'::jsonb,
  status text NOT NULL CHECK (status IN ('proposed','computed','blocked','failed')) DEFAULT 'proposed',
  created_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS decision_portfolio_items (
  ...
  risk_consumption numeric NOT NULL DEFAULT 0 CHECK (risk_consumption >= 0),
  escalation_required boolean NOT NULL DEFAULT false,
  status text NOT NULL CHECK (status IN ('candidate','ready','approved','executed','deferred','blocked')) DEFAULT 'candidate',
  evidence jsonb NOT NULL DEFAULT '{}'::jsonb,
  updated_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS autonomy_domain_controls (
  ...
  min_evidence_quality numeric NOT NULL DEFAULT 0.9 CHECK (min_evidence_quality BETWEEN 0 AND 1),
  min_confidence numeric NOT NULL DEFAULT 0.9 CHECK (min_confidence BETWEEN 0 AND 1),
  max_risk_budget numeric NOT NULL DEFAULT 0 CHECK (max_risk_budget >= 0),
  rollback_enabled boolean NOT NULL DEFAULT true,
  trust_state text NOT NULL CHECK (trust_state IN ('eligible','degraded','blocked')) DEFAULT 'blocked',
  last_certified_at timestamptz,
  evidence jsonb NOT NULL DEFAULT '{}'::jsonb,
  UNIQUE(company_id,domain_key)
);
...
CREATE TABLE IF NOT EXISTS autonomy_certification_runs (
  ...
  risk_budget_valid boolean NOT NULL DEFAULT false,
...
CREATE TABLE IF NOT EXISTS autonomy_rollback_drills (
  ...
  status text NOT NULL CHECK (status IN ('planned','passed','failed')) DEFAULT 'planned',
...
ALTER TABLE report_execution_jobs ENABLE ROW LEVEL SECURITY;
ALTER TABLE report_source_versions ENABLE ROW LEVEL SECURITY;
ALTER TABLE canonical_text_artifacts ENABLE ROW LEVEL SECURITY;
ALTER TABLE report_row_lineage ENABLE ROW LEVEL SECURITY;
ALTER TABLE report_consolidation_runs ENABLE ROW LEVEL SECURITY;
ALTER TABLE report_consolidation_items ENABLE ROW LEVEL SECURITY;
ALTER TABLE bounded_scenario_runs ENABLE ROW LEVEL SECURITY;
ALTER TABLE decision_portfolio_items ENABLE ROW LEVEL SECURITY;
ALTER TABLE autonomy_domain_controls ENABLE ROW LEVEL SECURITY;
ALTER TABLE autonomy_certification_runs ENABLE ROW LEVEL SECURITY;
ALTER TABLE autonomy_rollback_drills ENABLE ROW LEVEL SECURITY;

REVOKE ALL ON TABLE report_execution_jobs,report_source_versions,canonical_text_artifacts,report_row_lineage,report_consolidation_runs,report_consolidation_items,bounded_scenario_runs,decision_portfolio_items,autonomy_domain_controls,autonomy_certification_runs,autonomy_rollback_drills FROM anon;
