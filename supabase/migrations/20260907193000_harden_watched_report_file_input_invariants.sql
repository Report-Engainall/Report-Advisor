...
-- Blank paths/hashes and negative sizes are not meaningful file provenance.
-- PostgreSQL regex [^[:space:]] rejects space, tab, newline and other whitespace-only values.

ALTER TABLE public.watched_report_files
  ADD CONSTRAINT watched_report_files_relative_path_nonblank_check
  CHECK (relative_path ~ '[^[:space:]]'),
...
BEGIN
  ...
  IF p_relative_path IS NULL OR p_relative_path !~ '[^[:space:]]' THEN RAISE EXCEPTION 'RELATIVE_PATH_REQUIRED'; END IF;
  IF p_content_hash IS NULL OR p_content_hash !~ '[^[:space:]]' THEN RAISE EXCEPTION 'CONTENT_HASH_REQUIRED'; END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE f.id = p_folder_id AND f.company_id = v_company_id
  ) THEN RAISE EXCEPTION 'FOLDER_TENANT_MISMATCH'; END IF;

  INSERT INTO public.watched_report_files(
    company_id, folder_id, relative_path, content_hash, size_bytes, modified_at, state
  ) VALUES (
    v_company_id, p_folder_id, p_relative_path, p_content_hash, p_size_bytes, p_modified_at, p_state
  )
  ...
  WHERE company_id = v_company_id AND folder_id = p_folder_id AND relative_path = p_relative_path;
  RETURN v_id;
END;
$$;
