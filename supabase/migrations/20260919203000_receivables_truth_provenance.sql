-- Forward-only provenance contract for the canonical receivables report.
-- Keeps the existing RPC name/signature and binds KPI totals + paged rows to one query snapshot.
...
DECLARE
  ...
  v_rows jsonb := '[]'::jsonb;
  v_as_of timestamptz := current_timestamp;
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'provenance', jsonb_build_object(
      ...
      'formula', 'sum(max(total - paid_amount, 0)) for non-cancelled outstanding invoices',
      'tenant_id', v_company_id,
      'as_of', v_as_of,
      'freshness', 'query_time',
      'period', jsonb_build_object('kind', 'as_of', 'as_of_date', v_as_of_date),
      'evidence', jsonb_build_object(
        'rpc', 'get_receivables_report_page',
        'filter', 'company=current_company_id AND total>paid_amount AND status<>cancelled'
      )
    )
  );
END;
