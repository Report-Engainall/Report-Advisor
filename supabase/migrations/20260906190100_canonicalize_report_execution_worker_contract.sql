...
-- This migration is intentionally idempotent so a fresh replay can converge on
-- the same table, constraints and lease-token fenced RPCs as the live runtime.
...
begin
  ...
  update public.report_execution_jobs
   ...
   where id=p_job_id and company_id=public.current_company_id()
     and status in ('leased','processing') and lease_owner=p_worker_id
...
create or replace function public.advance_report_execution_checkpoint(
  p_job_id uuid, p_worker_id text, p_lease_token uuid, p_checkpoint jsonb
)
returns boolean
language plpgsql
security definer
...
begin
  ...
  update public.report_execution_jobs
     set status='completed', evidence=p_evidence, lease_owner=null, lease_token=null,
...
create or replace function public.fail_report_execution_job(
  p_job_id uuid, p_worker_id text, p_lease_token uuid, p_error jsonb
)
returns boolean
language plpgsql
security definer
...
begin
  ...
  update public.report_execution_jobs
     set status=case when attempt>=max_attempts then 'dead_letter' else 'failed' end,
         last_error=p_error, lease_owner=null, lease_token=null, lease_expires_at=null,
...
begin
  update public.report_execution_jobs
     set status='queued', lease_owner=null, lease_token=null, lease_expires_at=null,
         last_error='{}'::jsonb, completed_at=null, updated_at=now()
   where id=p_job_id and company_id=public.current_company_id()
