...
CREATE TABLE IF NOT EXISTS entitlement_plan_capabilities (
  plan_id uuid NOT NULL REFERENCES entitlement_plans(id) ON DELETE CASCADE,
  capability text NOT NULL,
  quota numeric NOT NULL DEFAULT 0 CHECK (quota >= 0),
  enabled boolean NOT NULL DEFAULT true,
  PRIMARY KEY(plan_id, capability)
);
CREATE TABLE IF NOT EXISTS company_entitlements (
  company_id uuid PRIMARY KEY REFERENCES companies(id) ON DELETE CASCADE,
  plan_id uuid REFERENCES entitlement_plans(id),
...
RETURNS numeric LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  SELECT COALESCE(SUM(quantity),0) FROM entitlement_usage_events
  WHERE company_id = public.current_company_id() AND capability = p_capability AND billing_period = p_period;
$$;
