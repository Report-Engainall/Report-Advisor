...
begin
  ...
  if p_entity_type='customers' and jsonb_array_length(p_rows) <= 10 then
    for v_row in select value from jsonb_array_elements(p_rows) loop
      ...
      from public.import_upsert_customer(v_company_id,v_row->>'name',v_row->>'code',v_row->>'phone',v_row->>'email',v_row->>'segment',nullif(v_row->>'credit_limit','')::numeric,nullif(v_row->>'payment_terms_days','')::integer,p_null_policy);
