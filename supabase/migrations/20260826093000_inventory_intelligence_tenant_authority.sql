-- Tenant authority closure for inventory intelligence.
-- The previous contracts accepted p_company_id from callers. No repository consumer
-- was found for these RPCs; replace the signatures rather than preserving an
-- authorization-bearing compatibility overload.

DROP FUNCTION IF EXISTS public.demand_reorder_snapshot(uuid,date,integer,numeric,numeric);
DROP FUNCTION IF EXISTS public.inventory_liquidity_velocity(uuid,date,integer);
DROP FUNCTION IF EXISTS public.cash_liquidity_snapshot(uuid,date,date);
...
AS $$
WITH tenant AS (SELECT public.current_company_id() AS company_id),
sales AS (
  ...
  WHERE t.company_id IS NOT NULL
    AND inv.company_id = t.company_id
  ...
  CROSS JOIN tenant t
  WHERE t.company_id IS NOT NULL AND ib.company_id = t.company_id
  GROUP BY ib.product_id
), base AS (
  SELECT p.id, p.sku, p.name, coalesce(st.qty,0) stock_qty,
         CASE WHEN p.cost_price IS NULL OR st.qty IS NULL THEN NULL ELSE st.qty * p.cost_price END stock_value,
         s.last_sale,
  ...
  CROSS JOIN tenant t
  WHERE t.company_id IS NOT NULL AND p.company_id = t.company_id AND p.is_active
)
...
AS $$
WITH tenant AS (SELECT public.current_company_id() AS company_id),
days AS (SELECT generate_series(p_from,p_to,'1 day')::date d),
flow AS (
  ...
  FROM payments
  CROSS JOIN tenant t
  WHERE t.company_id IS NOT NULL AND payments.company_id=t.company_id
    AND payment_date BETWEEN p_from AND p_to
  GROUP BY payment_date
), x AS (
  SELECT days.d, inflow, outflow FROM days LEFT JOIN flow ON flow.d=days.d
), y AS (
...
REVOKE ALL ON FUNCTION public.inventory_liquidity_velocity(date,integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.demand_reorder_snapshot(date,integer,numeric,numeric) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.cash_liquidity_snapshot(date,date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.inventory_liquidity_velocity(date,integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.demand_reorder_snapshot(date,integer,numeric,numeric) TO authenticated;
GRANT EXECUTE ON FUNCTION public.cash_liquidity_snapshot(date,date) TO authenticated;
