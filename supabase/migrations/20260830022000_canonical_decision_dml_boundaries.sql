...
BEGIN
  ...
  INSERT INTO public.business_intelligence_decisions(
    company_id, decision_key, decision_type, status, confidence,
...
BEGIN
  ...
  INSERT INTO public.alerts(
    company_id, severity, category, title, description,
...
-- These tables are lifecycle-sensitive. Authenticated clients must use the
-- canonical RPCs above rather than issuing direct DML against the tables.
...
COMMENT ON FUNCTION public.create_runtime_recommendation(text,text,text,text,jsonb,numeric,uuid,jsonb)
IS 'Canonical tenant-authoritative recommendation creation boundary; authenticated clients must not insert directly.';
COMMENT ON FUNCTION public.create_runtime_decision(text,text,numeric,numeric,jsonb)
IS 'Canonical tenant-authoritative decision creation boundary; authenticated clients must not insert directly.';
COMMENT ON FUNCTION public.notify_decision_work_item(uuid,text,text)
IS 'Canonical tenant-authoritative decision work-item notification boundary; authenticated clients must not insert alerts directly.';
