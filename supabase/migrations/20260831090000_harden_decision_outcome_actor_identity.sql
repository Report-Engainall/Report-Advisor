-- Outcome provenance must retain the authenticated actor that recorded the observation.
...
CREATE INDEX IF NOT EXISTS idx_recommendation_outcomes_tenant_observed_by
  ON public.recommendation_outcomes(company_id, observed_by, observed_at DESC);
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.recommendations r WHERE r.id::text = p_recommendation_key AND r.company_id = v_company)
     AND NOT EXISTS (SELECT 1 FROM public.business_intelligence_decisions d WHERE d.decision_key = p_recommendation_key AND d.company_id = v_company) THEN
  ...
  INSERT INTO public.recommendation_outcomes(company_id,recommendation_key,decision_id,observed_at,observed_by,expected_impact,actual_impact,outcome_quality,status,evidence)
  VALUES(v_company,p_recommendation_key,p_decision_id,p_observed_at,v_user,p_expected_impact,p_actual_impact,p_outcome_quality,p_status,COALESCE(p_evidence,'{}'::jsonb))
  ON CONFLICT(company_id,recommendation_key) DO UPDATE SET
    decision_id=EXCLUDED.decision_id, observed_at=EXCLUDED.observed_at, observed_by=EXCLUDED.observed_by,
    expected_impact=EXCLUDED.expected_impact, actual_impact=EXCLUDED.actual_impact,
    outcome_quality=EXCLUDED.outcome_quality, status=EXCLUDED.status, evidence=EXCLUDED.evidence
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.business_intelligence_decisions d WHERE d.company_id=v_company AND d.decision_key=p_decision_fingerprint)
     AND NOT EXISTS (SELECT 1 FROM public.decision_action_receipts r WHERE r.company_id=v_company AND r.decision_fingerprint=p_decision_fingerprint) THEN RAISE EXCEPTION 'OUTCOME_PROVENANCE_NOT_FOUND'; END IF;
