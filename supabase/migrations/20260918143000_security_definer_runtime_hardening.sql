...
BEGIN
  ...
  VALUES(v_company,p_decision_key,p_decision_type,'PROPOSED',p_confidence,p_expected_impact,p_evidence)
  RETURNING id INTO v_id;
  RETURN v_id;
END; $$;
...
declare v_company uuid := public.current_company_id(); v_user uuid := auth.uid(); v_decision uuid; v_decision_status text; v_status text; v_requested_by uuid;
begin
  if v_company is null or v_user is null then raise exception 'TENANT_CONTEXT_REQUIRED'; end if;
...
declare v_company_id uuid; v_recommendation_company uuid; v_recommendation_decision uuid; v_decision_company uuid; v_decision_recommendation uuid;
begin
  v_company_id := public.current_company_id();
  if auth.uid() is null or v_company_id is null then raise exception 'TENANT_CONTEXT_REQUIRED'; end if;
  select company_id, decision_id into v_recommendation_company, v_recommendation_decision from public.recommendations where id=p_recommendation_id and company_id=v_company_id for update;
  if not found then raise exception 'RECOMMENDATION_NOT_FOUND_OR_FORBIDDEN'; end if;
  select company_id, recommendation_id into v_decision_company, v_decision_recommendation from public.business_intelligence_decisions where id=p_decision_id and company_id=v_company_id for update;
  if not found then raise exception 'DECISION_NOT_FOUND_OR_FORBIDDEN'; end if;
  if v_recommendation_decision is not null and v_recommendation_decision <> p_decision_id then raise exception 'RECOMMENDATION_ALREADY_LINKED'; end if;
  if v_decision_recommendation is not null and v_decision_recommendation <> p_recommendation_id then raise exception 'DECISION_ALREADY_LINKED'; end if;
...
BEGIN
  ...
  INSERT INTO public.alerts(company_id,severity,category,title,description,entity_type,entity_id) VALUES(v_company,'info','decision_action',p_title,p_description,'decision_work_item',p_work_item_id) RETURNING id INTO v_id;
  RETURN v_id;
END; $$;

CREATE OR REPLACE FUNCTION public.record_recommendation_outcome(p_recommendation_key text, p_observed_at timestamptz, p_expected_impact numeric DEFAULT NULL::numeric, p_actual_impact numeric DEFAULT NULL::numeric, p_outcome_quality numeric DEFAULT NULL::numeric, p_status text DEFAULT 'insufficient'::text, p_decision_id uuid DEFAULT NULL::uuid, p_evidence jsonb DEFAULT '{}'::jsonb)
RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_catalog AS $$
...
RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_catalog AS $$
declare v_company uuid := public.current_company_id(); v_id uuid; v_user uuid := auth.uid(); v_existing_status text; v_decision_status text;
begin
  if v_company is null or v_user is null then raise exception 'TENANT_CONTEXT_REQUIRED'; end if;
  select d.status into v_decision_status from public.business_intelligence_decisions d where d.id = p_decision_id and d.company_id = v_company for update;
  if v_decision_status is distinct from 'PROPOSED' then raise exception 'DECISION_NOT_APPROVABLE'; end if;
  select a.status into v_existing_status from public.decision_approvals a where a.company_id = v_company and a.decision_id = p_decision_id for update;
  if v_existing_status in ('APPROVED','REJECTED','CANCELLED') then raise exception 'APPROVAL_TERMINAL_NOT_REOPENABLE'; end if;
  insert into public.decision_approvals(company_id,decision_id,status,requested_by,reason) values(v_company,p_decision_id,'PENDING',v_user,p_reason) on conflict(company_id,decision_id) do update set status='PENDING',requested_by=excluded.requested_by,reason=excluded.reason,requested_at=now(),decided_at=null,decided_by=null where public.decision_approvals.status not in ('APPROVED','REJECTED','CANCELLED') returning id into v_id;
  if v_id is null then raise exception 'APPROVAL_TERMINAL_NOT_REOPENABLE'; end if;
  return v_id;
end; $$;
...
BEGIN
  ...
  UPDATE public.report_execution_jobs SET status='queued',lease_owner=null,lease_token=null,lease_expires_at=null,last_error='{}'::jsonb,updated_at=clock_timestamp() WHERE id=p_job_id AND company_id=p_company_id AND status='failed' AND attempt<max_attempts;
...
GRANT EXECUTE ON FUNCTION public.create_decision_work_item(uuid,uuid,text,uuid,text,text,text,text,timestamptz,numeric,jsonb) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.create_runtime_decision(text,text,numeric,numeric,jsonb) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.decide_approval(uuid,boolean,text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.link_recommendation_to_decision(uuid,uuid) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_decision_work_item(uuid,text,text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.record_recommendation_outcome(text,timestamptz,numeric,numeric,numeric,text,uuid,jsonb) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.request_decision_approval(uuid,text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.retry_report_execution_job(uuid,uuid) TO authenticated, service_role;
