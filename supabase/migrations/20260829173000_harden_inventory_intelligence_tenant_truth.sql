-- Canonical tenant/security hardening for inventory intelligence primitives.
-- These functions previously existed only in migration history and were not present
-- in the target database. They now fail closed on tenant mismatch, bound analysis
-- windows, use SECURITY INVOKER, fixed search_path, and authenticated-only execute.

CREATE OR REPLACE FUNCTION public.inventory_liquidity_velocity(
  p_company_id uuid,
  p_as_of date DEFAULT current_date,
  p_days integer DEFAULT 90
)
RETURNS TABLE(
  product_id uuid, sku text, product_name text, stock_qty numeric, stock_value numeric,
  last_sale_date date, days_since_sale integer, avg_daily_sales numeric,
  avg_weekly_sales numeric, avg_half_month_sales numeric, avg_monthly_sales numeric,
  avg_half_year_sales numeric, avg_yearly_sales numeric, estimated_days_to_clear numeric,
...
BEGIN
  ...
  IF p_days < 1 OR p_days > 3650 THEN
    RAISE EXCEPTION 'INVALID_ANALYSIS_WINDOW';
  END IF;
  IF p_as_of > current_date THEN
    RAISE EXCEPTION 'INVALID_AS_OF_DATE';
  END IF;

  RETURN QUERY
  WITH sales AS (
    SELECT si.product_id,
           sum(si.quantity) FILTER (WHERE inv.invoice_date >= p_as_of - (greatest(p_days,1)-1)) AS qty,
...
BEGIN
  ...
  IF p_days < 1 OR p_days > 3650 OR p_lead_time_days < 0 OR p_safety_days < 0 THEN
    RAISE EXCEPTION 'INVALID_REORDER_PARAMETERS';
  END IF;
  RETURN QUERY
  WITH v AS (
    SELECT * FROM public.inventory_liquidity_velocity(p_company_id,p_as_of,p_days)
  ), x AS (
    SELECT v.*, coalesce(avg_daily_sales,0) d, coalesce(stock_qty,0) s FROM v
  )
...
RETURNS TABLE(
  period_date date, inflows numeric, outflows numeric, net_cash numeric,
  cumulative_net numeric, risk_level text
)
LANGUAGE plpgsql STABLE SECURITY INVOKER SET search_path TO public
AS $$
BEGIN
  IF p_company_id IS NULL OR public.current_company_id() IS NULL THEN RAISE EXCEPTION 'TENANT_REQUIRED'; END IF;
  IF p_company_id <> public.current_company_id() THEN RAISE EXCEPTION 'TENANT_CONTEXT_MISMATCH'; END IF;
  IF p_from > p_to OR p_to-p_from > 366 THEN RAISE EXCEPTION 'INVALID_CASHFLOW_WINDOW'; END IF;
  RETURN QUERY
...
REVOKE ALL ON FUNCTION public.inventory_liquidity_velocity(uuid,date,integer) FROM PUBLIC,anon;
REVOKE ALL ON FUNCTION public.demand_reorder_snapshot(uuid,date,integer,numeric,numeric) FROM PUBLIC,anon;
REVOKE ALL ON FUNCTION public.cash_liquidity_snapshot(uuid,date,date) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.inventory_liquidity_velocity(uuid,date,integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.demand_reorder_snapshot(uuid,date,integer,numeric,numeric) TO authenticated;
GRANT EXECUTE ON FUNCTION public.cash_liquidity_snapshot(uuid,date,date) TO authenticated;
