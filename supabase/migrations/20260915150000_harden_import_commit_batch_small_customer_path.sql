-- Surgical hardening for real customer persistence on large tenants.
-- Small customer batches do not need the bulk TEMP TABLE path; route them through
-- the canonical upsert inside the same import_commit_batch transaction.
-- Large customer batches retain the existing bulk path.
...
BEGIN
  ...
  IF p_entity_type='customers' AND jsonb_array_length(p_rows) <= 10 THEN
    -- Real small-batch path: no TEMP TABLE and no bulk tenant scan.
