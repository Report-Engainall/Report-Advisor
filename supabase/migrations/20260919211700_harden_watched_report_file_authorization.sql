-- Forward-only watched-report authorization hardening.
-- Authenticated callers may only record discovery states; terminal/control-plane
-- states remain trusted-worker/service territory.
...
DECLARE
  ...
  v_id uuid;
  v_existing_modified_at timestamptz;
...
BEGIN
  ...
  IF p_folder_id IS NULL OR p_relative_path IS NULL OR btrim(p_relative_path) = ''
     OR p_content_hash IS NULL OR btrim(p_content_hash) = '' THEN
    RAISE EXCEPTION 'WATCHED_FILE_IDENTITY_REQUIRED';
  END IF;
  IF p_size_bytes IS NULL OR p_size_bytes < 0 THEN
    RAISE EXCEPTION 'WATCHED_FILE_SIZE_INVALID';
  END IF;
  IF p_modified_at IS NULL THEN
    RAISE EXCEPTION 'WATCHED_FILE_MODIFIED_AT_REQUIRED';
  END IF;
  ...
  IF (auth.jwt()->>'role') = 'authenticated' THEN
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION 'WATCHED_FILE_ROLE_NOT_AUTHORIZED';
    END IF;
    IF p_state NOT IN ('new','changed','unchanged') THEN
      RAISE EXCEPTION 'WATCHED_FILE_CONTROL_STATE_FORBIDDEN';
    END IF;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM public.watched_report_folders f
    WHERE f.id = p_folder_id AND f.company_id = v_company
  ) THEN
    RAISE EXCEPTION 'FOLDER_NOT_FOUND_OR_FORBIDDEN';
  END IF;

  SELECT content_hash, modified_at
    INTO v_existing_hash, v_existing_modified_at
  ...
  IF v_existing_modified_at IS NOT NULL
     AND p_modified_at < v_existing_modified_at
     AND p_content_hash <> v_existing_hash THEN
    RAISE EXCEPTION 'WATCHED_FILE_STALE_SOURCE_VERSION';
  END IF;

  INSERT INTO public.watched_report_files(
    company_id,folder_id,relative_path,content_hash,size_bytes,modified_at,state
  )
  VALUES(
    v_company,p_folder_id,p_relative_path,p_content_hash,p_size_bytes,p_modified_at,p_state
  )
