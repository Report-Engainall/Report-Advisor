-- Close two real public SECURITY DEFINER exposure gaps identified by the live Supabase advisor.
-- 1) get_data_quality_snapshot is read-only and all source tables already expose
--    tenant-scoped authenticated SELECT policies, so it should execute as invoker.
-- 2) record_watched_report_file has no repository caller; keep the canonical
--    ingest boundary unavailable to browser-authenticated clients until a
--    deliberate authenticated adapter is introduced.
...
REVOKE ALL ON FUNCTION public.record_watched_report_file(uuid,text,text,bigint,timestamptz,text) FROM PUBLIC, anon, authenticated;
