-- Harden recommendation status writes so direct RPC callers cannot bypass the
-- decision/work lifecycle. UI aliases remain accepted but are constrained by
-- backend state and predecessor invariants.
...
begin
  ...
  select r.status,r.decision_id into v_current,v_decision
    from public.recommendations r
   where r.id=p_recommendation_id and r.company_id=v_company_id
   for update;
  if not found then raise exception 'RECOMMENDATION_NOT_FOUND_OR_FORBIDDEN'; end if;
  if v_current in ('approved','in_progress','completed','rejected','dismissed') and v_next<>v_current then
  ...
  if v_next in ('rejected','dismissed') and v_current not in ('new','OPEN') then raise exception 'RECOMMENDATION_INVALID_TRANSITION'; end if;
  if v_next='approved' then
    ...
    if v_decision is null or not exists(select 1 from public.business_intelligence_decisions d where d.id=v_decision and d.company_id=v_company_id and d.status='APPROVED') then
  ...
  if v_next='in_progress' then
    ...
    if not exists(select 1 from public.decision_work_items w where w.company_id=v_company_id and w.recommendation_id=p_recommendation_id and w.status='IN_PROGRESS') then
  ...
  if v_next='completed' then
    ...
    if not exists(select 1 from public.decision_work_items w where w.company_id=v_company_id and w.recommendation_id=p_recommendation_id and w.status='COMPLETED') then
