-- Fail-closed refinement for dashboard secondary truth.
-- When any transactional line is incomplete, no financial ranking/category/trend value may
-- escape the canonical RPC as a seemingly valid number. Presentation-only counts may remain.
...
WITH params AS (
  ...
  SELECT m.month_start,
         sum(i.line_total)::numeric AS sales,
         sum(i.cost_price * i.quantity)::numeric AS cost,
         count(DISTINCT inv.id)::bigint AS invoices
  FROM months m
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object(
    'id', x.customer_id,
    'name', coalesce(x.customer_name, x.customer_id::text),
    'value', CASE WHEN q.incomplete_rows = 0 THEN x.value ELSE NULL END
  ) ORDER BY x.value DESC), '[]'::jsonb) AS value
  FROM (
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object(
    'id', x.product_id,
    'name', coalesce(x.product_name, x.product_id::text),
    'value', CASE WHEN q.incomplete_rows = 0 THEN x.value ELSE NULL END,
    'secondary', CASE WHEN q.incomplete_rows = 0 THEN x.quantity ELSE NULL END
  ) ORDER BY x.value DESC), '[]'::jsonb) AS value
  FROM (
    ...
    GROUP BY i.product_id, p.name ORDER BY value DESC LIMIT 5
  ) x CROSS JOIN quality q
), categories AS (
  SELECT coalesce(jsonb_agg(jsonb_build_object(
    'name', x.name,
    'sales', CASE WHEN q.incomplete_rows = 0 THEN x.sales ELSE NULL END,
    'profit', CASE WHEN q.incomplete_rows = 0 THEN x.profit ELSE NULL END,
    'quantity', CASE WHEN q.incomplete_rows = 0 THEN x.quantity ELSE NULL END
  ) ORDER BY x.sales DESC), '[]'::jsonb) AS value
  FROM (
    SELECT coalesce(c.name, 'غير مصنف') name,
           sum(i.line_total)::numeric sales,
           sum(i.line_total - i.cost_price * i.quantity)::numeric profit,
