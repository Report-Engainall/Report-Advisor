...
CREATE INDEX IF NOT EXISTS idx_report_runs_queue ON report_runs(status, created_at);
CREATE INDEX IF NOT EXISTS idx_report_runs_company ON report_runs(company_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_report_runs_lease ON report_runs(status, lease_expires_at);

CREATE TABLE IF NOT EXISTS report_run_artifacts (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id uuid NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
  run_id uuid NOT NULL REFERENCES report_runs(id) ON DELETE CASCADE,
  format text NOT NULL CHECK (format IN ('web','pdf','xlsx')),
...
CREATE POLICY authenticated_report_runs_tenant ON report_runs FOR ALL TO authenticated
...
CREATE POLICY authenticated_report_run_artifacts_tenant ON report_run_artifacts FOR ALL TO authenticated
  USING (company_id = public.current_company_id()) WITH CHECK (company_id = public.current_company_id());
DROP POLICY IF EXISTS authenticated_report_run_evidence_tenant ON report_run_evidence;
CREATE POLICY authenticated_report_run_evidence_tenant ON report_run_evidence FOR ALL TO authenticated
  USING (company_id = public.current_company_id()) WITH CHECK (company_id = public.current_company_id());
DROP POLICY IF EXISTS authenticated_report_delivery_receipts_tenant ON report_delivery_receipts;
CREATE POLICY authenticated_report_delivery_receipts_tenant ON report_delivery_receipts FOR ALL TO authenticated
  USING (company_id = public.current_company_id()) WITH CHECK (company_id = public.current_company_id());
...
DECLARE v_id uuid;
BEGIN
  IF p_company_id IS NULL OR p_idempotency_key IS NULL OR p_idempotency_key = '' THEN RAISE EXCEPTION 'tenant and idempotency key are required'; END IF;
  ...
  VALUES(p_company_id,p_report_id,p_requested_by,p_idempotency_key,p_request_fingerprint,p_source_snapshot_id,coalesce(p_parameters,'{}'),coalesce(p_formats,'[]'),p_input_fingerprint,greatest(1,p_max_attempts))
...
BEGIN
  UPDATE report_runs SET status=p_status, error_code=p_error_code, error_message=p_error_message,
    lease_expires_at=NULL, worker_id=NULL, completed_at=CASE WHEN p_status IN ('succeeded','failed','cancelled') THEN now() ELSE completed_at END, updated_at=now()
  WHERE id=p_run_id AND worker_id=p_worker_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'run not found or lease not owned'; END IF;
END; $$;
