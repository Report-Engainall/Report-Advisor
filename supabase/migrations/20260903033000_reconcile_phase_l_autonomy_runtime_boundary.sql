-- Reconcile the Phase L autonomy runtime boundary that was present in the canonical
-- migration chain but absent from the live Staging migration history.
-- This restores the canonical cockpit tables and preserves the existing gate semantics.
...
CREATE INDEX IF NOT EXISTS idx_control_plane_health_latest ON public.control_plane_health_snapshots(company_id, observed_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_executive_evidence_graph_target ON public.executive_evidence_graph(company_id,target_type,target_key);
...
CREATE INDEX IF NOT EXISTS idx_autonomy_certification_evidence ON public.autonomy_certification_evidence(company_id,certification_key,passed);
