...
begin
  ...
  monthly as (
    select jsonb_agg(jsonb_build_object(
      ...
      'profit', coalesce(x.sales, 0) - coalesce(x.cost, 0),
      'invoices', coalesce(x.invoices, 0)
    ...
    left join (
      select date_trunc('month', fi.invoice_date)::date as month_start,
             sum(fi.subtotal) as sales,
             sum(coalesce(fi_cost.cost, 0)) as cost,
  ...
  top_customers as (
    ...
    from (
      ...
      group by fi.customer_id, c.name
      order by value desc
      limit v_limit
    ) q
  ),
  ...
  categories as (
    ...
    from (
      ...
      from filtered_items fi
      group by coalesce(fi.category_name, 'غير مصنف')
    ) q
  ),
  aging as (
    ...
    group by q.bucket, q.bucket_order, q.outstanding
  )
  select jsonb_build_object(
    'as_of', v_as_of,
    'months', v_months,
    'limit', v_limit,
    'monthly_trend', coalesce(monthly.jsonb_agg, '[]'::jsonb),
    'top_customers', top_customers.value,
    'top_products', top_products.value,
    'category_breakdown', categories.value,
    'aging_buckets', aging.value
  ) into v_result
