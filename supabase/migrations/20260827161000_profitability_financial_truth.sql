...
-- Cancelled/void invoices are excluded; source negatives remain evidence for returns/adjustments.
...
BEGIN
  ...
  ELSIF jsonb_array_length(v_reasons) = 0 THEN
    SELECT sum(s.total - s.tax_amount),
           coalesce((SELECT sum(si.quantity * si.cost_price)
                       FROM public.sale_items si
                      WHERE si.invoice_id = s.id), 0)
