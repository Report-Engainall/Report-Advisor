...
begin
  ...
  v_key_column := case p_entity_type
    ...
    when 'sales_invoices' then 'invoice_number'
    else 'code'
  end;
  ...
  $q$,
    p_entity_type,
    v_key_column,
    case when p_entity_type = 'customers' then
      ' or public.normalize_import_key(t.name) in (select public.normalize_import_key(value->>''name'') from jsonb_array_elements($3) value where nullif(btrim(value->>''name''), '''') is not null)'
    else '' end
  );
