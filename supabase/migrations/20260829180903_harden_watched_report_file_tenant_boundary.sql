-- Harden watched-report file recording against cross-tenant folder references.
-- The SECURITY DEFINER boundary remains intentional, but the folder must belong
-- to the authenticated caller's authoritative company context.
...
BEGIN
  ...
  IF p_folder_id IS NULL
     OR p_relative_path IS NULL OR btrim(p_relative_path) = ''
  ...
  IF p_state NOT IN (
    'new','changed','unchanged','processing',
    'processed','failed','dead_letter','deleted'
  ) THEN
  ...
  VALUES (
    v_company, p_folder_id, p_relative_path, p_content_hash,
