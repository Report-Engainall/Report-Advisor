...
-- Canonical IDs intentionally align with the existing semantic metric vocabulary.
-- Missing evidence is NULL/UNKNOWN, never a fabricated zero.
...
VALUES
('net_sales','Revenue','Authoritative posted/confirmed sales revenue after line-level discounts/taxes according to the sales policy.','SUM(sale_items.line_total)','{sales_invoices,sale_items}','{company,branch,customer,product,category,period}','invoice_date',1440,'finance',1,'{sales}'),
...
('gross_profit','Gross Profit','Revenue minus verified cost of sales.','net_sales - cost','{sales_invoices,sale_items}','{company,branch,product,category,period}','invoice_date',1440,'finance',1,'{net_sales,cost}'),
('gross_margin_pct','Gross Margin %','Gross profit divided by revenue when both are evidenced.','gross_profit / net_sales * 100','{sales_invoices,sale_items}','{company,branch,product,category,period}','invoice_date',1440,'finance',1,'{gross_profit,net_sales}'),
...
('cash_position','Cash Position','Recorded cash inflows minus cash outflows within the selected period.','SUM(inflows) - SUM(outflows)','{payments}','{company,period}','payment_date',120,'finance',1,'{payments}'),
('dso','DSO','Days sales outstanding using evidenced receivables and sales over the selected window.','receivables / net_sales * days','{sales_invoices,payments}','{company,period}','invoice_date',1440,'finance',1,'{receivables,net_sales}'),
...
base AS (
  SELECT sales.*, receivables.rows_count AS ar_rows, receivables.value AS ar_value, receivables.source_updated_at AS ar_updated_at,
    payables.rows_count AS ap_rows, payables.value AS ap_value, payables.source_updated_at AS ap_updated_at,
    stock.rows_count AS stock_rows, stock.value AS inventory_value, stock.source_updated_at AS inventory_updated_at,
    payments.rows_count AS payment_rows, payments.inflow, payments.outflow, payments.source_updated_at AS payment_updated_at
  FROM sales, receivables, payables, stock, payments
),
metrics AS (
  SELECT jsonb_build_array(
    jsonb_build_object('metric_id','net_sales','value',CASE WHEN rows_count > 0 THEN revenue ELSE NULL END,'status',CASE WHEN rows_count > 0 THEN 'CONFIRMED' ELSE 'UNKNOWN' END,'source_rows',rows_count,'source_updated_at',source_updated_at,'version',1),
