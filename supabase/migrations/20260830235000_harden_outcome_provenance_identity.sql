-- CYCLE-024: close authenticated outcome provenance identity bypasses.
-- Outcome writers must prove that the supplied decision/recommendation fingerprint
-- belongs to the caller's tenant before creating or updating outcome truth.
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM public.business_intelligence_decisions d
    WHERE d.company_id = v_company AND d.decision_key = p_decision_fingerprint
  ) AND NOT EXISTS (
    SELECT 1 FROM public.decision_action_receipts r
    WHERE r.company_id = v_company AND r.decision_fingerprint = p_decision_fingerprint
  ) THEN
    RAISE EXCEPTION 'OUTCOME_PROVENANCE_NOT_FOUND';
  END IF;
  ...
  INSERT INTO public.decision_outcomes(
    company_id,decision_fingerprint,evidence_snapshot_id,action_id,observed_at,
    label,actual_value,expected_value,impact_value,notes
  ) VALUES(
    v_company,p_decision_fingerprint,p_evidence_snapshot_id,p_action_id,p_observed_at,
    p_label,p_actual_value,p_expected_value,p_impact_value,p_notes
  )
