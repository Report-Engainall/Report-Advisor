...
begin
  ...
  update public.report_execution_jobs
   ...
   where id=p_job_id and company_id=p_company_id
     and status in ('leased','processing') and lease_owner=p_worker_id and lease_token=p_lease_token
...
begin
  ...
  select checkpoint->>'stage',checkpoint->>'sourceHash' into old_stage,old_hash
   ...
   where id=p_job_id and company_id=p_company_id and status in ('leased','processing')
     ...
     and lease_expires_at is not null and lease_expires_at>now() for update;
...
begin
  ...
  update public.report_execution_jobs
     set status='completed',evidence=p_evidence,lease_owner=null,lease_token=null,lease_expires_at=null,
