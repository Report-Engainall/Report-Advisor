-- Reassert the canonical work-item lifecycle after later migration drift.
-- A work item may be completed only from IN_PROGRESS, never from OPEN,
-- BLOCKED, CANCELLED, COMPLETED, or any future non-actionable state.
...
IS 'Canonical terminal boundary: only IN_PROGRESS work items attached to tenant-scoped APPROVED decisions can create outcomes and mark the decision EXECUTED.';
